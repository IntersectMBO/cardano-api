...
prop_check_default_alonzo_genesis_roundtrips = H.propertyOnce $ do
  ...
  forM_ eras $ \(Some aeo) -> do
    let defaultCostModels =
          L.mkCostModels (M.singleton L.PlutusV1 (L.agPlutusV1CostModel alonzoGenesisDefaults))
            <> fromMaybe mempty (L.agExtraConfig alonzoGenesisDefaults >>= L.aecCostModels)

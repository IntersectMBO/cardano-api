...
prop_golden_6 =
  matchMetadata
    ...
    (txMetadataSingleton
       0
       (TxMetaList [ TxMetaNumber 1, TxMetaText "a", TxMetaBytes "\x42" ]))
...
matchMetadata hasSchema jsonStr expectedMetadata =
  withFrozenCallStack $ H.propertyOnce $ do
...
prop_metadata_chunks :: ( Show str, Eq str, Monoid str )
                     => Gen str
                     -> (str -> TxMetadataValue)
...
tests =
  testGroup
    ...
    , testProperty
        "noschema json roundtrip via metadata"
        prop_noschema_json_roundtrip_via_metadata
    , testProperty
        "schema json roundtrip via metadata"
        prop_schema_json_roundtrip_via_metadata
    , testProperty
        "metadata roundtrip via schema json"

...
prop_golden_5 =
  matchMetadata TxMetadataJsonNoSchema [q|{"0": [] }|] (txMetadataSingleton 0 (TxMetaList []))
...
prop_golden_6 =
  matchMetadata
    TxMetadataJsonNoSchema
    [q|{"0": [1, "a", "0x42"] }|]
    (txMetadataSingleton 0 (TxMetaList [TxMetaNumber 1, TxMetaText "a", TxMetaBytes "\x42"]))

prop_golden_7 :: Property
prop_golden_7 =
  matchMetadata TxMetadataJsonNoSchema [q|{"0": {} }|] (txMetadataSingleton 0 (TxMetaMap []))
...
matchMetadata ::
  ...
  -> Property
matchMetadata hasSchema jsonStr expectedMetadata =
  withFrozenCallStack
    $ H.propertyOnce
    $ do
...
prop_noschema_json_roundtrip_via_metadata =
  H.property $ do
    json <- H.forAll (genJsonForTxMetadata TxMetadataJsonNoSchema)
    Right json
      === (fmap (metadataToJson TxMetadataJsonNoSchema) . metadataFromJson TxMetadataJsonNoSchema)
            json
...
prop_schema_json_roundtrip_via_metadata =
  H.property $ do
...
prop_metadata_roundtrip_via_schema_json =
  H.property $ do
...
prop_metadata_chunks ::
  ...
  -> Property
prop_metadata_chunks genStr toMetadataValue extractChunk =
  H.property $ do
    str <- H.forAll genStr
    case toMetadataValue str of
      metadataValue@(TxMetaList chunks) -> do
        ...
        where metadata = makeTransactionMetadata (Map.singleton 0 metadataValue)
      _ -> H.failure

...
module Test.Cardano.Api.Crypto ( tests ) where

import           Cardano.Api.Crypto.Ed25519Bip32
       ( Ed25519Bip32DSIGN, SignKeyDSIGN(..) )
        ...
        , testGroup
            ...
            , testProperty "SignKey"
              $ prop_cbor_with
                ...
                decodeSignKeyDSIGN
            , testProperty "Sig"
              $ prop_cbor_with @(SigDSIGN v) encodeSigDSIGN decodeSigDSIGN ]
        , testGroup
            "To/FromCBOR class"
...
instance ( DSIGNAlgorithm v
         , ContextDSIGN v ~ ()
         , Signable v ~ SignableRepresentation ) => Arbitrary (SigDSIGN v) where

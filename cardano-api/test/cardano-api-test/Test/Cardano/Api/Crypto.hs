  ...
  testGroup
    ...
    [ testGroup
        ...
        [ testGroup
            ...
            , testProperty "Sig"
                $ prop_raw_serialise @(SigDSIGN v) rawSerialiseSigDSIGN rawDeserialiseSigDSIGN
            ]
        , testGroup
            "size"
            [ testProperty "VerKey"
            ...
            , testProperty "Sig"
                $ prop_size_serialise @(SigDSIGN v) rawSerialiseSigDSIGN (sizeSigDSIGN (Proxy @v))
            ]
        , testGroup
            "direct CBOR"
            [ testProperty "VerKey"
                $ prop_cbor_with @(VerKeyDSIGN v) encodeVerKeyDSIGN decodeVerKeyDSIGN
            , testProperty "SignKey"
                $ prop_cbor_with @(SignKeyDSIGN v) encodeSignKeyDSIGN decodeSignKeyDSIGN
            , testProperty "Sig" $ prop_cbor_with @(SigDSIGN v) encodeSigDSIGN decodeSigDSIGN
            ]
        , testGroup
            "To/FromCBOR class"
        ...
        , testGroup
            ...
            [ testProperty "VerKey" $ prop_cbor_direct_vs_class @(VerKeyDSIGN v) encodeVerKeyDSIGN
            , testProperty "SignKey"
                $ prop_cbor_direct_vs_class @(SignKeyDSIGN v) encodeSignKeyDSIGN
            , testProperty "Sig" $ prop_cbor_direct_vs_class @(SigDSIGN v) encodeSigDSIGN
            ]
        ]
    , testGroup
        "verify"
...
instance (DSIGNAlgorithm v, ContextDSIGN v ~ (), Signable v ~ SignableRepresentation) =>
...
instance Eq (SignKeyDSIGN Ed25519Bip32DSIGN) where
  (SignKeyEd25519Bip32DSIGN x) == (SignKeyEd25519Bip32DSIGN y) = CC.unXPrv x == CC.unXPrv y
  (SignKeyEd25519Bip32DSIGN x) /= (SignKeyEd25519Bip32DSIGN y) = CC.unXPrv x /= CC.unXPrv y

...
module Test.Cardano.Api.KeysByron ( tests ) where

import           Cardano.Api
       ( AsType(AsByronKey, AsSigningKey), Key(deterministicSigningKey) )
...
prop_roundtrip_byron_key_CBOR = H.property $ do
  seed <- H.forAll
    $ deterministicSigningKey AsByronKey <$> Gen.genSeedForKey AsByronKey

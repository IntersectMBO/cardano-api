...
module Test.Cardano.Api.Eras ( tests ) where
...
prop_roundtrip_JSON_Shelley = property $ do
  anySbe <- forAll
    $ Gen.element
    $ id @[ AnyShelleyBasedEra ] [ minBound .. maxBound ]
...
prop_roundtrip_JSON_Cardano = property $ do
  anyEra
    <- forAll $ Gen.element $ id @[ AnyCardanoEra ] [ minBound .. maxBound ]
...
tests =
  testGroup
    ...
    [ testProperty
        "maxBound cardano matches shelley"
        prop_maxBound_CardanoMatchesShelley
    ...
    , testProperty
        "toJSON cardano matches shelley"
        prop_toJSON_CardanoMatchesShelley ]

...
prop_roundtrip_txbodycontent_txouts =
  H.property $ do
    ...
    body <-
      case createAndValidateTransactionBody era content of
  ...
  -- correctly resolved given a datum matching the hash was generated.
    ...
    matchDatum =
      \case
        (TxOutDatumHash _ dh, TxOutDatumInTx _ d) -> dh === hashScriptDataBytes d
        (a, b) -> a === b
  ...
  -- because V2 is a superset of V1. So we accept that as a valid conversion.
    matchRefScript ::
         MonadTest m => (ReferenceScript BabbageEra, ReferenceScript BabbageEra) -> m ()
    matchRefScript (a, b)

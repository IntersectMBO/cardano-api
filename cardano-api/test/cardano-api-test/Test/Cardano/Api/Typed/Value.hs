module Test.Cardano.Api.Typed.Value ( tests ) where

import           Cardano.Api
       ( MaryEraOnwards(..), ShelleyBasedEra(..), ValueNestedBundle(..)
       , ValueNestedRep(..), fromLedgerValue, valueFromNestedRep
       , valueToNestedRep )
...
prop_roundtrip_Value_JSON = property $ do
  v <- forAll
...
prop_roundtrip_Value_flatten_unflatten = property $ do
  v <- forAll
    $ fromLedgerValue ShelleyBasedEraConway
    <$> genValueDefault MaryEraOnwardsConway
...
prop_roundtrip_Value_unflatten_flatten = property $ do
...
prop_roundtrip_AssetName_JSON = property $ do
  v <- forAll genAssetName
  tripping v encode eitherDecode

prop_roundtrip_AssetName_JSONKey :: Property
prop_roundtrip_AssetName_JSONKey = property $ do
  v <- forAll genAssetName

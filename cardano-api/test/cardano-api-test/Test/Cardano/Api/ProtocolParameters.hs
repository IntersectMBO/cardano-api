...
import Cardano.Api (CardanoEra(..), ProtocolParametersConversionError, inEonForEra, prettyPrintJSON)
...
protocolParametersAreCompatible era =
  property $ do
    ValidatedSerializedPair {serializedProtocolParameters, serializedPParams = _} <-
      forAll $ genValidSerializedPair era
    void
      (leftFail
         (eitherDecode serializedProtocolParameters :: Either
            String
...
genValidSerializedPair era = do
  ...
  let mValidatedSerializedPair = do
        unrefinedPParams <-
          convertToLedgerProtocolParameters sbe unrefinedProtocolParameters :: (Either
                                                                                  ProtocolParametersConversionError
                                                                                  (LedgerProtocolParameters
                                                                                     era))
        let refinedProtocolParams =
              fromLedgerPParams sbe $ unLedgerProtocolParameters unrefinedPParams
        ...
        return
          $ ValidatedSerializedPair
              ...
              , serializedPParams =
                  LBS.fromStrict $ prettyPrintJSON . unLedgerProtocolParameters $ refinedPParams
              }
  ...
  where
    ...
    patchProtocolParamRepresentation o = do
      ...
      renameKey "committeeTermLength" "committeeMaxTermLength"
        =<< renameKey "minCommitteeSize" "committeeMinSize" (applyFilters filters o)

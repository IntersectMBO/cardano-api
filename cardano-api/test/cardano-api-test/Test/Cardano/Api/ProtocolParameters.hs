...
import           Cardano.Api
       ( CardanoEra(..), ProtocolParametersConversionError, inEonForEra
       , prettyPrintJSON )
import           Cardano.Api.Eon.ShelleyBasedEra
       ( ShelleyBasedEra(..), ShelleyLedgerEra )
import           Cardano.Api.Ledger              ( PParams(..) )
import           Cardano.Api.ProtocolParameters
       ( LedgerProtocolParameters(..), convertToLedgerProtocolParameters
       , fromLedgerPParams )

import           Control.Monad                   ( void )

import           Data.Aeson
       ( FromJSON, Object, ToJSON, eitherDecode )
...
import           Hedgehog
       ( (===), Gen, MonadTest, Property, forAll, property, success )
...
tests =
  testGroup
    ...
    [ testGroup
        ...
        [ testProperty "ShelleyEra"
          $ protocolParametersSerializeTheSame ShelleyEra
        , testProperty "AlonzoEra"
          $ protocolParametersSerializeTheSame AlonzoEra
        , testProperty "BabbageEra"
          $ protocolParametersSerializeTheSame BabbageEra ]
    , testGroup
        "ProtocolParameters ToJSON can be read by PParams FromJSON"
        [ testProperty "ShelleyEra"
          $ protocolParametersAreCompatible ShelleyEra
        , testProperty "AlonzoEra" $ protocolParametersAreCompatible AlonzoEra
        , testProperty "BabbageEra"
          $ protocolParametersAreCompatible BabbageEra ]
    , testGroup
        "PParams roundtrip"
        [ testProperty "ShelleyEra"
          $ roundtripBetweenPParamsAndLegacyProtocolParameters ShelleyEra
        , testProperty "AlonzoEra"
          $ roundtripBetweenPParamsAndLegacyProtocolParameters AlonzoEra
        , testProperty "BabbageEra"
          $ roundtripBetweenPParamsAndLegacyProtocolParameters BabbageEra ] ]
...
protocolParametersSerializeTheSame
  :: forall era. ToJSON (PParams (ShelleyLedgerEra era))
  => CardanoEra era
  -> Property
protocolParametersSerializeTheSame era = property $ do
  ValidatedSerializedPair { serializedProtocolParameters, serializedPParams }
...
protocolParametersAreCompatible
  ...
  => CardanoEra era
  -> Property
protocolParametersAreCompatible era = property $ do
  ValidatedSerializedPair
    { serializedProtocolParameters, serializedPParams = _ }
    <- forAll $ genValidSerializedPair era
  void
    (leftFail
       (eitherDecode serializedProtocolParameters
...
roundtripBetweenPParamsAndLegacyProtocolParameters
  :: forall era. ( FromJSON (PParams (ShelleyLedgerEra era))
                 , ToJSON (PParams (ShelleyLedgerEra era)) )
  => CardanoEra era
  -> Property
roundtripBetweenPParamsAndLegacyProtocolParameters era = property $ do
  ValidatedSerializedPair
    { serializedProtocolParameters, serializedPParams = _ }
    <- forAll $ genValidSerializedPair era
  patchedserializedProtocolParameters
    <- patchProtocolParamsJSONOrFail era serializedProtocolParameters
  case eitherDecode serializedProtocolParameters
    :: Either String (PParams (ShelleyLedgerEra era)) of
    Left err      -> fail err
    Right pParams -> prettyPrintJSON pParams
      === LBS.toStrict patchedserializedProtocolParameters
...
data ValidatedSerializedPair era =
  ValidatedSerializedPair
...
genValidSerializedPair :: forall era. ToJSON (PParams (ShelleyLedgerEra era))
                       => CardanoEra era
                       -> Gen (ValidatedSerializedPair era)
genValidSerializedPair era = do
  unrefinedProtocolParameters <- genProtocolParameters era
  let mValidatedSerializedPair = do
        unrefinedPParams
          <- convertToLedgerProtocolParameters sbe unrefinedProtocolParameters
            :: (Either
                  ProtocolParametersConversionError
                  (LedgerProtocolParameters era))
        let refinedProtocolParams =
              fromLedgerPParams sbe
              $ unLedgerProtocolParameters unrefinedPParams
        refinedPParams
          <- convertToLedgerProtocolParameters sbe refinedProtocolParams
        return
          $ ValidatedSerializedPair
          { serializedProtocolParameters =
              LBS.fromStrict $ prettyPrintJSON refinedProtocolParams
          , serializedPParams = LBS.fromStrict
              $ prettyPrintJSON . unLedgerProtocolParameters
              $ refinedPParams
          }
...
patchProtocolParamsJSONOrFail
  :: ( MonadTest m, MonadFail m )
  => CardanoEra era
  -> LBS.ByteString
  -> m LBS.ByteString
patchProtocolParamsJSONOrFail era s =
  LBS.fromStrict . prettyPrintJSON
  <$> (patchProtocolParamRepresentation =<< leftFail (eitherDecode s))
  where
    ...
    filtersForEra BabbageEra =
      return [ "decentralization", "extraPraosEntropy", "minUTxOValue" ]
    filtersForEra era'       =
      fail $ "filtersForEra is not defined for: " <> show era'

    applyFilters :: [ String ] -> Object -> Object
    applyFilters filters o =
      foldl' (flip Aeson.delete) o (map Aeson.fromString filters)
    ...
    renameKey src dest o =
      ...
      in  case Aeson.lookup srcKey o of
            ...
            Just v  -> if Aeson.member destKey o
              then fail
                $ "renameKey failed because there is already an entry with the new name: "
                <> dest

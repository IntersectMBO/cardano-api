...
module Test.Cardano.Api.EpochLeadership ( tests ) where

import           Cardano.Api
       ( Key(verificationKeyHash), deterministicSigningKey, getVerificationKey )
import           Cardano.Api.Block
       ( EpochNo(..), Hash(StakePoolKeyHash), SlotNo(..) )
...
import           Cardano.Api.Shelley
       ( Hash(VrfKeyHash), VrfKey, proxyToAsType, unStakePoolKeyHash )
...
tests :: TestTree
tests =
  testGroup "Epoch Leadership" [ test_currentEpochEligibleLeadershipSlots ]
  ...
  $ do
    let sbe = ShelleyBasedEraShelley
        ...
        eInfo =
          ...
          , epochInfoSlotToRelativeTime_ = \(SlotNo x) -> pure
              $ RelativeTime (secondsToNominalDiffTime (fromIntegral x * 60))
          , epochInfoSlotLength_ = const (pure $ mkSlotLength 100)
          }
        pp = emptyPParams
        chainDepState =
          TPraosState Origin (API.initialChainDepState NeutralNonce Map.empty)
        ptclState = encodeProtocolState chainDepState
        poolid =
          StakePoolKeyHash
          { unStakePoolKeyHash = KeyHash
              "83c5da842d7437e411d3c4db8aaa7a7d2c1642aee932108c9857282d"
          }
        vrskey1 =
        ...
        poolDistr :: PoolDistr StandardCrypto =
          ...
          $ Map.fromList
            [ ( KeyHash
                  "a2927c1e43974b036d8e6838d410279266946e8a094895cfc748c91d"
              , IndividualPoolStake
            ...
            , ( KeyHash
                  "362c2c2128ee75ca39690c27b42e809301231098003443669e2b03f3"
              , IndividualPoolStake
                { individualPoolStake    = 1 % 3
        ...
        serPoolDistr =
          SerialisedPoolDistribution
            (Serialised (serialize (toByronCBOR poolDistr)))
        ...
        eEligibleSlots =
          currentEpochEligibleLeadershipSlots
            ...
            serPoolDistr
            currentEpoch
        expectedEligibleSlots =
          [ SlotNo 406, SlotNo 432, SlotNo 437, SlotNo 443, SlotNo 484 ]

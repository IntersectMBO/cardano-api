...
import Cardano.Api (Key(verificationKeyHash), deterministicSigningKey, getVerificationKey)
...
import Cardano.Api.Query (ProtocolState(..), SerialisedPoolDistribution(SerialisedPoolDistribution))
...
import Ouroboros.Consensus.Shelley.Ledger.Query.Types (IndividualPoolStake(..), PoolDistr(..))
...
--
tests :: TestTree
tests = testGroup "Epoch Leadership" [test_currentEpochEligibleLeadershipSlots]
            ...
            eInfo =
              EpochInfo
                ...
                , epochInfoSlotToRelativeTime_ =
                    \(SlotNo x) ->
                      pure $ RelativeTime (secondsToNominalDiffTime (fromIntegral x * 60))
              ...
              PoolDistr
                $ Map.fromList
                    [ ( KeyHash "a2927c1e43974b036d8e6838d410279266946e8a094895cfc748c91d"
                      , IndividualPoolStake
                          {individualPoolStake = 1 % 3, individualPoolStakeVrf = hash1})
                    , ( KeyHash "83c5da842d7437e411d3c4db8aaa7a7d2c1642aee932108c9857282d"
                      , IndividualPoolStake
                          {individualPoolStake = 1 % 3, individualPoolStakeVrf = hash2})
                    , ( KeyHash "362c2c2128ee75ca39690c27b42e809301231098003443669e2b03f3"
                      , IndividualPoolStake
                          {individualPoolStake = 1 % 3, individualPoolStakeVrf = hash3})
                    ]
            serPoolDistr =
              SerialisedPoolDistribution (Serialised (serialize (toByronCBOR poolDistr)))

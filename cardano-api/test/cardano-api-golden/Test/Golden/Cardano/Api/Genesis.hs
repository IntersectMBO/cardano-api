...
module Test.Golden.Cardano.Api.Genesis ( exampleShelleyGenesis ) where
...
import           Cardano.Ledger.Credential
       ( Credential(..), PaymentCredential, StakeCredential
       , StakeReference(..) )
import           Cardano.Ledger.Crypto             ( StandardCrypto )
import           Cardano.Ledger.Keys
       ( GenDelegPair(..), Hash, KeyHash(..), KeyRole(..), VerKeyVRF )
...
exampleShelleyGenesis =
  ShelleyGenesis
  { sgSystemStart       = posixSecondsToUTCTime
      $ realToFrac (1234566789 :: Integer)
  ...
  where
    ...
    initialFundedAddress =
      Addr Testnet paymentCredential (StakeRefBase stakingCredential)
      where

...
test_golden_ProtocolParameters =
  testProperty "golden ProtocolParameters" $ do
...
goldenLegacyProtocolParametersToPParams ::
     forall pp. FromJSON pp
  => Proxy pp
  -> Property
goldenLegacyProtocolParametersToPParams proxy =
  property
    $ case decodedLegacyCardanoApiProtocolParameters of
        Left err ->
          failWith
            Nothing
...
legacyCardanoApiProtocolParameters =
  ...
  where
    ...
    executionUnitPrices =
      ExecutionUnitPrices {priceExecutionSteps = 0.3, priceExecutionMemory = 0.2}
    ...
    executionUnits = ExecutionUnits {executionSteps = 4300, executionMemory = 2300}
    executionUnits2 :: ExecutionUnits
    executionUnits2 = ExecutionUnits {executionSteps = 5600, executionMemory = 3400}

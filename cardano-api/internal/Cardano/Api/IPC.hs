...
module Cardano.Api.IPC
  ...
  , toAcquiringFailure
  , NodeToClientVersion(..)
  , UnsupportedNtcVersionError(..) ) where
...
import           Control.Concurrent.STM
       ( TMVar, atomically, newEmptyTMVarIO, putTMVar, takeTMVar, tryPutTMVar )
...
connectToLocalNodeWithVersion
  LocalNodeConnectInfo
  { localNodeSocketPath, localNodeNetworkId, localConsensusModeParams }
  clients = liftIO $ Net.withIOManager $ \iomgr -> Net.connectTo
...
mkVersionedProtocols
  :: forall block.
  ( Consensus.ShowQuery (Consensus.Query block), ProtocolClient block )
  => NetworkId
...
mkVersionedProtocols networkid ptcl unversionedClients =
  ...
  Net.foldMapVersions
    (\( ptclVersion, ptclBlockVersion ) -> Net.versionedNodeToClientProtocols
       ptclVersion
    ...
    protocols
      ...
      { localChainSyncProtocol    =
          Net.InitiatorProtocolOnly $ case localChainSyncClientForBlock of
            NoLocalChainSyncClient -> Net.mkMiniProtocolCbFromPeer
              $ const ( nullTracer, cChainSyncCodec, Net.chainSyncPeerNull )
            LocalChainSyncClient client -> Net.mkMiniProtocolCbFromPeer
              $ const
                ( nullTracer
                , cChainSyncCodec
                , Net.Sync.chainSyncClientPeer client )
            LocalChainSyncClientPipelined
              clientPipelined -> Net.mkMiniProtocolCbFromPeerPipelined
              $ const
                ( nullTracer
                , cChainSyncCodec
                , Net.SyncP.chainSyncClientPeerPipelined clientPipelined )
      , localTxSubmissionProtocol = Net.InitiatorProtocolOnly
          ...
          $ const
            ...
            , maybe
                ...
                localTxSubmissionClientForBlock )
      , localStateQueryProtocol   = Net.InitiatorProtocolOnly
          $ Net.mkMiniProtocolCbFromPeer
          $ const
            ( nullTracer
            , cStateQueryCodec
            , maybe
                ...
                localStateQueryClientForBlock )
      , localTxMonitorProtocol    = Net.InitiatorProtocolOnly
          $ Net.mkMiniProtocolCbFromPeer
          $ const
            ( nullTracer
            , cTxMonitorCodec
...
data LocalNodeClientProtocolsForBlock block =
  LocalNodeClientProtocolsForBlock
  { localChainSyncClientForBlock
      :: LocalChainSyncClient block (Consensus.Point block) (Net.Tip block) IO
  , localStateQueryClientForBlock
      :: Maybe
        (LocalStateQueryClient
           block
...
mkLocalNodeClientParams modeparams clients =
  ...
  --
  case modeparams of
    CardanoModeParams epochSlots -> LocalNodeClientParamsCardano
...
convLocalNodeClientProtocols
  LocalNodeClientProtocols { localChainSyncClient
                           , localTxSubmissionClient
                           , localStateQueryClient
  ...
  { localChainSyncClientForBlock    = case localChainSyncClient of
      ...
      LocalChainSyncClientPipelined
        clientPipelined -> LocalChainSyncClientPipelined
        $ convLocalChainSyncClientPipelined clientPipelined
      LocalChainSyncClient
        client -> LocalChainSyncClient $ convLocalChainSyncClient client
  , localTxSubmissionClientForBlock =
      convLocalTxSubmissionClient <$> localTxSubmissionClient
  , localStateQueryClientForBlock   =
      convLocalStateQueryClient <$> localStateQueryClient
  , localTxMonitoringClientForBlock =
      convLocalTxMonitoringClient <$> localTxMonitoringClient
  }
...
convLocalTxMonitoringClient
  ...
  -> LocalTxMonitorClient
    ...
    a
convLocalTxMonitoringClient =
  mapLocalTxMonitoringClient toConsensusTxId fromConsensusGenTx
...
mapLocalTxMonitoringClient convTxid convTx ltxmc =
  ...
  where
    convClientStateIdle :: CTxMon.ClientStIdle txid tx SlotNo m a
...
queryNodeLocalState connctInfo mpoint query = do
  ...
  where
    singleQuery :: Net.Query.Target ChainPoint
                -> TMVar (Either AcquiringFailure result)
                -> Net.Query.LocalStateQueryClient
                  BlockInMode
                  ChainPoint
                  QueryInMode
                  IO
                  ()
    singleQuery mPointVar' resultVar' = LocalStateQueryClient $ do
      pure
        ...
        , Net.Query.recvMsgFailure  = \failure -> do
            atomically
              $ putTMVar resultVar' (Left (toAcquiringFailure failure))
            pure $ Net.Query.SendMsgDone ()
        }

submitTxToNodeLocal :: MonadIO m
                    => LocalNodeConnectInfo
                    -> TxInMode
...
submitTxToNodeLocal connctInfo tx = do
  ...
  where
    ...
    localTxSubmissionClientSingle resultVar =
      LocalTxSubmissionClient $ pure $ Net.Tx.SendMsgSubmitTx tx $ \result -> do
...
queryTxMonitoringLocal :: MonadIO m
                       => LocalNodeConnectInfo
                       -> LocalTxMonitoringQuery
...
queryTxMonitoringLocal connectInfo localTxMonitoringQuery = do
  ...
  where
    ...
    localTxMonitorClientTxExists tIdInMode@(TxIdInMode _ txid) resultVar = do
      LocalTxMonitorClient $ return $ CTxMon.SendMsgAcquire $ \slt -> do
        return $ CTxMon.SendMsgHasTx tIdInMode $ \txPresentBool -> do
          if txPresentBool
            then atomically . putTMVar resultVar
              $ LocalTxMonitoringTxExists txid slt
            else atomically . putTMVar resultVar
              $ LocalTxMonitoringTxDoesNotExist txid slt
    ...
    localTxMonitorNextTx
      ...
      => TMVar LocalTxMonitoringResult
      -> LocalTxMonitorClient TxIdInMode TxInMode SlotNo IO ()
    localTxMonitorNextTx resultVar = LocalTxMonitorClient $ return $ do
    ...
    localTxMonitorMempoolInfo
      ...
      => TMVar LocalTxMonitoringResult
      -> LocalTxMonitorClient TxIdInMode TxInMode SlotNo IO ()
    localTxMonitorMempoolInfo resultVar = LocalTxMonitorClient $ return $ do
      CTxMon.SendMsgAcquire $ \slt -> do

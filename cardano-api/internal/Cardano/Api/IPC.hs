...
import Ouroboros.Network.NodeToClient (NodeToClientProtocols(..), NodeToClientVersionData(..))
...
import Control.Concurrent.STM (TMVar, atomically, newEmptyTMVarIO, putTMVar, takeTMVar, tryPutTMVar)
...
connectToLocalNodeWithVersion ::
  ...
  -> m ()
connectToLocalNodeWithVersion LocalNodeConnectInfo { localNodeSocketPath
                                                   , localNodeNetworkId
    ...
    $ \iomgr ->
        Net.connectTo
          ...
          Net.NetworkConnectTracers
            {Net.nctMuxTracer = nullTracer, Net.nctHandshakeTracer = nullTracer}
          versionedProtocls
...
mkVersionedProtocols ::
     forall block. (Consensus.ShowQuery (Consensus.Query block), ProtocolClient block)
  => NetworkId
...
mkVersionedProtocols networkid ptcl unversionedClients
 ...
 =
  Net.foldMapVersions
    (\(ptclVersion, ptclBlockVersion) ->
       Net.versionedNodeToClientProtocols
         ptclVersion
         NodeToClientVersionData {networkMagic = toNetworkMagic networkid, query = False}
  ...
  where
    ...
    protocols LocalNodeClientProtocolsForBlock { localChainSyncClientForBlock
                                               , localTxSubmissionClientForBlock
                                               , localStateQueryClientForBlock
                                               , localTxMonitoringClientForBlock
                                               } ptclBlockVersion ptclVersion =
      NodeToClientProtocols
        { localChainSyncProtocol =
            Net.InitiatorProtocolOnly
              $ case localChainSyncClientForBlock of
                  NoLocalChainSyncClient ->
                    Net.mkMiniProtocolCbFromPeer
                      $ const (nullTracer, cChainSyncCodec, Net.chainSyncPeerNull)
                  LocalChainSyncClient client ->
                    Net.mkMiniProtocolCbFromPeer
                      $ const (nullTracer, cChainSyncCodec, Net.Sync.chainSyncClientPeer client)
      ...
      where
        Consensus.Codecs { Consensus.cChainSyncCodec
    ...
    codecConfig = Consensus.pClientInfoCodecConfig (protocolClientInfo ptcl)
...
data LocalNodeClientParams where
  ...
  LocalNodeClientParamsCardano
    :: (ProtocolClient block, CardanoHardForkConstraints (ConsensusCryptoForBlock block))=> ProtocolClientInfoArgs
         block
    -> (NodeToClientVersion -> LocalNodeClientProtocolsForBlock block)
    -> LocalNodeClientParams

data LocalNodeClientProtocolsForBlock block = LocalNodeClientProtocolsForBlock
  { localChainSyncClientForBlock :: LocalChainSyncClient
      block
      (Consensus.Point block)
      (Net.Tip block)
      IO
  , localStateQueryClientForBlock :: Maybe
      (LocalStateQueryClient block (Consensus.Point block) (Consensus.Query block) IO ())
  , localTxSubmissionClientForBlock :: Maybe
      (LocalTxSubmissionClient (Consensus.GenTx block) (Consensus.ApplyTxErr block) IO ())
...
convLocalNodeClientProtocols ::
  ...
  => Consensus.CardanoBlock L.StandardCrypto ~ block =>
       LocalNodeClientProtocolsInMode -> LocalNodeClientProtocolsForBlock block
convLocalNodeClientProtocols LocalNodeClientProtocols { localChainSyncClient
                                                      , localTxSubmissionClient
                                                      , localStateQueryClient
  ...
  LocalNodeClientProtocolsForBlock
    { localChainSyncClientForBlock =
        case localChainSyncClient of
          NoLocalChainSyncClient -> NoLocalChainSyncClient
          LocalChainSyncClientPipelined clientPipelined ->
            LocalChainSyncClientPipelined $ convLocalChainSyncClientPipelined clientPipelined
...
convLocalTxMonitoringClient ::
  ...
  => Consensus.CardanoBlock L.StandardCrypto ~ block =>
       Functor m =>
         LocalTxMonitorClient TxIdInMode TxInMode SlotNo m a -> LocalTxMonitorClient
                                                                  (Consensus.TxId
                                                                     (Consensus.GenTx block))
                                                                  (Consensus.GenTx block)
                                                                  SlotNo
                                                                  m
                                                                  a
convLocalTxMonitoringClient = mapLocalTxMonitoringClient toConsensusTxId fromConsensusGenTx

convLocalChainSyncClient ::
     forall block m a. ()
  => Consensus.CardanoBlock L.StandardCrypto ~ block =>
       Functor m =>
         ChainSyncClient BlockInMode ChainPoint ChainTip m a -> ChainSyncClient
                                                                  block
                                                                  (Net.Point block)
                                                                  (Net.Tip block)
                                                                  m
...
convLocalChainSyncClientPipelined ::
  ...
  => Consensus.CardanoBlock L.StandardCrypto ~ block =>
       Functor m =>
         ChainSyncClientPipelined BlockInMode ChainPoint ChainTip m a -> ChainSyncClientPipelined
                                                                           block
                                                                           (Net.Point block)
                                                                           (Net.Tip block)
                                                                           m
...
convLocalTxSubmissionClient ::
  ...
  => Consensus.CardanoBlock L.StandardCrypto ~ block =>
       Functor m =>
         LocalTxSubmissionClient TxInMode TxValidationErrorInCardanoMode m a -> LocalTxSubmissionClient
                                                                                  (Consensus.GenTx
                                                                                     block)
...
convLocalStateQueryClient ::
  ...
  => Consensus.CardanoBlock L.StandardCrypto ~ block =>
       Functor m =>
         LocalStateQueryClient BlockInMode ChainPoint QueryInMode m a -> LocalStateQueryClient
                                                                           block
                                                                           (Consensus.Point block)
                                                                           ...
                                                                           a
convLocalStateQueryClient =
  Net.Query.mapLocalStateQueryClient toConsensusPointHF toConsensusQuery fromConsensusQueryResult

--TODO: Move to consensus
mapLocalTxMonitoringClient ::
     forall txid txid' tx tx' m a. ()
  => Functor m =>
       (txid -> txid') -> (tx' -> tx) -> LocalTxMonitorClient txid tx SlotNo m a -> LocalTxMonitorClient
                                                                                      txid'
                                                                                      tx'
                                                                                      SlotNo
                                                                                      m
...
mapLocalTxMonitoringClient convTxid convTx ltxmc =
  ...
  where
    convClientStateIdle ::
         CTxMon.ClientStIdle txid tx SlotNo m a -> CTxMon.ClientStIdle txid' tx' SlotNo m a
    ...
    convClientStateAcquired ::
         CTxMon.ClientStAcquired txid tx SlotNo m a -> CTxMon.ClientStAcquired txid' tx' SlotNo m a
...
instance ToJSON LocalTxMonitoringResult where
  toJSON result =
    object
      $ case result of
          LocalTxMonitoringTxExists tx slot -> ["exists" .= True, "txId" .= tx, "slot" .= slot]
          LocalTxMonitoringTxDoesNotExist tx slot ->
            ["exists" .= False, "txId" .= tx, "slot" .= slot]
          LocalTxMonitoringNextTx txInMode slot -> ["nextTx" .= txId, "slot" .= slot]
            where txId =
                    case txInMode of
...
queryTxMonitoringLocal ::
     MonadIO m => LocalNodeConnectInfo -> LocalTxMonitoringQuery -> m LocalTxMonitoringResult
queryTxMonitoringLocal connectInfo localTxMonitoringQuery = do
  resultVar <- liftIO newEmptyTMVarIO
  let client =
        case localTxMonitoringQuery of
          LocalTxMonitoringQueryTx txidInMode -> localTxMonitorClientTxExists txidInMode resultVar
          LocalTxMonitoringSendNextTx -> localTxMonitorNextTx resultVar
          LocalTxMonitoringMempoolInformation -> localTxMonitorMempoolInfo resultVar
  connectToLocalNode
    connectInfo
    LocalNodeClientProtocols
      { localChainSyncClient = NoLocalChainSyncClient
  ...
  where
    ...
    localTxMonitorClientTxExists tIdInMode@(TxIdInMode _ txid) resultVar = do
      LocalTxMonitorClient
        $ return
        $ CTxMon.SendMsgAcquire
        $ \slt -> do
            return
              $ CTxMon.SendMsgHasTx tIdInMode
              $ \txPresentBool -> do
                  if txPresentBool
    ...
    localTxMonitorMempoolInfo resultVar =
      LocalTxMonitorClient
        ...
        $ do
            CTxMon.SendMsgAcquire $ \slt -> do
              return
                $ CTxMon.SendMsgGetSizes
                $ \mempoolCapacity -> do
                    atomically
                      $ putTMVar resultVar
                      $ LocalTxMonitoringMempoolSizeAndCapacity mempoolCapacity slt
                    return $ CTxMon.SendMsgRelease $ return $ CTxMon.SendMsgDone ()
...
chainSyncGetCurrentTip ::
     () => TMVar ChainTip -> ChainSyncClient BlockInMode ChainPoint ChainTip IO ()
chainSyncGetCurrentTip tipVar = ChainSyncClient $ pure clientStIdle
  where
    ...
    clientStIdle = Net.Sync.SendMsgRequestNext (pure ()) clientStNext

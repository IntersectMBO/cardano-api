...
instance IOLike m => Protocol m ByronBlockHFC where
  data ProtocolInfoArgs ByronBlockHFC =
    ProtocolInfoArgsByron (ProtocolParams Byron.ByronBlock)

  protocolInfo (ProtocolInfoArgsByron params) =
    ( inject $ protocolInfoByron params
    , pure . map inject $ blockForgingByron params )

instance ( CardanoHardForkConstraints StandardCrypto, IOLike m )
...
instance CardanoHardForkConstraints StandardCrypto
  => ProtocolClient (CardanoBlock StandardCrypto) where
...
instance ( IOLike m
  ...
  => Protocol m (ShelleyBlockHFC
                   (Consensus.TPraos StandardCrypto)
                   StandardShelley) where
  data ProtocolInfoArgs
      (ShelleyBlockHFC (Consensus.TPraos StandardCrypto) StandardShelley) =
    ProtocolInfoArgsShelley (ShelleyGenesis StandardCrypto)
  ...
  protocolInfo
    (ProtocolInfoArgsShelley genesis paramsShelleyBased_ paramsShelley_) =
...
instance Consensus.LedgerSupportsProtocol
  ...
  => ProtocolClient (ShelleyBlockHFC
                       (Consensus.TPraos StandardCrypto)
                       StandardShelley) where
  data ProtocolClientInfoArgs
      (ShelleyBlockHFC (Consensus.TPraos StandardCrypto) StandardShelley) =

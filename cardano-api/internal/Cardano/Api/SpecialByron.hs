...
module Cardano.Api.SpecialByron
  ...
  , applicationName
  , applicationVersion
  , softwareVersion ) where

import           Cardano.Api.HasTypeProxy
import           Cardano.Api.Keys.Byron
import           Cardano.Api.NetworkId
       ( NetworkId, toByronProtocolMagicId )
import           Cardano.Api.SerialiseRaw
import qualified Cardano.Binary                           as Binary
import           Cardano.Chain.Common
       ( LovelacePortion, TxFeePolicy )
import           Cardano.Chain.Slotting
import           Cardano.Chain.Update
       ( AProposal(aBody, annotation), InstallerHash, ProposalBody(ProposalBody)
       , ProtocolParametersUpdate(..), ProtocolVersion, SoftforkRule
       , SoftwareVersion, SystemTag, UpId, mkVote, recoverUpId, recoverVoteId
       , signProposal )
import qualified Cardano.Chain.Update                     as Update
import qualified Cardano.Chain.Update.Vote                as ByronVote
import           Cardano.Crypto
       ( SafeSigner, noPassSafeSigner )
import qualified Cardano.Ledger.Binary                    as Binary
       ( Annotated(..), ByteSpan(..), annotation, annotationBytes, byronProtVer
       , reAnnotate )
  ...
  deserialiseFromRawBytes AsByronUpdateProposal bs =
    ...
    in  case Binary.decodeFull lBs of
          Left e -> Left
            ...
            $ "Unable to deserialise ByronUpdateProposal: " <> show e
...
makeByronUpdateProposal
  ...
  -> ByronProtocolParametersUpdate
  -> ByronUpdateProposal
makeByronUpdateProposal nId pVer sVer sysTag insHash bWit paramsToUpdate =
  let nonAnnotatedProposal :: AProposal ()
      nonAnnotatedProposal =
        signProposal (toByronProtocolMagicId nId) proposalBody noPassSigningKey
      annotatedPropBody :: Binary.Annotated ProposalBody ByteString
      annotatedPropBody =
        Binary.reAnnotate Binary.byronProtVer $ aBody nonAnnotatedProposal
...
makeProtocolParametersUpdate
  :: ByronProtocolParametersUpdate -> ProtocolParametersUpdate
...
instance SerialiseAsRawBytes ByronVote where
  serialiseToRawBytes (ByronVote vote) =
    Binary.serialize' $ fmap (const ()) vote
  ...
  deserialiseFromRawBytes AsByronVote bs =
    ...
    in  case Binary.decodeFull lBs of
          Left e     -> Left
            $ SerialiseAsRawBytesError
            $ "Unable to deserialise ByronVote: " <> show e
          Right vote -> Right . ByronVote $ annotateVote vote lBs
    where
      annotateVote :: ByronVote.AVote Binary.ByteSpan
                   -> LB.ByteString
                   -> ByronVote.AVote ByteString
...
makeByronVote nId sKey (ByronUpdateProposal proposal) yesOrNo =
  let signingKey = toByronSigningKey sKey
      ...
      nonAnnotatedVote =
        mkVote
          (toByronProtocolMagicId nId)
          signingKey
          (recoverUpId proposal)
          yesOrNo
      annotatedProposalId :: Binary.Annotated UpId ByteString
      annotatedProposalId =
        Binary.reAnnotate Binary.byronProtVer
        $ ByronVote.aProposalId nonAnnotatedVote
  in  ByronVote
      $ nonAnnotatedVote
...
toByronLedgertoByronVote (ByronVote vote) =
  Mempool.ByronUpdateVote (recoverVoteId vote) vote

...
instance Key ByronKey where
  ...
  getVerificationKey (ByronSigningKey sk) = ByronVerificationKey (Crypto.toVerification sk)
  verificationKeyHash :: VerificationKey ByronKey -> Hash ByronKey
  verificationKeyHash (ByronVerificationKey vkey) = ByronKeyHash (Crypto.hashKey vkey)
...
instance SerialiseAsRawBytes (VerificationKey ByronKey) where
  serialiseToRawBytes (ByronVerificationKey (Crypto.VerificationKey xvk)) = Crypto.HD.unXPub xvk
  deserialiseFromRawBytes (AsVerificationKey AsByronKey) bs =
    first
      (\msg -> SerialiseAsRawBytesError ("Unable to deserialise VerificationKey ByronKey" ++ msg))
...
data instance Hash ByronKey =
  ByronKeyHash Crypto.KeyHash
  deriving (Eq, Ord)
...
instance Key ByronKeyLegacy where
  ...
  verificationKeyHash (ByronVerificationKeyLegacy vkey) = ByronKeyHashLegacy (Crypto.hashKey vkey)
...
data instance Hash ByronKeyLegacy =
  ByronKeyHashLegacy Crypto.KeyHash
  deriving (Eq, Ord)
...
instance SerialiseAsRawBytes (VerificationKey ByronKeyLegacy) where
  ...
  deserialiseFromRawBytes (AsVerificationKey AsByronKeyLegacy) bs =
    first
      (\msg ->
         SerialiseAsRawBytesError ("Unable to deserialise VerificationKey ByronKeyLegacy" ++ msg))
...
instance SerialiseAsRawBytes (SigningKey ByronKeyLegacy) where
  serialiseToRawBytes (ByronSigningKeyLegacy (Crypto.SigningKey xsk)) = Crypto.HD.unXPrv xsk
  deserialiseFromRawBytes (AsSigningKey AsByronKeyLegacy) bs =
    first
      (\e -> SerialiseAsRawBytesError ("Unable to deserialise SigningKey ByronKeyLegacy" ++ show e))
    ...
    where
      ...
      decodeXPrv =
        CBOR.decodeBytesCanonical >>= either (fail . formatToString build) pure . Wallet.xprv

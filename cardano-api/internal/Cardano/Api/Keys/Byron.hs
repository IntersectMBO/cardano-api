...
toByronSigningKey bWit = case bWit of
...
instance Key ByronKey where
  ...
  newtype SigningKey ByronKey = ByronSigningKey Crypto.SigningKey
      deriving ( Show, IsString ) via UsingRawBytesHex (SigningKey ByronKey)
      deriving newtype ( ToCBOR, FromCBOR )
      deriving anyclass SerialiseAsCBOR

  deterministicSigningKey
    :: AsType ByronKey -> Crypto.Seed -> SigningKey ByronKey
...
instance SerialiseAsRawBytes (VerificationKey ByronKey) where
  ...
  deserialiseFromRawBytes (AsVerificationKey AsByronKey) bs =
    first (\msg -> SerialiseAsRawBytesError
             ("Unable to deserialise VerificationKey ByronKey" ++ msg))
...
instance SerialiseAsRawBytes (SigningKey ByronKey) where
  ...
  deserialiseFromRawBytes (AsSigningKey AsByronKey) bs =
    first (\e -> SerialiseAsRawBytesError
             ("Unable to deserialise SigningKey ByronKey" ++ show e))
    $ ByronSigningKey . snd
    <$> CBOR.deserialiseFromBytes fromCBOR (LB.fromStrict bs)
...
instance SerialiseAsRawBytes (Hash ByronKey) where
  ...
  deserialiseFromRawBytes (AsHash AsByronKey) bs =
    maybeToRight
      (SerialiseAsRawBytesError "Unable to deserialise Hash ByronKey")
...
instance CastVerificationKeyRole ByronKey PaymentExtendedKey where
  castVerificationKey (ByronVerificationKey vk) =
    PaymentExtendedVerificationKey (Crypto.unVerificationKey vk)
...
instance Key ByronKeyLegacy where
  ...
  newtype SigningKey ByronKeyLegacy = ByronSigningKeyLegacy Crypto.SigningKey
      deriving ( Show
  ...
  deterministicSigningKey
    :: AsType ByronKeyLegacy -> Crypto.Seed -> SigningKey ByronKeyLegacy
  deterministicSigningKey _ _ =
    error "Please generate a non legacy Byron key instead"
...
instance SerialiseAsRawBytes (Hash ByronKeyLegacy) where
  ...
  deserialiseFromRawBytes (AsHash AsByronKeyLegacy) bs =
    maybeToRight
      (SerialiseAsRawBytesError "Unable to deserialise Hash ByronKeyLegacy")
...
instance SerialiseAsRawBytes (VerificationKey ByronKeyLegacy) where
  ...
  deserialiseFromRawBytes (AsVerificationKey AsByronKeyLegacy) bs =
    first (\msg -> SerialiseAsRawBytesError
             ("Unable to deserialise VerificationKey ByronKeyLegacy" ++ msg))
...
instance SerialiseAsRawBytes (SigningKey ByronKeyLegacy) where
  ...
  deserialiseFromRawBytes (AsSigningKey AsByronKeyLegacy) bs =
    first (\e -> SerialiseAsRawBytesError
             ("Unable to deserialise SigningKey ByronKeyLegacy" ++ show e))
    $ ByronSigningKeyLegacy . snd
    ...
    where
      ...
      enforceSize lbl requestedSize =
        CBOR.decodeListLenCanonical >>= matchSize requestedSize lbl
      ...
      decodeXPrv =
        CBOR.decodeBytesCanonical
        >>= either (fail . formatToString build) pure . Wallet.xprv
...
instance CastVerificationKeyRole ByronKeyLegacy ByronKey where
  castVerificationKey (ByronVerificationKeyLegacy vk) = ByronVerificationKey vk

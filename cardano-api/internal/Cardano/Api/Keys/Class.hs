...
module Cardano.Api.Keys.Class
  ...
  , CastSigningKeyRole(..)
  , AsType(AsVerificationKey, AsSigningKey) ) where
...
class ( Eq (VerificationKey keyrole)
      ...
      , HasTextEnvelope (SigningKey keyrole) ) => Key keyrole where
  ...
  deterministicSigningKey
    :: AsType keyrole -> Crypto.Seed -> SigningKey keyrole
...
--
generateSigningKey
  :: MonadIO m => Key keyrole => AsType keyrole -> m (SigningKey keyrole)
...
generateInsecureSigningKey g keytype = do
  let ( bs, g' ) =
        Random.genByteString
          (fromIntegral $ deterministicSigningKeySeedSize keytype)
          g
  case deserialiseFromRawBytes (AsSigningKey keytype) bs of
    Right key -> return ( key, g' )
    Left (SerialiseAsRawBytesError msg) -> error
      $ "generateInsecureSigningKey: Unable to generate insecure key: " <> msg

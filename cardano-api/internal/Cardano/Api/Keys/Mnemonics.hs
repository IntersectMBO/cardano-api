...
module Cardano.Api.Keys.Mnemonics
  ...
  , SecondFactor
  , signingStakeKeyFromMnemonic
  )
where

import           Cardano.Api.Keys.Class (Key (..))
import           Cardano.Api.Keys.Shelley (SigningKey (StakeSigningKey), StakeKey)
...
signingStakeKeyFromMnemonic
  ...
  -> Either MnemonicToSigningStakeKeyError (SigningKey StakeKey)
signingStakeKeyFromMnemonic mnemonicWords mSecondFactor accNo payKeyNo = do

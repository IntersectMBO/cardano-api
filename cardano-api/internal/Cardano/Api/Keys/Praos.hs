...
instance Key KesKey where
  ...
  newtype SigningKey KesKey = KesSigningKey (Shelley.SignKeyKES StandardCrypto)
  ...
  deterministicSigningKeySeedSize AsKesKey = Crypto.seedSizeKES proxy
    where
...
instance SerialiseAsRawBytes (VerificationKey KesKey) where
  serialiseToRawBytes (KesVerificationKey vk) = Crypto.rawSerialiseVerKeyKES vk

  deserialiseFromRawBytes (AsVerificationKey AsKesKey) bs =
    maybeToRight
      (SerialiseAsRawBytesError "Unable to deserialise VerificationKey KesKey")
    $ KesVerificationKey <$> Crypto.rawDeserialiseVerKeyKES bs

instance SerialiseAsRawBytes (SigningKey KesKey) where
  serialiseToRawBytes (KesSigningKey sk) = Crypto.rawSerialiseSignKeyKES sk

  deserialiseFromRawBytes (AsSigningKey AsKesKey) bs =
    maybeToRight
      (SerialiseAsRawBytesError "Unable to deserialise SigningKey KesKey")
...
newtype instance Hash KesKey =
  KesKeyHash (Shelley.Hash StandardCrypto (Shelley.VerKeyKES StandardCrypto))
...
instance SerialiseAsRawBytes (Hash KesKey) where
  serialiseToRawBytes (KesKeyHash vkh) = Crypto.hashToBytes vkh
...
instance HasTextEnvelope (VerificationKey KesKey) where
  textEnvelopeType
    _ = "KesVerificationKey_" <> fromString (Crypto.algorithmNameKES proxy)
    where
      proxy :: Proxy (Shelley.KES StandardCrypto)
      proxy = Proxy
...
instance HasTextEnvelope (SigningKey KesKey) where
  textEnvelopeType
    _ = "KesSigningKey_" <> fromString (Crypto.algorithmNameKES proxy)
    where
      proxy :: Proxy (Shelley.KES StandardCrypto)
      proxy = Proxy
...
instance Key VrfKey where
  ...
  newtype SigningKey VrfKey = VrfSigningKey (Shelley.SignKeyVRF StandardCrypto)
  ...
  deterministicSigningKey AsVrfKey seed = VrfSigningKey (Crypto.genKeyVRF seed)

  deterministicSigningKeySeedSize :: AsType VrfKey -> Word
  deterministicSigningKeySeedSize AsVrfKey = Crypto.seedSizeVRF proxy
    where
...
instance SerialiseAsRawBytes (VerificationKey VrfKey) where
  serialiseToRawBytes (VrfVerificationKey vk) = Crypto.rawSerialiseVerKeyVRF vk

  deserialiseFromRawBytes (AsVerificationKey AsVrfKey) bs =
    maybeToRight
      (SerialiseAsRawBytesError "Unable to deserialise VerificationKey VrfKey")
    $ VrfVerificationKey <$> Crypto.rawDeserialiseVerKeyVRF bs

instance SerialiseAsRawBytes (SigningKey VrfKey) where
  serialiseToRawBytes (VrfSigningKey sk) = Crypto.rawSerialiseSignKeyVRF sk

  deserialiseFromRawBytes (AsSigningKey AsVrfKey) bs =
    maybeToRight
      (SerialiseAsRawBytesError "Unable to deserialise SigningKey VrfKey")
...
newtype instance Hash VrfKey =
  VrfKeyHash (Shelley.Hash StandardCrypto (Shelley.VerKeyVRF StandardCrypto))
  deriving stock ( Eq, Ord )
...
instance SerialiseAsRawBytes (Hash VrfKey) where
  serialiseToRawBytes (VrfKeyHash vkh) = Crypto.hashToBytes vkh
...
instance HasTextEnvelope (VerificationKey VrfKey) where
  textEnvelopeType
    _ = "VrfVerificationKey_" <> fromString (Crypto.algorithmNameVRF proxy)
    where
      proxy :: Proxy (Shelley.VRF StandardCrypto)
      proxy = Proxy
...
instance HasTextEnvelope (SigningKey VrfKey) where
  textEnvelopeType
    _ = "VrfSigningKey_" <> fromString (Crypto.algorithmNameVRF proxy)
    where
      proxy :: Proxy (Shelley.VRF StandardCrypto)
      proxy = Proxy

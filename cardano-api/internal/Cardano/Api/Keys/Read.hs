...
readKeyFile ::
     AsType a -> NonEmpty (InputFormat a) -> FilePath -> IO (Either (FileError InputDecodeError) a)
...
readKeyFileTextEnvelope ::
     HasTextEnvelope a => AsType a -> File content In -> IO (Either (FileError InputDecodeError) a)
...
readKeyFileAnyOf bech32Types textEnvTypes path = do
  ...
  case eContent of
    Left e -> return $ Left e
    Right content ->
      return . first (FileError (unFile path))

...
-- envelope format.
readKeyFileAnyOf :: forall content b. [ FromSomeType SerialiseAsBech32 b ]
...
readKeyFileAnyOf bech32Types textEnvTypes path = do
  ...
  case eContent of
    Left e        -> return $ Left e
    Right content -> return . first (FileError (unFile path))

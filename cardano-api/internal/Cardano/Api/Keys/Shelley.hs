...
import           Data.Aeson.Types
       ( ToJSONKey(..), toJSONKeyText, withText )
...
instance Key PaymentKey where
  ...
  deterministicSigningKey
    :: AsType PaymentKey -> Crypto.Seed -> SigningKey PaymentKey
...
instance HasTextEnvelope (SigningKey PaymentKey) where
  textEnvelopeType _ =
    "PaymentSigningKeyShelley_" <> fromString (Crypto.algorithmNameDSIGN proxy)
    where
      proxy :: Proxy (Shelley.DSIGN StandardCrypto)
      proxy = Proxy
...
instance Key PaymentExtendedKey where
  ...
  --
  verificationKeyHash
    :: VerificationKey PaymentExtendedKey -> Hash PaymentExtendedKey
...
instance SerialiseAsRawBytes (Hash PaymentExtendedKey) where
  ...
  deserialiseFromRawBytes (AsHash AsPaymentExtendedKey) bs =
    maybeToRight
      (SerialiseAsRawBytesError "Unable to deserialise Hash PaymentExtendedKey")
...
instance Key StakeKey where
  ...
  deterministicSigningKey
    :: AsType StakeKey -> Crypto.Seed -> SigningKey StakeKey
...
instance SerialiseAsRawBytes (VerificationKey StakeKey) where
  ...
  deserialiseFromRawBytes (AsVerificationKey AsStakeKey) bs =
    maybeToRight
      (SerialiseAsRawBytesError
         "Unable to deserialise VerificationKey StakeKey")
...
instance SerialiseAsRawBytes (SigningKey StakeKey) where
  ...
  deserialiseFromRawBytes (AsSigningKey AsStakeKey) bs =
    maybeToRight
      (SerialiseAsRawBytesError "Unable to deserialise SigningKey StakeKey")
...
instance SerialiseAsRawBytes (Hash StakeKey) where
  ...
  deserialiseFromRawBytes (AsHash AsStakeKey) bs =
    maybeToRight
      (SerialiseAsRawBytesError "Unable to deserialise Hash StakeKey")
...
instance HasTextEnvelope (SigningKey StakeKey) where
  textEnvelopeType _ =
    "StakeSigningKeyShelley_" <> fromString (Crypto.algorithmNameDSIGN proxy)
    where
      proxy :: Proxy (Shelley.DSIGN StandardCrypto)
      proxy = Proxy
...
instance Key StakeExtendedKey where
  ...
  newtype SigningKey StakeExtendedKey = StakeExtendedSigningKey Crypto.HD.XPrv
      deriving anyclass SerialiseAsCBOR
      deriving ( Show
               , IsString ) via UsingRawBytesHex (SigningKey StakeExtendedKey)

  deterministicSigningKey
    :: AsType StakeExtendedKey -> Crypto.Seed -> SigningKey StakeExtendedKey
  ...
  getVerificationKey
    :: SigningKey StakeExtendedKey -> VerificationKey StakeExtendedKey
  ...
  --
  verificationKeyHash
    :: VerificationKey StakeExtendedKey -> Hash StakeExtendedKey
...
instance SerialiseAsRawBytes (SigningKey StakeExtendedKey) where
  ...
  deserialiseFromRawBytes (AsSigningKey AsStakeExtendedKey) bs =
    first (\msg -> SerialiseAsRawBytesError
             ("Unable to deserialise SigningKey StakeExtendedKey: " ++ msg))
...
instance SerialiseAsRawBytes (Hash StakeExtendedKey) where
  ...
  deserialiseFromRawBytes (AsHash AsStakeExtendedKey) bs =
    maybeToRight
      (SerialiseAsRawBytesError "Unable to deserialise Hash StakeExtendedKey")
...
instance Key GenesisKey where
  ...
  deterministicSigningKey
    :: AsType GenesisKey -> Crypto.Seed -> SigningKey GenesisKey
...
instance SerialiseAsRawBytes (VerificationKey GenesisKey) where
  ...
  deserialiseFromRawBytes (AsVerificationKey AsGenesisKey) bs =
    maybeToRight
      (SerialiseAsRawBytesError
         "Unable to deserialise VerificationKey GenesisKey")
...
instance SerialiseAsRawBytes (SigningKey GenesisKey) where
  ...
  deserialiseFromRawBytes (AsSigningKey AsGenesisKey) bs =
    maybeToRight
      (SerialiseAsRawBytesError "Unable to deserialise SigningKey GenesisKey")
...
instance SerialiseAsRawBytes (Hash GenesisKey) where
  ...
  deserialiseFromRawBytes (AsHash AsGenesisKey) bs =
    maybeToRight
      (SerialiseAsRawBytesError "Unable to deserialise Hash GenesisKey")
    $ GenesisKeyHash . Shelley.KeyHash <$> Crypto.hashFromBytes bs

instance HasTextEnvelope (VerificationKey GenesisKey) where
  textEnvelopeType _ =
    "GenesisVerificationKey_" <> fromString (Crypto.algorithmNameDSIGN proxy)
    where
      proxy :: Proxy (Shelley.DSIGN StandardCrypto)
      proxy = Proxy
...
instance HasTextEnvelope (SigningKey GenesisKey) where
  textEnvelopeType
    _ = "GenesisSigningKey_" <> fromString (Crypto.algorithmNameDSIGN proxy)
    where
      proxy :: Proxy (Shelley.DSIGN StandardCrypto)
      proxy = Proxy
...
instance Key CommitteeHotKey where
  ...
  deterministicSigningKey
    :: AsType CommitteeHotKey -> Crypto.Seed -> SigningKey CommitteeHotKey
  ...
  deterministicSigningKeySeedSize
    AsCommitteeHotKey = Crypto.seedSizeDSIGN proxy
    where
      proxy :: Proxy (Shelley.DSIGN StandardCrypto)
      proxy = Proxy

  getVerificationKey
    :: SigningKey CommitteeHotKey -> VerificationKey CommitteeHotKey
  getVerificationKey (CommitteeHotSigningKey sk) =
    CommitteeHotVerificationKey (Shelley.VKey (Crypto.deriveVerKeyDSIGN sk))

  verificationKeyHash
    :: VerificationKey CommitteeHotKey -> Hash CommitteeHotKey
...
newtype instance Hash CommitteeHotKey =
  ...
  { unCommitteeHotKeyHash
      :: Shelley.KeyHash Shelley.HotCommitteeRole StandardCrypto
  }
  deriving stock ( Eq, Ord )
...
instance Key CommitteeColdKey where
  ...
  deterministicSigningKey
    :: AsType CommitteeColdKey -> Crypto.Seed -> SigningKey CommitteeColdKey
  ...
  deterministicSigningKeySeedSize
    AsCommitteeColdKey = Crypto.seedSizeDSIGN proxy
    where
      proxy :: Proxy (Shelley.DSIGN StandardCrypto)
      proxy = Proxy

  getVerificationKey
    :: SigningKey CommitteeColdKey -> VerificationKey CommitteeColdKey
  getVerificationKey (CommitteeColdSigningKey sk) =
    CommitteeColdVerificationKey (Shelley.VKey (Crypto.deriveVerKeyDSIGN sk))

  verificationKeyHash
    :: VerificationKey CommitteeColdKey -> Hash CommitteeColdKey
...
newtype instance Hash CommitteeColdKey =
  ...
  { unCommitteeColdKeyHash
      :: Shelley.KeyHash Shelley.ColdCommitteeRole StandardCrypto
  }
  deriving stock ( Eq, Ord )
...
newtype instance Hash CommitteeColdExtendedKey =
  ...
  { unCommitteeColdExtendedKeyHash
      :: Shelley.KeyHash Shelley.ColdCommitteeRole StandardCrypto
  }
  deriving stock ( Eq, Ord )
  deriving ( Show, IsString ) via UsingRawBytesHex (Hash CommitteeColdKey)
...
instance SerialiseAsRawBytes (Hash CommitteeColdExtendedKey) where
  ...
  deserialiseFromRawBytes (AsHash AsCommitteeColdExtendedKey) bs =
    ...
    $ CommitteeColdExtendedKeyHash . Shelley.KeyHash
    <$> Crypto.hashFromBytes bs
...
instance Key CommitteeHotExtendedKey where
  ...
  --
  verificationKeyHash
    :: VerificationKey CommitteeHotExtendedKey -> Hash CommitteeHotExtendedKey
...
instance SerialiseAsRawBytes (Hash CommitteeHotExtendedKey) where
  ...
  deserialiseFromRawBytes (AsHash AsCommitteeHotExtendedKey) bs =
    maybeToRight
      (SerialiseAsRawBytesError
         "Unable to deserialise Hash CommitteeHotExtendedKey")
...
instance Key GenesisExtendedKey where
  ...
  --
  verificationKeyHash
    :: VerificationKey GenesisExtendedKey -> Hash GenesisExtendedKey
...
instance SerialiseAsRawBytes (VerificationKey GenesisExtendedKey) where
  ...
  deserialiseFromRawBytes (AsVerificationKey AsGenesisExtendedKey) bs =
    first
      (const
         (SerialiseAsRawBytesError
            "Unable to deserialise VerificationKey GenesisExtendedKey"))
...
instance SerialiseAsRawBytes (SigningKey GenesisExtendedKey) where
  ...
  deserialiseFromRawBytes (AsSigningKey AsGenesisExtendedKey) bs =
    first (\msg -> SerialiseAsRawBytesError
             ("Unable to deserialise SigningKey GenesisExtendedKey" ++ msg))
...
instance SerialiseAsRawBytes (Hash GenesisExtendedKey) where
  ...
  deserialiseFromRawBytes (AsHash AsGenesisExtendedKey) bs =
    maybeToRight
      (SerialiseAsRawBytesError "Unable to deserialise Hash GenesisExtendedKey")
...
instance Key GenesisDelegateKey where
  ...
  deterministicSigningKeySeedSize
    AsGenesisDelegateKey = Crypto.seedSizeDSIGN proxy
    where
      proxy :: Proxy (Shelley.DSIGN StandardCrypto)
      proxy = Proxy

  getVerificationKey
    :: SigningKey GenesisDelegateKey -> VerificationKey GenesisDelegateKey
  getVerificationKey (GenesisDelegateSigningKey sk) =
    GenesisDelegateVerificationKey (Shelley.VKey (Crypto.deriveVerKeyDSIGN sk))

  verificationKeyHash
    :: VerificationKey GenesisDelegateKey -> Hash GenesisDelegateKey
...
instance SerialiseAsRawBytes (VerificationKey GenesisDelegateKey) where
  ...
  deserialiseFromRawBytes (AsVerificationKey AsGenesisDelegateKey) bs =
    maybeToRight
      (SerialiseAsRawBytesError
         "Unable to deserialise VerificationKey GenesisDelegateKey")
    $ GenesisDelegateVerificationKey . Shelley.VKey
    <$> Crypto.rawDeserialiseVerKeyDSIGN bs
...
instance SerialiseAsRawBytes (SigningKey GenesisDelegateKey) where
  ...
  deserialiseFromRawBytes (AsSigningKey AsGenesisDelegateKey) bs =
    maybeToRight
      (SerialiseAsRawBytesError
         "Unable to deserialise SigningKey GenesisDelegateKey")
...
newtype instance Hash GenesisDelegateKey =
  ...
  { unGenesisDelegateKeyHash
      :: Shelley.KeyHash Shelley.GenesisDelegate StandardCrypto
  }
  deriving stock ( Eq, Ord )
...
instance SerialiseAsRawBytes (Hash GenesisDelegateKey) where
  ...
  deserialiseFromRawBytes (AsHash AsGenesisDelegateKey) bs =
    maybeToRight
      (SerialiseAsRawBytesError "Unable to deserialise Hash GenesisDelegateKey")
...
instance SerialiseAsRawBytes (VerificationKey GenesisDelegateExtendedKey) where
  ...
  deserialiseFromRawBytes (AsVerificationKey AsGenesisDelegateExtendedKey) bs =
    first
      (\msg -> SerialiseAsRawBytesError
         ("Unable to deserialise VerificationKey GenesisDelegateExtendedKey: "
          ++ msg))
...
instance SerialiseAsRawBytes (SigningKey GenesisDelegateExtendedKey) where
  ...
  deserialiseFromRawBytes (AsSigningKey AsGenesisDelegateExtendedKey) bs =
    first (\msg -> SerialiseAsRawBytesError
             ("Unable to deserialise SigningKey GenesisDelegateExtendedKey: "
              ++ msg))
...
newtype instance Hash GenesisDelegateExtendedKey =
  ...
  { unGenesisDelegateExtendedKeyHash
...
instance SerialiseAsRawBytes (Hash GenesisDelegateExtendedKey) where
  ...
  deserialiseFromRawBytes (AsHash AsGenesisDelegateExtendedKey) bs =
    ...
    $ GenesisDelegateExtendedKeyHash . Shelley.KeyHash
    <$> Crypto.hashFromBytes bs
...
instance Key GenesisUTxOKey where
  ...
  deterministicSigningKey
    :: AsType GenesisUTxOKey -> Crypto.Seed -> SigningKey GenesisUTxOKey
  ...
  getVerificationKey
    :: SigningKey GenesisUTxOKey -> VerificationKey GenesisUTxOKey
...
instance SerialiseAsRawBytes (VerificationKey GenesisUTxOKey) where
  ...
  deserialiseFromRawBytes (AsVerificationKey AsGenesisUTxOKey) bs =
    maybeToRight
      (SerialiseAsRawBytesError
         "Unable to deserialise VerificationKey GenesisUTxOKey")
...
instance SerialiseAsRawBytes (Hash GenesisUTxOKey) where
  ...
  deserialiseFromRawBytes (AsHash AsGenesisUTxOKey) bs =
    maybeToRight
      (SerialiseAsRawBytesError "Unable to deserialise Hash GenesisUTxOKey")
...
instance HasTextEnvelope (SigningKey GenesisUTxOKey) where
  textEnvelopeType _ =
    "GenesisUTxOSigningKey_" <> fromString (Crypto.algorithmNameDSIGN proxy)
    where
      proxy :: Proxy (Shelley.DSIGN StandardCrypto)
      proxy = Proxy
...
instance Key StakePoolKey where
  ...
  deterministicSigningKey
    :: AsType StakePoolKey -> Crypto.Seed -> SigningKey StakePoolKey
...
instance SerialiseAsRawBytes (VerificationKey StakePoolKey) where
  ...
  deserialiseFromRawBytes (AsVerificationKey AsStakePoolKey) bs =
    maybeToRight
      (SerialiseAsRawBytesError
         "Unable to deserialise VerificationKey StakePoolKey")
...
instance HasTextEnvelope (VerificationKey StakePoolKey) where
  textEnvelopeType _ =
    "StakePoolVerificationKey_" <> fromString (Crypto.algorithmNameDSIGN proxy)
    where
      proxy :: Proxy (Shelley.DSIGN StandardCrypto)
      proxy = Proxy
...
instance HasTextEnvelope (SigningKey StakePoolKey) where
  textEnvelopeType
    _ = "StakePoolSigningKey_" <> fromString (Crypto.algorithmNameDSIGN proxy)
    where
      proxy :: Proxy (Shelley.DSIGN StandardCrypto)
      proxy = Proxy
...
instance Key DRepKey where
  ...
  deterministicSigningKey
    :: AsType DRepKey -> Crypto.Seed -> SigningKey DRepKey
...
instance SerialiseAsRawBytes (VerificationKey DRepKey) where
  ...
  deserialiseFromRawBytes (AsVerificationKey AsDRepKey) bs =
    maybeToRight
      (SerialiseAsRawBytesError "Unable to deserialise VerificationKey DRepKey")
...
instance SerialiseAsRawBytes (SigningKey DRepKey) where
  ...
  deserialiseFromRawBytes (AsSigningKey AsDRepKey) bs =
    maybe
      (Left
         (SerialiseAsRawBytesError "Unable to deserialise SigningKey DRepKey"))
...
instance HasTextEnvelope (VerificationKey DRepKey) where
  textEnvelopeType
    _ = "DRepVerificationKey_" <> fromString (Crypto.algorithmNameDSIGN proxy)
    where
      proxy :: Proxy (Shelley.DSIGN StandardCrypto)
      proxy = Proxy
...
instance HasTextEnvelope (SigningKey DRepKey) where
  textEnvelopeType
    _ = "DRepSigningKey_" <> fromString (Crypto.algorithmNameDSIGN proxy)
    where
      proxy :: Proxy (Shelley.DSIGN StandardCrypto)
      proxy = Proxy
...
instance Key DRepExtendedKey where
  ...
  newtype SigningKey DRepExtendedKey = DRepExtendedSigningKey Crypto.HD.XPrv
      deriving anyclass SerialiseAsCBOR
      deriving ( Show
               , IsString ) via UsingRawBytesHex (SigningKey PaymentExtendedKey)

  deterministicSigningKey
    :: AsType DRepExtendedKey -> Crypto.Seed -> SigningKey DRepExtendedKey
  ...
  getVerificationKey
    :: SigningKey DRepExtendedKey -> VerificationKey DRepExtendedKey
  ...
  --
  verificationKeyHash
    :: VerificationKey DRepExtendedKey -> Hash DRepExtendedKey
...
instance SerialiseAsRawBytes (Hash DRepExtendedKey) where
  ...
  deserialiseFromRawBytes (AsHash AsDRepExtendedKey) bs =
    maybeToRight
      (SerialiseAsRawBytesError "Unable to deserialise Hash DRepExtendedKey")

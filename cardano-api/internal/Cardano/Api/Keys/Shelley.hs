...
instance Key PaymentKey where
  ...
  deterministicSigningKey AsPaymentKey seed = PaymentSigningKey (Crypto.genKeyDSIGN seed)
  deterministicSigningKeySeedSize :: AsType PaymentKey -> Word
  deterministicSigningKeySeedSize AsPaymentKey = Crypto.seedSizeDSIGN proxy
    where
  ...
  verificationKeyHash (PaymentVerificationKey vkey) = PaymentKeyHash (Shelley.hashKey vkey)

instance SerialiseAsRawBytes (VerificationKey PaymentKey) where
  serialiseToRawBytes (PaymentVerificationKey (Shelley.VKey vk)) = Crypto.rawSerialiseVerKeyDSIGN vk
...
instance SerialiseAsRawBytes (SigningKey PaymentKey) where
  serialiseToRawBytes (PaymentSigningKey sk) = Crypto.rawSerialiseSignKeyDSIGN sk
...
data instance Hash PaymentKey = PaymentKeyHash
  { unPaymentKeyHash :: Shelley.KeyHash Shelley.Payment StandardCrypto
...
instance SerialiseAsRawBytes (Hash PaymentKey) where
  serialiseToRawBytes (PaymentKeyHash (Shelley.KeyHash vkh)) = Crypto.hashToBytes vkh
...
instance HasTextEnvelope (VerificationKey PaymentKey) where
  textEnvelopeType _ =
    "PaymentVerificationKeyShelley_" <> fromString (Crypto.algorithmNameDSIGN proxy)
    where
      proxy :: Proxy (Shelley.DSIGN StandardCrypto)
      proxy = Proxy

instance HasTextEnvelope (SigningKey PaymentKey) where
  textEnvelopeType _ = "PaymentSigningKeyShelley_" <> fromString (Crypto.algorithmNameDSIGN proxy)
    where
      proxy :: Proxy (Shelley.DSIGN StandardCrypto)
      proxy = Proxy
...
instance Key PaymentExtendedKey where
  ...
  deterministicSigningKey ::
       AsType PaymentExtendedKey -> Crypto.Seed -> SigningKey PaymentExtendedKey
  deterministicSigningKey AsPaymentExtendedKey seed =
    PaymentExtendedSigningKey (Crypto.HD.generate seedbs BS.empty)
    where
  ...
  verificationKeyHash (PaymentExtendedVerificationKey vk) =
    PaymentExtendedKeyHash . Shelley.KeyHash . Crypto.castHash
      $ Crypto.hashWith Crypto.HD.xpubPublicKey vk

instance ToCBOR (VerificationKey PaymentExtendedKey) where
  toCBOR (PaymentExtendedVerificationKey xpub) = toCBOR (Crypto.HD.unXPub xpub)

instance FromCBOR (VerificationKey PaymentExtendedKey) where
  fromCBOR = do
    bs <- fromCBOR
    either fail (return . PaymentExtendedVerificationKey) (Crypto.HD.xpub (bs :: ByteString))

instance ToCBOR (SigningKey PaymentExtendedKey) where
  toCBOR (PaymentExtendedSigningKey xprv) = toCBOR (Crypto.HD.unXPrv xprv)

instance FromCBOR (SigningKey PaymentExtendedKey) where
  fromCBOR = do
    bs <- fromCBOR
    either fail (return . PaymentExtendedSigningKey) (Crypto.HD.xprv (bs :: ByteString))

instance SerialiseAsRawBytes (VerificationKey PaymentExtendedKey) where
  serialiseToRawBytes (PaymentExtendedVerificationKey xpub) = Crypto.HD.unXPub xpub
...
instance SerialiseAsRawBytes (SigningKey PaymentExtendedKey) where
  serialiseToRawBytes (PaymentExtendedSigningKey xprv) = Crypto.HD.unXPrv xprv
...
data instance Hash PaymentExtendedKey = PaymentExtendedKeyHash
  { unPaymentExtendedKeyHash :: Shelley.KeyHash Shelley.Payment StandardCrypto
  } deriving stock (Eq, Ord)
...
instance SerialiseAsRawBytes (Hash PaymentExtendedKey) where
  serialiseToRawBytes (PaymentExtendedKeyHash (Shelley.KeyHash vkh)) = Crypto.hashToBytes vkh
...
instance CastVerificationKeyRole PaymentExtendedKey PaymentKey where
  castVerificationKey (PaymentExtendedVerificationKey vk) =
    PaymentVerificationKey
      ...
      $ vk
    where
      impossible = error "castVerificationKey: byron and shelley key sizes do not match!"
...
instance Key StakeKey where
  data VerificationKey StakeKey = StakeVerificationKey
  ...
  deterministicSigningKey AsStakeKey seed = StakeSigningKey (Crypto.genKeyDSIGN seed)
  deterministicSigningKeySeedSize :: AsType StakeKey -> Word
  deterministicSigningKeySeedSize AsStakeKey = Crypto.seedSizeDSIGN proxy
    where
      proxy :: Proxy (Shelley.DSIGN StandardCrypto)
      proxy = Proxy
  ...
  verificationKeyHash (StakeVerificationKey vkey) = StakeKeyHash (Shelley.hashKey vkey)

instance SerialiseAsRawBytes (VerificationKey StakeKey) where
  serialiseToRawBytes (StakeVerificationKey (Shelley.VKey vk)) = Crypto.rawSerialiseVerKeyDSIGN vk
  deserialiseFromRawBytes (AsVerificationKey AsStakeKey) bs =
    maybeToRight (SerialiseAsRawBytesError "Unable to deserialise VerificationKey StakeKey")
      $ StakeVerificationKey . Shelley.VKey <$> Crypto.rawDeserialiseVerKeyDSIGN bs

instance SerialiseAsRawBytes (SigningKey StakeKey) where
  serialiseToRawBytes (StakeSigningKey sk) = Crypto.rawSerialiseSignKeyDSIGN sk
...
data instance Hash StakeKey = StakeKeyHash
...
instance SerialiseAsRawBytes (Hash StakeKey) where
  serialiseToRawBytes (StakeKeyHash (Shelley.KeyHash vkh)) = Crypto.hashToBytes vkh
...
instance HasTextEnvelope (VerificationKey StakeKey) where
  textEnvelopeType _ =
    "StakeVerificationKeyShelley_" <> fromString (Crypto.algorithmNameDSIGN proxy)
    where
      proxy :: Proxy (Shelley.DSIGN StandardCrypto)
      proxy = Proxy

instance HasTextEnvelope (SigningKey StakeKey) where
  textEnvelopeType _ = "StakeSigningKeyShelley_" <> fromString (Crypto.algorithmNameDSIGN proxy)
    where
      proxy :: Proxy (Shelley.DSIGN StandardCrypto)
      proxy = Proxy
...
instance Key StakeExtendedKey where
  ...
  deterministicSigningKey :: AsType StakeExtendedKey -> Crypto.Seed -> SigningKey StakeExtendedKey
  deterministicSigningKey AsStakeExtendedKey seed =
    StakeExtendedSigningKey (Crypto.HD.generate seedbs BS.empty)
    where
      (seedbs, _) = Crypto.getBytesFromSeedT 32 seed
  ...
  verificationKeyHash (StakeExtendedVerificationKey vk) =
    StakeExtendedKeyHash . Shelley.KeyHash . Crypto.castHash
      $ Crypto.hashWith Crypto.HD.xpubPublicKey vk

instance ToCBOR (VerificationKey StakeExtendedKey) where
  toCBOR (StakeExtendedVerificationKey xpub) = toCBOR (Crypto.HD.unXPub xpub)

instance FromCBOR (VerificationKey StakeExtendedKey) where
  fromCBOR = do
    bs <- fromCBOR
    either fail (return . StakeExtendedVerificationKey) (Crypto.HD.xpub (bs :: ByteString))

instance ToCBOR (SigningKey StakeExtendedKey) where
  toCBOR (StakeExtendedSigningKey xprv) = toCBOR (Crypto.HD.unXPrv xprv)

instance FromCBOR (SigningKey StakeExtendedKey) where
  fromCBOR = do
    bs <- fromCBOR
    either fail (return . StakeExtendedSigningKey) (Crypto.HD.xprv (bs :: ByteString))

instance SerialiseAsRawBytes (VerificationKey StakeExtendedKey) where
  serialiseToRawBytes (StakeExtendedVerificationKey xpub) = Crypto.HD.unXPub xpub
  deserialiseFromRawBytes (AsVerificationKey AsStakeExtendedKey) bs =
    first
      (\msg ->
         SerialiseAsRawBytesError
           ("Unable to deserialise VerificationKey StakeExtendedKey: " ++ msg))
      $ StakeExtendedVerificationKey <$> Crypto.HD.xpub bs

instance SerialiseAsRawBytes (SigningKey StakeExtendedKey) where
  serialiseToRawBytes (StakeExtendedSigningKey xprv) = Crypto.HD.unXPrv xprv
  deserialiseFromRawBytes (AsSigningKey AsStakeExtendedKey) bs =
    first
      (\msg ->
         SerialiseAsRawBytesError ("Unable to deserialise SigningKey StakeExtendedKey: " ++ msg))
...
data instance Hash StakeExtendedKey = StakeExtendedKeyHash
  { unStakeExtendedKeyHash :: Shelley.KeyHash Shelley.Staking StandardCrypto
  } deriving stock (Eq, Ord)
...
instance SerialiseAsRawBytes (Hash StakeExtendedKey) where
  serialiseToRawBytes (StakeExtendedKeyHash (Shelley.KeyHash vkh)) = Crypto.hashToBytes vkh
...
instance Key GenesisKey where
  ...
  deterministicSigningKey AsGenesisKey seed = GenesisSigningKey (Crypto.genKeyDSIGN seed)
  deterministicSigningKeySeedSize :: AsType GenesisKey -> Word
  deterministicSigningKeySeedSize AsGenesisKey = Crypto.seedSizeDSIGN proxy
    where
      proxy :: Proxy (Shelley.DSIGN StandardCrypto)
      proxy = Proxy
  ...
  verificationKeyHash (GenesisVerificationKey vkey) = GenesisKeyHash (Shelley.hashKey vkey)

instance SerialiseAsRawBytes (VerificationKey GenesisKey) where
  serialiseToRawBytes (GenesisVerificationKey (Shelley.VKey vk)) = Crypto.rawSerialiseVerKeyDSIGN vk
  deserialiseFromRawBytes (AsVerificationKey AsGenesisKey) bs =
    maybeToRight (SerialiseAsRawBytesError "Unable to deserialise VerificationKey GenesisKey")
      $ GenesisVerificationKey . Shelley.VKey <$> Crypto.rawDeserialiseVerKeyDSIGN bs

instance SerialiseAsRawBytes (SigningKey GenesisKey) where
  serialiseToRawBytes (GenesisSigningKey sk) = Crypto.rawSerialiseSignKeyDSIGN sk
...
data instance Hash GenesisKey = GenesisKeyHash
  { unGenesisKeyHash :: Shelley.KeyHash Shelley.Genesis StandardCrypto
  } deriving stock (Eq, Ord)
...
instance SerialiseAsRawBytes (Hash GenesisKey) where
  serialiseToRawBytes (GenesisKeyHash (Shelley.KeyHash vkh)) = Crypto.hashToBytes vkh
...
instance HasTextEnvelope (VerificationKey GenesisKey) where
  textEnvelopeType _ = "GenesisVerificationKey_" <> fromString (Crypto.algorithmNameDSIGN proxy)
    where
      proxy :: Proxy (Shelley.DSIGN StandardCrypto)
      proxy = Proxy

instance HasTextEnvelope (SigningKey GenesisKey) where
  textEnvelopeType _ = "GenesisSigningKey_" <> fromString (Crypto.algorithmNameDSIGN proxy)
    where
      proxy :: Proxy (Shelley.DSIGN StandardCrypto)
      proxy = Proxy
...
instance Key CommitteeHotKey where
  ...
  deterministicSigningKey AsCommitteeHotKey seed = CommitteeHotSigningKey (Crypto.genKeyDSIGN seed)
  deterministicSigningKeySeedSize :: AsType CommitteeHotKey -> Word
  deterministicSigningKeySeedSize AsCommitteeHotKey = Crypto.seedSizeDSIGN proxy
    where
      proxy :: Proxy (Shelley.DSIGN StandardCrypto)
      proxy = Proxy
...
instance SerialiseAsRawBytes (VerificationKey CommitteeHotKey) where
  ...
  deserialiseFromRawBytes (AsVerificationKey AsCommitteeHotKey) bs =
    maybeToRight
      ...
      $ CommitteeHotVerificationKey . Shelley.VKey <$> Crypto.rawDeserialiseVerKeyDSIGN bs

instance SerialiseAsRawBytes (SigningKey CommitteeHotKey) where
  serialiseToRawBytes (CommitteeHotSigningKey sk) = Crypto.rawSerialiseSignKeyDSIGN sk
...
data instance Hash CommitteeHotKey = CommitteeHotKeyHash
  { unCommitteeHotKeyHash :: Shelley.KeyHash Shelley.HotCommitteeRole StandardCrypto
  } deriving stock (Eq, Ord)
...
instance SerialiseAsRawBytes (Hash CommitteeHotKey) where
  serialiseToRawBytes (CommitteeHotKeyHash (Shelley.KeyHash vkh)) = Crypto.hashToBytes vkh
...
instance HasTextEnvelope (VerificationKey CommitteeHotKey) where
  textEnvelopeType _ =
    "ConstitutionalCommitteeHotVerificationKey_" <> fromString (Crypto.algorithmNameDSIGN proxy)
    where
      proxy :: Proxy (Shelley.DSIGN StandardCrypto)
      proxy = Proxy
...
instance HasTextEnvelope (SigningKey CommitteeHotKey) where
  textEnvelopeType _ =
    "ConstitutionalCommitteeHotSigningKey_" <> fromString (Crypto.algorithmNameDSIGN proxy)
    where
      proxy :: Proxy (Shelley.DSIGN StandardCrypto)
      proxy = Proxy
...
instance Key CommitteeColdKey where
  ...
  deterministicSigningKeySeedSize AsCommitteeColdKey = Crypto.seedSizeDSIGN proxy
    where
      proxy :: Proxy (Shelley.DSIGN StandardCrypto)
      proxy = Proxy
...
instance SerialiseAsRawBytes (VerificationKey CommitteeColdKey) where
  ...
  deserialiseFromRawBytes (AsVerificationKey AsCommitteeColdKey) bs =
    maybeToRight
      ...
      $ CommitteeColdVerificationKey . Shelley.VKey <$> Crypto.rawDeserialiseVerKeyDSIGN bs

instance SerialiseAsRawBytes (SigningKey CommitteeColdKey) where
  serialiseToRawBytes (CommitteeColdSigningKey sk) = Crypto.rawSerialiseSignKeyDSIGN sk
...
data instance Hash CommitteeColdKey = CommitteeColdKeyHash
  { unCommitteeColdKeyHash :: Shelley.KeyHash Shelley.ColdCommitteeRole StandardCrypto
  } deriving stock (Eq, Ord)
...
instance SerialiseAsRawBytes (Hash CommitteeColdKey) where
  serialiseToRawBytes (CommitteeColdKeyHash (Shelley.KeyHash vkh)) = Crypto.hashToBytes vkh
...
instance HasTextEnvelope (VerificationKey CommitteeColdKey) where
  textEnvelopeType _ =
    "ConstitutionalCommitteeColdVerificationKey_" <> fromString (Crypto.algorithmNameDSIGN proxy)
    where
      proxy :: Proxy (Shelley.DSIGN StandardCrypto)
      proxy = Proxy
...
instance HasTextEnvelope (SigningKey CommitteeColdKey) where
  textEnvelopeType _ =
    "ConstitutionalCommitteeColdSigningKey_" <> fromString (Crypto.algorithmNameDSIGN proxy)
    where
      proxy :: Proxy (Shelley.DSIGN StandardCrypto)
      proxy = Proxy
...
instance Key CommitteeColdExtendedKey where
  ...
  data SigningKey CommitteeColdExtendedKey =
    ...
    deriving (Show, IsString) via UsingRawBytesHex (SigningKey PaymentExtendedKey)
  deterministicSigningKey ::
       AsType CommitteeColdExtendedKey -> Crypto.Seed -> SigningKey CommitteeColdExtendedKey
  deterministicSigningKey AsCommitteeColdExtendedKey seed =
    CommitteeColdExtendedSigningKey (Crypto.HD.generate seedbs BS.empty)
    where
      (seedbs, _) = Crypto.getBytesFromSeedT 32 seed
  ...
  getVerificationKey ::
       SigningKey CommitteeColdExtendedKey -> VerificationKey CommitteeColdExtendedKey
  ...
  verificationKeyHash (CommitteeColdExtendedVerificationKey vk) =
    CommitteeColdExtendedKeyHash . Shelley.KeyHash . Crypto.castHash
      $ Crypto.hashWith Crypto.HD.xpubPublicKey vk

data instance Hash CommitteeColdExtendedKey = CommitteeColdExtendedKeyHash
  { unCommitteeColdExtendedKeyHash :: Shelley.KeyHash Shelley.ColdCommitteeRole StandardCrypto
  } deriving stock (Eq, Ord)
    deriving (Show, IsString) via UsingRawBytesHex (Hash CommitteeColdKey)
    deriving (ToCBOR, FromCBOR) via UsingRawBytes (Hash CommitteeColdKey)
    deriving anyclass (SerialiseAsCBOR)
...
instance ToCBOR (VerificationKey CommitteeColdExtendedKey) where
  toCBOR (CommitteeColdExtendedVerificationKey xpub) = toCBOR (Crypto.HD.unXPub xpub)

instance FromCBOR (VerificationKey CommitteeColdExtendedKey) where
  fromCBOR = do
    bs <- fromCBOR
    either fail (return . CommitteeColdExtendedVerificationKey) (Crypto.HD.xpub (bs :: ByteString))

instance ToCBOR (SigningKey CommitteeColdExtendedKey) where
  toCBOR (CommitteeColdExtendedSigningKey xprv) = toCBOR (Crypto.HD.unXPrv xprv)

instance FromCBOR (SigningKey CommitteeColdExtendedKey) where
  fromCBOR = do
    bs <- fromCBOR
    either fail (return . CommitteeColdExtendedSigningKey) (Crypto.HD.xprv (bs :: ByteString))

instance SerialiseAsRawBytes (VerificationKey CommitteeColdExtendedKey) where
  serialiseToRawBytes (CommitteeColdExtendedVerificationKey xpub) = Crypto.HD.unXPub xpub
  deserialiseFromRawBytes (AsVerificationKey AsCommitteeColdExtendedKey) bs =
    first
      (const
         (SerialiseAsRawBytesError "Unable to deserialise VerificationKey CommitteeColdExtendedKey"))
...
instance SerialiseAsRawBytes (SigningKey CommitteeColdExtendedKey) where
  serialiseToRawBytes (CommitteeColdExtendedSigningKey xprv) = Crypto.HD.unXPrv xprv
...
instance SerialiseAsRawBytes (Hash CommitteeColdExtendedKey) where
  serialiseToRawBytes (CommitteeColdExtendedKeyHash (Shelley.KeyHash vkh)) = Crypto.hashToBytes vkh
...
instance Key CommitteeHotExtendedKey where
  ...
  data SigningKey CommitteeHotExtendedKey =
    ...
    deriving (Show, IsString) via UsingRawBytesHex (SigningKey PaymentExtendedKey)
  deterministicSigningKey ::
       AsType CommitteeHotExtendedKey -> Crypto.Seed -> SigningKey CommitteeHotExtendedKey
  deterministicSigningKey AsCommitteeHotExtendedKey seed =
    CommitteeHotExtendedSigningKey (Crypto.HD.generate seedbs BS.empty)
    where
      (seedbs, _) = Crypto.getBytesFromSeedT 32 seed
  ...
  getVerificationKey ::
       SigningKey CommitteeHotExtendedKey -> VerificationKey CommitteeHotExtendedKey
  ...
  verificationKeyHash (CommitteeHotExtendedVerificationKey vk) =
    CommitteeHotExtendedKeyHash . Shelley.KeyHash . Crypto.castHash
      $ Crypto.hashWith Crypto.HD.xpubPublicKey vk

data instance Hash CommitteeHotExtendedKey = CommitteeHotExtendedKeyHash
  { unCommitteeHotExtendedKeyHash :: Shelley.KeyHash Shelley.HotCommitteeRole StandardCrypto
  } deriving stock (Eq, Ord)
    deriving (Show, IsString) via UsingRawBytesHex (Hash CommitteeHotKey)
    deriving (ToCBOR, FromCBOR) via UsingRawBytes (Hash CommitteeHotKey)
    deriving anyclass (SerialiseAsCBOR)
...
instance ToCBOR (VerificationKey CommitteeHotExtendedKey) where
  toCBOR (CommitteeHotExtendedVerificationKey xpub) = toCBOR (Crypto.HD.unXPub xpub)

instance FromCBOR (VerificationKey CommitteeHotExtendedKey) where
  fromCBOR = do
    bs <- fromCBOR
    either fail (return . CommitteeHotExtendedVerificationKey) (Crypto.HD.xpub (bs :: ByteString))

instance ToCBOR (SigningKey CommitteeHotExtendedKey) where
  toCBOR (CommitteeHotExtendedSigningKey xprv) = toCBOR (Crypto.HD.unXPrv xprv)

instance FromCBOR (SigningKey CommitteeHotExtendedKey) where
  fromCBOR = do
    bs <- fromCBOR
    either fail (return . CommitteeHotExtendedSigningKey) (Crypto.HD.xprv (bs :: ByteString))

instance SerialiseAsRawBytes (VerificationKey CommitteeHotExtendedKey) where
  serialiseToRawBytes (CommitteeHotExtendedVerificationKey xpub) = Crypto.HD.unXPub xpub
  deserialiseFromRawBytes (AsVerificationKey AsCommitteeHotExtendedKey) bs =
    first
      (const
         (SerialiseAsRawBytesError "Unable to deserialise VerificationKey CommitteeHotExtendedKey"))
...
instance SerialiseAsRawBytes (SigningKey CommitteeHotExtendedKey) where
  serialiseToRawBytes (CommitteeHotExtendedSigningKey xprv) = Crypto.HD.unXPrv xprv
...
instance SerialiseAsRawBytes (Hash CommitteeHotExtendedKey) where
  serialiseToRawBytes (CommitteeHotExtendedKeyHash (Shelley.KeyHash vkh)) = Crypto.hashToBytes vkh
...
instance Key GenesisExtendedKey where
  ...
  deterministicSigningKey ::
       AsType GenesisExtendedKey -> Crypto.Seed -> SigningKey GenesisExtendedKey
  deterministicSigningKey AsGenesisExtendedKey seed =
    GenesisExtendedSigningKey (Crypto.HD.generate seedbs BS.empty)
    where
      (seedbs, _) = Crypto.getBytesFromSeedT 32 seed
  ...
  verificationKeyHash (GenesisExtendedVerificationKey vk) =
    GenesisExtendedKeyHash . Shelley.KeyHash . Crypto.castHash
      $ Crypto.hashWith Crypto.HD.xpubPublicKey vk

instance ToCBOR (VerificationKey GenesisExtendedKey) where
  toCBOR (GenesisExtendedVerificationKey xpub) = toCBOR (Crypto.HD.unXPub xpub)

instance FromCBOR (VerificationKey GenesisExtendedKey) where
  fromCBOR = do
    bs <- fromCBOR
    either fail (return . GenesisExtendedVerificationKey) (Crypto.HD.xpub (bs :: ByteString))

instance ToCBOR (SigningKey GenesisExtendedKey) where
  toCBOR (GenesisExtendedSigningKey xprv) = toCBOR (Crypto.HD.unXPrv xprv)

instance FromCBOR (SigningKey GenesisExtendedKey) where
  fromCBOR = do
    bs <- fromCBOR
    either fail (return . GenesisExtendedSigningKey) (Crypto.HD.xprv (bs :: ByteString))

instance SerialiseAsRawBytes (VerificationKey GenesisExtendedKey) where
  serialiseToRawBytes (GenesisExtendedVerificationKey xpub) = Crypto.HD.unXPub xpub
  deserialiseFromRawBytes (AsVerificationKey AsGenesisExtendedKey) bs =
    first
      (const (SerialiseAsRawBytesError "Unable to deserialise VerificationKey GenesisExtendedKey"))
      $ GenesisExtendedVerificationKey <$> Crypto.HD.xpub bs

instance SerialiseAsRawBytes (SigningKey GenesisExtendedKey) where
  serialiseToRawBytes (GenesisExtendedSigningKey xprv) = Crypto.HD.unXPrv xprv
  deserialiseFromRawBytes (AsSigningKey AsGenesisExtendedKey) bs =
    first
      (\msg ->
         SerialiseAsRawBytesError ("Unable to deserialise SigningKey GenesisExtendedKey" ++ msg))
      $ GenesisExtendedSigningKey <$> Crypto.HD.xprv bs

data instance Hash GenesisExtendedKey = GenesisExtendedKeyHash
  { unGenesisExtendedKeyHash :: Shelley.KeyHash Shelley.Staking StandardCrypto
  } deriving stock (Eq, Ord)
...
instance SerialiseAsRawBytes (Hash GenesisExtendedKey) where
  serialiseToRawBytes (GenesisExtendedKeyHash (Shelley.KeyHash vkh)) = Crypto.hashToBytes vkh
...
instance Key GenesisDelegateKey where
  ...
  data SigningKey GenesisDelegateKey =
    ...
    deriving anyclass (SerialiseAsCBOR)
  deterministicSigningKey ::
       AsType GenesisDelegateKey -> Crypto.Seed -> SigningKey GenesisDelegateKey
  ...
  deterministicSigningKeySeedSize AsGenesisDelegateKey = Crypto.seedSizeDSIGN proxy
    where
      proxy :: Proxy (Shelley.DSIGN StandardCrypto)
      proxy = Proxy
...
instance SerialiseAsRawBytes (VerificationKey GenesisDelegateKey) where
  ...
  deserialiseFromRawBytes (AsVerificationKey AsGenesisDelegateKey) bs =
    maybeToRight
      (SerialiseAsRawBytesError "Unable to deserialise VerificationKey GenesisDelegateKey")
      $ GenesisDelegateVerificationKey . Shelley.VKey <$> Crypto.rawDeserialiseVerKeyDSIGN bs

instance SerialiseAsRawBytes (SigningKey GenesisDelegateKey) where
  serialiseToRawBytes (GenesisDelegateSigningKey sk) = Crypto.rawSerialiseSignKeyDSIGN sk
...
data instance Hash GenesisDelegateKey = GenesisDelegateKeyHash
  { unGenesisDelegateKeyHash :: Shelley.KeyHash Shelley.GenesisDelegate StandardCrypto
  } deriving stock (Eq, Ord)
...
instance SerialiseAsRawBytes (Hash GenesisDelegateKey) where
  serialiseToRawBytes (GenesisDelegateKeyHash (Shelley.KeyHash vkh)) = Crypto.hashToBytes vkh
...
instance HasTextEnvelope (VerificationKey GenesisDelegateKey) where
  textEnvelopeType _ =
    "GenesisDelegateVerificationKey_" <> fromString (Crypto.algorithmNameDSIGN proxy)
    where
      proxy :: Proxy (Shelley.DSIGN StandardCrypto)
      proxy = Proxy

instance HasTextEnvelope (SigningKey GenesisDelegateKey) where
  textEnvelopeType _ = "GenesisDelegateSigningKey_" <> fromString (Crypto.algorithmNameDSIGN proxy)
    where
      proxy :: Proxy (Shelley.DSIGN StandardCrypto)
      proxy = Proxy
...
instance CastSigningKeyRole GenesisDelegateKey StakePoolKey where
  castSigningKey (GenesisDelegateSigningKey skey) = StakePoolSigningKey skey
...
instance Key GenesisDelegateExtendedKey where
  ...
  deterministicSigningKey ::
       AsType GenesisDelegateExtendedKey -> Crypto.Seed -> SigningKey GenesisDelegateExtendedKey
  deterministicSigningKey AsGenesisDelegateExtendedKey seed =
    GenesisDelegateExtendedSigningKey (Crypto.HD.generate seedbs BS.empty)
    where
      (seedbs, _) = Crypto.getBytesFromSeedT 32 seed
  ...
  getVerificationKey ::
       SigningKey GenesisDelegateExtendedKey -> VerificationKey GenesisDelegateExtendedKey
  getVerificationKey (GenesisDelegateExtendedSigningKey sk) =
    ...
    --
  verificationKeyHash ::
       VerificationKey GenesisDelegateExtendedKey -> Hash GenesisDelegateExtendedKey
  verificationKeyHash (GenesisDelegateExtendedVerificationKey vk) =
    GenesisDelegateExtendedKeyHash . Shelley.KeyHash . Crypto.castHash
      $ Crypto.hashWith Crypto.HD.xpubPublicKey vk

instance ToCBOR (VerificationKey GenesisDelegateExtendedKey) where
  toCBOR (GenesisDelegateExtendedVerificationKey xpub) = toCBOR (Crypto.HD.unXPub xpub)
...
instance ToCBOR (SigningKey GenesisDelegateExtendedKey) where
  toCBOR (GenesisDelegateExtendedSigningKey xprv) = toCBOR (Crypto.HD.unXPrv xprv)

instance FromCBOR (SigningKey GenesisDelegateExtendedKey) where
  fromCBOR = do
    bs <- fromCBOR
    either fail (return . GenesisDelegateExtendedSigningKey) (Crypto.HD.xprv (bs :: ByteString))

instance SerialiseAsRawBytes (VerificationKey GenesisDelegateExtendedKey) where
  serialiseToRawBytes (GenesisDelegateExtendedVerificationKey xpub) = Crypto.HD.unXPub xpub
...
instance SerialiseAsRawBytes (SigningKey GenesisDelegateExtendedKey) where
  serialiseToRawBytes (GenesisDelegateExtendedSigningKey xprv) = Crypto.HD.unXPrv xprv
  deserialiseFromRawBytes (AsSigningKey AsGenesisDelegateExtendedKey) bs =
    first
      (\msg ->
         SerialiseAsRawBytesError
           ("Unable to deserialise SigningKey GenesisDelegateExtendedKey: " ++ msg))
      $ GenesisDelegateExtendedSigningKey <$> Crypto.HD.xprv bs

data instance Hash GenesisDelegateExtendedKey = GenesisDelegateExtendedKeyHash
  { unGenesisDelegateExtendedKeyHash :: Shelley.KeyHash Shelley.Staking StandardCrypto
  } deriving stock (Eq, Ord)
...
instance SerialiseAsRawBytes (Hash GenesisDelegateExtendedKey) where
  ...
  deserialiseFromRawBytes (AsHash AsGenesisDelegateExtendedKey) bs =
    maybeToRight
      (SerialiseAsRawBytesError "Unable to deserialise Hash GenesisDelegateExtendedKey: ")
...
instance Key GenesisUTxOKey where
  ...
  deterministicSigningKey AsGenesisUTxOKey seed = GenesisUTxOSigningKey (Crypto.genKeyDSIGN seed)
  deterministicSigningKeySeedSize :: AsType GenesisUTxOKey -> Word
  deterministicSigningKeySeedSize AsGenesisUTxOKey = Crypto.seedSizeDSIGN proxy
    where
      proxy :: Proxy (Shelley.DSIGN StandardCrypto)
      proxy = Proxy
  ...
  verificationKeyHash (GenesisUTxOVerificationKey vkey) = GenesisUTxOKeyHash (Shelley.hashKey vkey)
...
instance SerialiseAsRawBytes (SigningKey GenesisUTxOKey) where
  serialiseToRawBytes (GenesisUTxOSigningKey sk) = Crypto.rawSerialiseSignKeyDSIGN sk
...
data instance Hash GenesisUTxOKey = GenesisUTxOKeyHash
  { unGenesisUTxOKeyHash :: Shelley.KeyHash Shelley.Payment StandardCrypto
  } deriving stock (Eq, Ord)
...
instance SerialiseAsRawBytes (Hash GenesisUTxOKey) where
  serialiseToRawBytes (GenesisUTxOKeyHash (Shelley.KeyHash vkh)) = Crypto.hashToBytes vkh
...
instance HasTextEnvelope (VerificationKey GenesisUTxOKey) where
  textEnvelopeType _ = "GenesisUTxOVerificationKey_" <> fromString (Crypto.algorithmNameDSIGN proxy)
    where
      proxy :: Proxy (Shelley.DSIGN StandardCrypto)
      proxy = Proxy

instance HasTextEnvelope (SigningKey GenesisUTxOKey) where
  textEnvelopeType _ = "GenesisUTxOSigningKey_" <> fromString (Crypto.algorithmNameDSIGN proxy)
    where
      proxy :: Proxy (Shelley.DSIGN StandardCrypto)
      proxy = Proxy
...
instance CastSigningKeyRole GenesisUTxOKey PaymentKey where
  castSigningKey (GenesisUTxOSigningKey skey) = PaymentSigningKey skey
...
instance Key StakePoolKey where
  ...
  deterministicSigningKey AsStakePoolKey seed = StakePoolSigningKey (Crypto.genKeyDSIGN seed)
  deterministicSigningKeySeedSize :: AsType StakePoolKey -> Word
  deterministicSigningKeySeedSize AsStakePoolKey = Crypto.seedSizeDSIGN proxy
    where
      proxy :: Proxy (Shelley.DSIGN StandardCrypto)
      proxy = Proxy
  ...
  verificationKeyHash (StakePoolVerificationKey vkey) = StakePoolKeyHash (Shelley.hashKey vkey)
...
instance SerialiseAsRawBytes (VerificationKey StakePoolKey) where
  ...
  deserialiseFromRawBytes (AsVerificationKey AsStakePoolKey) bs =
    maybeToRight (SerialiseAsRawBytesError "Unable to deserialise VerificationKey StakePoolKey")
      $ StakePoolVerificationKey . Shelley.VKey <$> Crypto.rawDeserialiseVerKeyDSIGN bs

instance SerialiseAsRawBytes (SigningKey StakePoolKey) where
  serialiseToRawBytes (StakePoolSigningKey sk) = Crypto.rawSerialiseSignKeyDSIGN sk
...
data instance Hash StakePoolKey = StakePoolKeyHash
...
instance SerialiseAsRawBytes (Hash StakePoolKey) where
  serialiseToRawBytes (StakePoolKeyHash (Shelley.KeyHash vkh)) = Crypto.hashToBytes vkh
...
instance HasTextEnvelope (VerificationKey StakePoolKey) where
  textEnvelopeType _ = "StakePoolVerificationKey_" <> fromString (Crypto.algorithmNameDSIGN proxy)
    where
      proxy :: Proxy (Shelley.DSIGN StandardCrypto)
      proxy = Proxy

instance HasTextEnvelope (SigningKey StakePoolKey) where
  textEnvelopeType _ = "StakePoolSigningKey_" <> fromString (Crypto.algorithmNameDSIGN proxy)
    where
      proxy :: Proxy (Shelley.DSIGN StandardCrypto)
      proxy = Proxy
...
instance Key DRepKey where
  ...
  deterministicSigningKey AsDRepKey seed = DRepSigningKey (Crypto.genKeyDSIGN seed)
  deterministicSigningKeySeedSize :: AsType DRepKey -> Word
  deterministicSigningKeySeedSize AsDRepKey = Crypto.seedSizeDSIGN proxy
    where
      proxy :: Proxy (Shelley.DSIGN StandardCrypto)
      proxy = Proxy
  ...
  verificationKeyHash (DRepVerificationKey vkey) = DRepKeyHash (Shelley.hashKey vkey)

instance SerialiseAsRawBytes (VerificationKey DRepKey) where
  serialiseToRawBytes (DRepVerificationKey (Shelley.VKey vk)) = Crypto.rawSerialiseVerKeyDSIGN vk
  deserialiseFromRawBytes (AsVerificationKey AsDRepKey) bs =
    maybeToRight (SerialiseAsRawBytesError "Unable to deserialise VerificationKey DRepKey")
      $ DRepVerificationKey . Shelley.VKey <$> Crypto.rawDeserialiseVerKeyDSIGN bs

instance SerialiseAsRawBytes (SigningKey DRepKey) where
  serialiseToRawBytes (DRepSigningKey sk) = Crypto.rawSerialiseSignKeyDSIGN sk
...
data instance Hash DRepKey = DRepKeyHash
  { unDRepKeyHash :: Shelley.KeyHash Shelley.DRepRole StandardCrypto
  } deriving stock (Eq, Ord)
...
instance SerialiseAsRawBytes (Hash DRepKey) where
  serialiseToRawBytes (DRepKeyHash (Shelley.KeyHash vkh)) = Crypto.hashToBytes vkh
      ...
      case deserialiseFromBech32 (AsHash AsDRepKey) str of
        Left err ->
          fail
            $ docToString
            $ mconcat
                ["Error deserialising Hash DRepKey: " <> pretty str, " Error: " <> prettyError err]
        Right h -> pure h

instance HasTextEnvelope (VerificationKey DRepKey) where
  textEnvelopeType _ = "DRepVerificationKey_" <> fromString (Crypto.algorithmNameDSIGN proxy)
    where
      proxy :: Proxy (Shelley.DSIGN StandardCrypto)
      proxy = Proxy

instance HasTextEnvelope (SigningKey DRepKey) where
  textEnvelopeType _ = "DRepSigningKey_" <> fromString (Crypto.algorithmNameDSIGN proxy)
    where
      proxy :: Proxy (Shelley.DSIGN StandardCrypto)
      proxy = Proxy
...
instance Key DRepExtendedKey where
  ...
  data SigningKey DRepExtendedKey =
    ...
    deriving anyclass (SerialiseAsCBOR)
    deriving (Show, IsString) via UsingRawBytesHex (SigningKey PaymentExtendedKey)
  deterministicSigningKey :: AsType DRepExtendedKey -> Crypto.Seed -> SigningKey DRepExtendedKey
  deterministicSigningKey AsDRepExtendedKey seed =
    DRepExtendedSigningKey (Crypto.HD.generate seedbs BS.empty)
    where
      (seedbs, _) = Crypto.getBytesFromSeedT 32 seed
  ...
  getVerificationKey (DRepExtendedSigningKey sk) = DRepExtendedVerificationKey (Crypto.HD.toXPub sk)
    -- | We use the hash of the normal non-extended pub key so that it is
    -- consistent with the one used in addresses and signatures.
    --
  ...
  verificationKeyHash (DRepExtendedVerificationKey vk) =
    DRepExtendedKeyHash . Shelley.KeyHash . Crypto.castHash
      $ Crypto.hashWith Crypto.HD.xpubPublicKey vk

data instance Hash DRepExtendedKey = DRepExtendedKeyHash
  { unDRepExtendedKeyHash :: Shelley.KeyHash Shelley.DRepRole StandardCrypto
  } deriving stock (Eq, Ord)
    deriving (Show, IsString) via UsingRawBytesHex (Hash DRepKey)
    deriving (ToCBOR, FromCBOR) via UsingRawBytes (Hash DRepKey)
    deriving anyclass (SerialiseAsCBOR)
...
instance ToCBOR (VerificationKey DRepExtendedKey) where
  toCBOR (DRepExtendedVerificationKey xpub) = toCBOR (Crypto.HD.unXPub xpub)

instance FromCBOR (VerificationKey DRepExtendedKey) where
  fromCBOR = do
    bs <- fromCBOR
    either fail (return . DRepExtendedVerificationKey) (Crypto.HD.xpub (bs :: ByteString))

instance ToCBOR (SigningKey DRepExtendedKey) where
  toCBOR (DRepExtendedSigningKey xprv) = toCBOR (Crypto.HD.unXPrv xprv)

instance FromCBOR (SigningKey DRepExtendedKey) where
  fromCBOR = do
    bs <- fromCBOR
    either fail (return . DRepExtendedSigningKey) (Crypto.HD.xprv (bs :: ByteString))

instance SerialiseAsRawBytes (VerificationKey DRepExtendedKey) where
  serialiseToRawBytes (DRepExtendedVerificationKey xpub) = Crypto.HD.unXPub xpub
...
instance SerialiseAsRawBytes (SigningKey DRepExtendedKey) where
  serialiseToRawBytes (DRepExtendedSigningKey xprv) = Crypto.HD.unXPrv xprv
...
instance SerialiseAsRawBytes (Hash DRepExtendedKey) where
  serialiseToRawBytes (DRepExtendedKeyHash (Shelley.KeyHash vkh)) = Crypto.hashToBytes vkh
...
instance CastVerificationKeyRole DRepExtendedKey DRepKey where
  castVerificationKey (DRepExtendedVerificationKey vk) =
    DRepVerificationKey
      ...
      $ vk
    where
      impossible = error "castVerificationKey (DRep): byron and shelley key sizes do not match!"

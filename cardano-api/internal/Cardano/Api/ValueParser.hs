...
evalValueExpr vExpr =
  case vExpr of
    ...
    ValueExprMultiAsset polId aName quant -> valueFromList [(AssetId polId aName, quant)]
...
parseValueExpr =
  buildExpressionParser operatorTable valueExprTerm <?> "multi-asset value expression"
  where
    operatorTable = [[Prefix negateOp], [Infix plusOp AssocLeft]]
...
valueExprTerm = do
  ...
  where
    ...
    assetIdUnspecified = spaces *> notFollowedBy alphaNum $> AdaAssetId
...
word64 = do
  ...
  if i > fromIntegral (maxBound :: Word64)
    then fail $ "expecting word64, but the number exceeds the max bound: " <> show i
    else return i
...
assetName = do
  ...
  failEitherWith (\e -> "AssetName deserisalisation failed: " ++ displayError e)
...
policyId = do
  ...
  where
    textToPolicyId =
      fmap PolicyId . deserialiseFromRawBytesHex AsScriptHash . Text.encodeUtf8 . Text.pack

-- | Asset ID parser.
assetId :: Parser AssetId
assetId = try adaAssetId <|> nonAdaAssetId <?> "asset ID"
  where

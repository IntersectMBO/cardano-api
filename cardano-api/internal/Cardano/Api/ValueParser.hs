module Cardano.Api.ValueParser ( parseValue, assetName, policyId ) where
...
import           Text.Parsec.Char
       ( alphaNum, char, digit, hexDigit, space, spaces, string )
...
evalValueExpr vExpr = case vExpr of
...
policyId = do
  ...
  failEitherWith (\e -> fail
                  $ "expecting a 56-hex-digit policy ID, but found "
                  ++ show (length hexText)
                  ++ " hex digits; "
                  ++ displayError e) (textToPolicyId hexText)
  where
    textToPolicyId =
      fmap PolicyId

...
module Cardano.Api.Eon.ShelleyBasedEra
  ...
  , shelleyBasedEraConstraints ) where
...
import           Data.Aeson
       ( FromJSON(..), ToJSON, toJSON, withText )
...
inEonForShelleyBasedEra
  :: () => Eon eon => a -> (eon era -> a) -> ShelleyBasedEra era -> a
inEonForShelleyBasedEra no yes = inEonForEra no yes . toCardanoEra

inEonForShelleyBasedEraMaybe
  :: () => Eon eon => (eon era -> a) -> ShelleyBasedEra era -> Maybe a
inEonForShelleyBasedEraMaybe yes = inEonForShelleyBasedEra Nothing (Just . yes)

forShelleyBasedEraMaybeEon
  :: () => Eon eon => ShelleyBasedEra era -> Maybe (eon era)
forShelleyBasedEraMaybeEon = inEonForEra Nothing Just . toCardanoEra

forShelleyBasedEraInEon
  :: () => Eon eon => ShelleyBasedEra era -> a -> (eon era -> a) -> a
forShelleyBasedEraInEon era no yes = inEonForShelleyBasedEra no yes era

forShelleyBasedEraInEonMaybe
  :: () => Eon eon => ShelleyBasedEra era -> (eon era -> a) -> Maybe a
...
data AnyShelleyBasedEra where
  AnyShelleyBasedEra
    :: Typeable era => ShelleyBasedEra era -> AnyShelleyBasedEra
...
instance FromJSON AnyShelleyBasedEra where
  parseJSON = withText "AnyShelleyBasedEra" $ \case
    ...
    wrong     -> fail
      $ "Failed to parse unknown shelley-based era: " <> Text.unpack wrong
...
data InAnyShelleyBasedEra thing where
  InAnyShelleyBasedEra :: Typeable era
    => ShelleyBasedEra era
    -> thing era
...
requireShelleyBasedEra
  :: () => Applicative m => CardanoEra era -> m (Maybe (ShelleyBasedEra era))

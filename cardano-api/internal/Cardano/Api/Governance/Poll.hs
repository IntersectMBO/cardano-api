                ...
                $ [ (pollMetadataKeyQuestion, metaTextChunks govPollQuestion)
                  , (pollMetadataKeyAnswers, TxMetaList (metaTextChunks <$> govPollAnswers))
                    ...
                    ++ case govPollNonce of
                         Nothing -> []
                         Just nonce -> [(pollMetadataKeyNonce, TxMetaNumber (toInteger nonce))])
          ]
    ...
    withNestedMap lbl pollMetadataLabel metadata $ \values ->
      GovernancePoll
        ...
        <*> (let key = pollMetadataKeyAnswers
              in case lookup key values of
                   Just (TxMetaList xs) -> traverse (expectTextChunks (fieldPath lbl key)) xs
                   Just _ -> Left $ malformedField (fieldPath lbl key) "List of Text (answers)"
                   Nothing -> Left $ missingField (fieldPath lbl key))
        <*> (let key = pollMetadataKeyNonce
              in case lookup key values of
                   Just (TxMetaNumber nonce) -> Just <$> expectWord (fieldPath lbl key) nonce
                   Nothing -> pure Nothing
                   Just _ -> Left $ malformedField (fieldPath lbl key) "Number (nonce)")
    where
      lbl = "GovernancePoll"
...
--
data instance Hash GovernancePoll = GovernancePollHash
...
hashGovernancePoll = GovernancePollHash . hashWith @(HASH StandardCrypto) serialiseToCBOR
      ...
      GovernancePollAnswer
        ...
        <*> (let key = pollMetadataKeyChoice
              in case lookup key values of
                   ...
                   Just _ -> Left $ malformedField (fieldPath lbl key) "Number (answer index)"
                   Nothing -> Left $ missingField (fieldPath lbl key))
    where
      ...
      expectHash key value =
        case value of
          ...
          _ -> Left (malformedField (fieldPath lbl key) "Bytes (32 bytes hash digest)")
...
renderGovernancePollError err =
  case err of
    ...
    ErrGovernancePollNoAnswer -> "No answer found in the provided transaction's metadata."
    ErrGovernancePollUnauthenticated ->
      mconcat
...
verifyPollAnswer ::
     GovernancePoll -> InAnyShelleyBasedEra Tx -> Either GovernancePollError [Hash PaymentKey]
  ...
  where
    ...
    hasMatchingHash answer calculatedHashFromPoll = do
      ...
      when (calculatedHashFromPoll /= specifiedHashInAnswer)
        ...
        $ GovernancePollMismatchError {specifiedHashInAnswer, calculatedHashFromPoll}
      ...
      when (govAnsChoice answer >= fromIntegral (length answers)) $ do
        ...
        Left
          ...
          $ GovernancePollInvalidAnswerError
              {invalidAnswerReceivedAnswer, invalidAnswerAcceptableAnswers}
    extraKeyWitnesses =
      \case
...
withNestedMap lbl topLevelLabel (TxMetadata m) continueWith =
  case Map.lookup topLevelLabel m of
    ...
    Nothing -> Left $ DecoderErrorCustom lbl ("missing expected label: " <> textShow topLevelLabel)
    Just _ -> Left $ DecoderErrorCustom lbl "malformed data; expected a key:value map"
...
expectWord lbl n
  ...
  | otherwise = Left $ DecoderErrorCustom lbl "invalid number; must be non-negative word"
...
malformedField lbl hint = DecoderErrorCustom lbl ("malformed field; must be: " <> hint)

...
instance IsShelleyBasedEra era => FromCBOR (GovernanceActionId era) where
  fromCBOR = do
    !v <-
      shelleyBasedEraConstraints (shelleyBasedEra @era) $ Ledger.fromEraCBOR @(ShelleyLedgerEra era)
    return $ GovernanceActionId v

newtype Voter era =
  Voter (Ledger.Voter (L.EraCrypto (ShelleyLedgerEra era)))
  deriving (Show, Eq, Ord)
...
instance IsShelleyBasedEra era => ToCBOR (VotingProcedures era) where
  toCBOR =
    \case
      VotingProcedures vp ->
        shelleyBasedEraConstraints (shelleyBasedEra @era) $ L.toEraCBOR @(ShelleyLedgerEra era) vp
        ...
        <$> case (Map.lookup voter mapa, Map.lookup voter mapb) of
              ...
              (Just va, Just vb) -- Here's the case where we're unioning different votes for the same voter
               ->
                ...
                where intersection = Map.keys $ Map.intersection va vb

...
instance TestEquality ScriptLanguage where
  ...
  testEquality (PlutusScriptLanguage lang) (PlutusScriptLanguage lang') = testEquality lang lang'
...
instance ToJSON AnyPlutusScriptVersion where
  toJSON (AnyPlutusScriptVersion PlutusScriptV1) = Aeson.String "PlutusScriptV1"
  toJSON (AnyPlutusScriptVersion PlutusScriptV2) = Aeson.String "PlutusScriptV2"
  toJSON (AnyPlutusScriptVersion PlutusScriptV3) = Aeson.String "PlutusScriptV3"
...
class HasTypeProxy lang =>
      IsScriptLanguage lang
  where
...
instance IsScriptLanguage lang => SerialiseAsCBOR (Script lang) where
  ...
  serialiseToCBOR (PlutusScript PlutusScriptV1 s) = CBOR.serialize' s
  serialiseToCBOR (PlutusScript PlutusScriptV2 s) = CBOR.serialize' s
  serialiseToCBOR (PlutusScript PlutusScriptV3 s) = CBOR.serialize' s
  deserialiseFromCBOR _ bs =
    case scriptLanguage :: ScriptLanguage lang of
      ...
      PlutusScriptLanguage PlutusScriptV1 -> PlutusScript PlutusScriptV1 <$> CBOR.decodeFull' bs
      PlutusScriptLanguage PlutusScriptV2 -> PlutusScript PlutusScriptV2 <$> CBOR.decodeFull' bs
      PlutusScriptLanguage PlutusScriptV3 -> PlutusScript PlutusScriptV3 <$> CBOR.decodeFull' bs
...
instance Eq ScriptInAnyLang where
  (==) (ScriptInAnyLang lang script) (ScriptInAnyLang lang' script') =
...
instance ToJSON ScriptInAnyLang where
  toJSON (ScriptInAnyLang l s) =
    object
      ...
      , "script" .= obtainScriptLangConstraint l (serialiseToTextEnvelope Nothing s)
      ]
    where
...
instance FromJSON ScriptInAnyLang where
  parseJSON =
    Aeson.withObject "ScriptInAnyLang" $ \o -> do
...
toScriptInAnyLang s@(SimpleScript _) = ScriptInAnyLang SimpleScriptLanguage s
toScriptInAnyLang s@(PlutusScript v _) = ScriptInAnyLang (PlutusScriptLanguage v) s
...
instance Eq (ScriptInEra era) where
  (==) (ScriptInEra langInEra script) (ScriptInEra langInEra' script') =
    case testEquality
...
--
scriptLanguageSupportedInEra ::
     ShelleyBasedEra era -> ScriptLanguage lang -> Maybe (ScriptLanguageInEra lang era)
scriptLanguageSupportedInEra era lang =
  case (era, lang) of
    (ShelleyBasedEraShelley, SimpleScriptLanguage) -> Just SimpleScriptInShelley
    (ShelleyBasedEraAllegra, SimpleScriptLanguage) -> Just SimpleScriptInAllegra
    (ShelleyBasedEraMary, SimpleScriptLanguage) -> Just SimpleScriptInMary
    (ShelleyBasedEraAlonzo, SimpleScriptLanguage) -> Just SimpleScriptInAlonzo
    (ShelleyBasedEraBabbage, SimpleScriptLanguage) -> Just SimpleScriptInBabbage
    (ShelleyBasedEraConway, SimpleScriptLanguage) -> Just SimpleScriptInConway
    (ShelleyBasedEraAlonzo, PlutusScriptLanguage PlutusScriptV1) -> Just PlutusScriptV1InAlonzo
    (ShelleyBasedEraBabbage, PlutusScriptLanguage PlutusScriptV1) -> Just PlutusScriptV1InBabbage
    (ShelleyBasedEraBabbage, PlutusScriptLanguage PlutusScriptV2) -> Just PlutusScriptV2InBabbage
    (ShelleyBasedEraConway, PlutusScriptLanguage PlutusScriptV1) -> Just PlutusScriptV1InConway
    (ShelleyBasedEraConway, PlutusScriptLanguage PlutusScriptV2) -> Just PlutusScriptV2InConway
    (ShelleyBasedEraConway, PlutusScriptLanguage PlutusScriptV3) -> Just PlutusScriptV3InConway
    _ -> Nothing
...
data PlutusScriptOrReferenceInput lang
  ...
  | PReferenceScript TxIn (Maybe ScriptHash) -- ^ Needed to construct the redeemer pointer map
...
instance Eq (ScriptWitness witctx era) where
  (==) (SimpleScriptWitness langInEra script) (SimpleScriptWitness langInEra' script') =
    case testEquality
           (languageOfScriptLanguageInEra langInEra)
           (languageOfScriptLanguageInEra langInEra') of
      Nothing -> False
      Just Refl -> script == script'
  (==) (PlutusScriptWitness langInEra version script datum redeemer execUnits) (PlutusScriptWitness langInEra' version' script' datum' redeemer' execUnits') =
    case testEquality
           (languageOfScriptLanguageInEra langInEra)
           (languageOfScriptLanguageInEra langInEra') of
      Nothing -> False
...
scriptWitnessScript (SimpleScriptWitness _ (SReferenceScript _ _)) = Nothing
scriptWitnessScript (PlutusScriptWitness _ _ (PReferenceScript _ _) _ _ _) = Nothing
...
--
newtype ScriptHash =
  ScriptHash (Shelley.ScriptHash StandardCrypto)
...
instance SerialiseAsRawBytes ScriptHash where
  serialiseToRawBytes (ScriptHash (Shelley.ScriptHash h)) = Crypto.hashToBytes h
...
examplePlutusScriptAlwaysSucceeds =
  PlutusScriptSerialised . Plutus.alwaysSucceedingNAryFunction . scriptArityForWitCtx
...
examplePlutusScriptAlwaysFails =
  PlutusScriptSerialised . Plutus.alwaysFailingNAryFunction . scriptArityForWitCtx
...
toShelleyScript (ScriptInEra langInEra (PlutusScript PlutusScriptV1 (PlutusScriptSerialised script))) =
  case langInEra of
...
toShelleyScript (ScriptInEra langInEra (PlutusScript PlutusScriptV2 (PlutusScriptSerialised script))) =
  case langInEra of
...
toShelleyScript (ScriptInEra langInEra (PlutusScript PlutusScriptV3 (PlutusScriptSerialised script))) =
  case langInEra of
    PlutusScriptV3InConway ->
      Alonzo.PlutusScript . Conway.ConwayPlutusV3 . Plutus.Plutus $ Plutus.PlutusBinary script

fromShelleyBasedScript ::
     ShelleyBasedEra era -> Ledger.Script (ShelleyLedgerEra era) -> ScriptInEra era
fromShelleyBasedScript sbe script =
  case sbe of
    ShelleyBasedEraShelley ->
      ScriptInEra SimpleScriptInShelley . SimpleScript $ fromShelleyMultiSig script
    ShelleyBasedEraAllegra ->
      ScriptInEra SimpleScriptInAllegra . SimpleScript $ fromAllegraTimelock script
    ShelleyBasedEraMary ->
      ScriptInEra SimpleScriptInMary . SimpleScript $ fromAllegraTimelock script
    ShelleyBasedEraAlonzo ->
      case script of
        Alonzo.PlutusScript (Alonzo.AlonzoPlutusV1 (PlutusScriptBinary s)) ->
          ScriptInEra PlutusScriptV1InAlonzo . PlutusScript PlutusScriptV1
            $ PlutusScriptSerialised s
        Alonzo.TimelockScript s ->
          ScriptInEra SimpleScriptInAlonzo . SimpleScript $ fromAllegraTimelock s
    ShelleyBasedEraBabbage ->
      case script of
        Alonzo.PlutusScript plutusV ->
          case plutusV of
            Babbage.BabbagePlutusV1 (PlutusScriptBinary s) ->
              ScriptInEra PlutusScriptV1InBabbage . PlutusScript PlutusScriptV1
                $ PlutusScriptSerialised s
            Babbage.BabbagePlutusV2 (PlutusScriptBinary s) ->
              ScriptInEra PlutusScriptV2InBabbage . PlutusScript PlutusScriptV2
                $ PlutusScriptSerialised s
        Alonzo.TimelockScript s ->
          ScriptInEra SimpleScriptInBabbage . SimpleScript $ fromAllegraTimelock s
    ShelleyBasedEraConway ->
      case script of
        Alonzo.PlutusScript plutusV ->
          case plutusV of
            Conway.ConwayPlutusV1 (PlutusScriptBinary s) ->
              ScriptInEra PlutusScriptV1InConway . PlutusScript PlutusScriptV1
                $ PlutusScriptSerialised s
            Conway.ConwayPlutusV2 (PlutusScriptBinary s) ->
              ScriptInEra PlutusScriptV2InConway . PlutusScript PlutusScriptV2
                $ PlutusScriptSerialised s
            Conway.ConwayPlutusV3 (PlutusScriptBinary s) ->
              ScriptInEra PlutusScriptV3InConway . PlutusScript PlutusScriptV3
                $ PlutusScriptSerialised s
        Alonzo.TimelockScript s ->
          ScriptInEra SimpleScriptInConway . SimpleScript $ fromAllegraTimelock s
...
--
toShelleyMultiSig ::
     SimpleScript -> Either MultiSigError (Shelley.MultiSig (ShelleyLedgerEra ShelleyEra))
...
toAllegraTimelock = go
  where
    ...
    go (RequireSignature (PaymentKeyHash kh)) = Shelley.RequireSignature (Shelley.asWitness kh)
...
instance ToJSON SimpleScript where
  toJSON (RequireSignature pKeyHash) =
    object ["type" .= String "sig", "keyHash" .= serialiseToRawBytesHexText pKeyHash]
  toJSON (RequireTimeBefore slot) = object ["type" .= String "before", "slot" .= slot]
  toJSON (RequireTimeAfter slot) = object ["type" .= String "after", "slot" .= slot]
  ...
  toJSON (RequireMOf reqNum reqScripts) =
    object ["type" .= String "atLeast", "required" .= reqNum, "scripts" .= map toJSON reqScripts]
        ...
        case r of
          Number sci ->
            case toBoundedInteger sci of
              ...
              Nothing -> fail $ "Error in \"required\" key: " <> show sci <> " is not a valid Int"
...
parsePaymentKeyHash =
  failEitherWith (\e -> "Error deserialising payment key hash: " ++ displayError e)
...
instance IsCardanoEra era => FromJSON (ReferenceScript era) where
  parseJSON =
    Aeson.withObject "ReferenceScript" $ \o ->
...
textEnvelopeToScript = deserialiseFromTextEnvelopeAnyOf textEnvTypes
  where
    ...
    textEnvTypes =
      [ FromSomeType (AsScript AsSimpleScript) (ScriptInAnyLang SimpleScriptLanguage)
      , FromSomeType

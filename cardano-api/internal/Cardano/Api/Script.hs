...
instance TestEquality ScriptLanguage where
  ...
  testEquality (PlutusScriptLanguage lang) (PlutusScriptLanguage lang') =
    testEquality lang lang'
...
instance Enum AnyPlutusScriptVersion where
  ...
  toEnum err =
    error $ "AnyPlutusScriptVersion.toEnum: bad argument: " <> show err
...
parsePlutusScriptVersion t = case t of
...
instance IsScriptLanguage lang => SerialiseAsCBOR (Script lang) where
  serialiseToCBOR (SimpleScript s) =
    CBOR.serialize'
      (toAllegraTimelock s :: Timelock.Timelock (ShelleyLedgerEra AllegraEra))
  ...
  deserialiseFromCBOR _ bs = case scriptLanguage :: ScriptLanguage lang of
    ...
    PlutusScriptLanguage
      PlutusScriptV1 -> PlutusScript PlutusScriptV1 <$> CBOR.decodeFull' bs

    PlutusScriptLanguage
      PlutusScriptV2 -> PlutusScript PlutusScriptV2 <$> CBOR.decodeFull' bs

    PlutusScriptLanguage
      PlutusScriptV3 -> PlutusScript PlutusScriptV3 <$> CBOR.decodeFull' bs

instance IsScriptLanguage lang => HasTextEnvelope (Script lang) where
  textEnvelopeType _ = case scriptLanguage :: ScriptLanguage lang of
...
instance ToJSON ScriptInAnyLang where
  toJSON (ScriptInAnyLang l s) =
    object
      ...
      , "script"
        .= obtainScriptLangConstraint l (serialiseToTextEnvelope Nothing s) ]
    where
...
scriptLanguageSupportedInEra era lang = case ( era, lang ) of
  ( ShelleyBasedEraShelley
    , SimpleScriptLanguage ) -> Just SimpleScriptInShelley

  ( ShelleyBasedEraAllegra
    , SimpleScriptLanguage ) -> Just SimpleScriptInAllegra
  ...
  ( ShelleyBasedEraBabbage
    , SimpleScriptLanguage ) -> Just SimpleScriptInBabbage

  ( ShelleyBasedEraConway, SimpleScriptLanguage ) -> Just SimpleScriptInConway

  ( ShelleyBasedEraAlonzo
    , PlutusScriptLanguage PlutusScriptV1 ) -> Just PlutusScriptV1InAlonzo

  ( ShelleyBasedEraBabbage
    , PlutusScriptLanguage PlutusScriptV1 ) -> Just PlutusScriptV1InBabbage

  ( ShelleyBasedEraBabbage
    , PlutusScriptLanguage PlutusScriptV2 ) -> Just PlutusScriptV2InBabbage

  ( ShelleyBasedEraConway
    , PlutusScriptLanguage PlutusScriptV1 ) -> Just PlutusScriptV1InConway

  ( ShelleyBasedEraConway
    , PlutusScriptLanguage PlutusScriptV2 ) -> Just PlutusScriptV2InConway

  ( ShelleyBasedEraConway
    , PlutusScriptLanguage PlutusScriptV3 ) -> Just PlutusScriptV3InConway

  _ -> Nothing

languageOfScriptLanguageInEra
  :: ScriptLanguageInEra lang era -> ScriptLanguage lang
languageOfScriptLanguageInEra langInEra = case langInEra of
...
eraOfScriptLanguageInEra langInEra = case langInEra of
...
instance Eq (ScriptWitness witctx era) where
  ...
  (==)
    (PlutusScriptWitness langInEra version script datum redeemer execUnits)
    (PlutusScriptWitness
       langInEra'
       version'
       script'
...
scriptWitnessScript
  (SimpleScriptWitness SimpleScriptInShelley (SScript script)) =
  Just $ ScriptInEra SimpleScriptInShelley (SimpleScript script)

scriptWitnessScript
  (SimpleScriptWitness SimpleScriptInAllegra (SScript script)) =
...
scriptWitnessScript
  (SimpleScriptWitness SimpleScriptInAlonzo (SScript script)) =
  Just $ ScriptInEra SimpleScriptInAlonzo (SimpleScript script)

scriptWitnessScript
  (SimpleScriptWitness SimpleScriptInBabbage (SScript script)) =
...
instance FromJSON ExecutionUnits where
  parseJSON = Aeson.withObject "ExecutionUnits" $ \o
    -> ExecutionUnits <$> o .: "steps" <*> o .: "memory"
...
pattern PlutusScriptBinary
  :: Plutus.PlutusLanguage l => ShortByteString -> Plutus.Plutus l
...
hashScript (SimpleScript s) =
  ...
  . (toAllegraTimelock
       :: SimpleScript -> Timelock.Timelock (ShelleyLedgerEra AllegraEra))
  $ s
...
instance IsPlutusScriptLanguage lang
  => HasTextEnvelope (PlutusScript lang) where
  textEnvelopeType _ = case plutusScriptVersion :: PlutusScriptVersion lang of
...
--
examplePlutusScriptAlwaysSucceeds
  :: WitCtx witctx -> PlutusScript PlutusScriptV1
examplePlutusScriptAlwaysSucceeds =
...
toShelleyScript
  (ScriptInEra langInEra (SimpleScript script)) = case langInEra of
...
toShelleyScript
  (ScriptInEra
     langInEra
     (PlutusScript PlutusScriptV1 (PlutusScriptSerialised script))) =
  case langInEra of
    PlutusScriptV1InAlonzo
      -> Alonzo.PlutusScript . Alonzo.AlonzoPlutusV1 . Plutus.Plutus
      $ Plutus.PlutusBinary script
    PlutusScriptV1InBabbage
      -> Alonzo.PlutusScript . Babbage.BabbagePlutusV1 . Plutus.Plutus
      $ Plutus.PlutusBinary script
...
toShelleyScript
  (ScriptInEra
     langInEra
     (PlutusScript PlutusScriptV2 (PlutusScriptSerialised script))) =
  case langInEra of
    PlutusScriptV2InBabbage
      -> Alonzo.PlutusScript . Babbage.BabbagePlutusV2 . Plutus.Plutus
      $ Plutus.PlutusBinary script
    PlutusScriptV2InConway
      -> Alonzo.PlutusScript . Conway.ConwayPlutusV2 . Plutus.Plutus
      $ Plutus.PlutusBinary script
...
fromShelleyBasedScript :: ShelleyBasedEra era
                       ...
                       -> ScriptInEra era
fromShelleyBasedScript sbe script = case sbe of
  ShelleyBasedEraShelley -> ScriptInEra SimpleScriptInShelley . SimpleScript
    $ fromShelleyMultiSig script
  ShelleyBasedEraAllegra -> ScriptInEra SimpleScriptInAllegra . SimpleScript
    $ fromAllegraTimelock script
  ShelleyBasedEraMary    -> ScriptInEra SimpleScriptInMary . SimpleScript
    $ fromAllegraTimelock script
  ShelleyBasedEraAlonzo  -> case script of
    ...
    Alonzo.TimelockScript s -> ScriptInEra SimpleScriptInAlonzo . SimpleScript
      $ fromAllegraTimelock s
  ShelleyBasedEraBabbage -> case script of
    Alonzo.PlutusScript plutusV -> case plutusV of
      ...
      Babbage.BabbagePlutusV2 (PlutusScriptBinary s)
        ...
        $ PlutusScriptSerialised s
    Alonzo.TimelockScript s     -> ScriptInEra SimpleScriptInBabbage
      . SimpleScript
      $ fromAllegraTimelock s

  ShelleyBasedEraConway  -> case script of
    Alonzo.PlutusScript plutusV -> case plutusV of
      ...
      Conway.ConwayPlutusV3 (PlutusScriptBinary s)
        ...
        $ PlutusScriptSerialised s
    Alonzo.TimelockScript s     -> ScriptInEra SimpleScriptInConway
      . SimpleScript
      $ fromAllegraTimelock s

data MultiSigError = MultiSigErrorTimelockNotsupported
  deriving Show
...
fromShelleyMultiSig = go
  where
    go (Shelley.RequireSignature kh) =
      RequireSignature (PaymentKeyHash (Shelley.coerceKeyRole kh))
...
--
toAllegraTimelock
  :: forall era. ( Allegra.AllegraEraScript era
...
instance ToJSON SimpleScript where
  ...
  toJSON (RequireTimeBefore slot)       =
    object [ "type" .= String "before", "slot" .= slot ]
  toJSON (RequireTimeAfter slot)        =
    object [ "type" .= String "after", "slot" .= slot ]
...
parseScriptAtLeast = Aeson.withObject "atLeast" $ \obj -> do
  ...
  case v :: Text of
    "atLeast" -> do
      ...
      vs <- obj .: "scripts"
      case r of
        Number sci -> case toBoundedInteger sci of
          Just reqInt -> do
...
parseScriptBefore = Aeson.withObject "before" $ \obj -> do
  v <- obj .: "type"
  case v :: Text of
...
parseScriptAfter = Aeson.withObject "after" $ \obj -> do
  v <- obj .: "type"
  case v :: Text of
...
scriptInEraToRefScript sIne@(ScriptInEra _ s) =
  caseShelleyToAlonzoOrBabbageEraOnwards
    ...
    (\w -> ReferenceScript w
     $ toScriptInAnyLang s) -- Any script can be a reference script

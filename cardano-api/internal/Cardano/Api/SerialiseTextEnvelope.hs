...
instance FromJSON TextEnvelope where
  parseJSON = withObject "TextEnvelope" $ \v -> TextEnvelope <$> (v .: "type")
    <*> (v .: "description")
...
textEnvelopeJSONConfig =
  defConfig
  { confCompare = textEnvelopeJSONKeyOrder
  }
...
instance Error TextEnvelopeError where
  prettyError = \case
    TextEnvelopeTypeError
      ...
      (TextEnvelopeType actType) -> mconcat
      [ "TextEnvelope type error: "
      , " Expected: " <> pretty expType
      , " Actual: " <> pretty actType ]

    TextEnvelopeTypeError expTypes (TextEnvelopeType actType) -> mconcat
      [ "TextEnvelope type error: "
      , " Expected one of: "
      , mconcat
        $ List.intersperse
          ", "
          [ pretty expType | TextEnvelopeType expType <- expTypes ]
      , " Actual: " <> pretty actType ]
    TextEnvelopeAesonDecodeError
      decErr -> "TextEnvelope aeson decode error: " <> pretty decErr
    TextEnvelopeDecodeError
      decErr -> "TextEnvelope decode error: " <> pshow decErr
...
deserialiseFromTextEnvelope ttoken te = do
  ...
  first TextEnvelopeDecodeError $ deserialiseFromCBOR ttoken (teRawCBOR te) --TODO: You have switched from CBOR to JSON

deserialiseFromTextEnvelopeAnyOf :: [ FromSomeType HasTextEnvelope b ]
                                 -> TextEnvelope
                                 -> Either TextEnvelopeError b
deserialiseFromTextEnvelopeAnyOf types te = case List.find matching types of
  Nothing -> Left (TextEnvelopeTypeError expectedTypes actualType)

  Just (FromSomeType ttoken f) -> first TextEnvelopeDecodeError
    $ f <$> deserialiseFromCBOR ttoken (teRawCBOR te)
  where
    actualType = teType te

    expectedTypes =
      [ textEnvelopeType ttoken | FromSomeType ttoken _f <- types ]

    matching (FromSomeType ttoken _f) =
      textEnvelopeType ttoken `legacyComparison` actualType
...
textEnvelopeToJSON
  :: HasTextEnvelope a => Maybe TextEnvelopeDescr -> a -> LBS.ByteString
...
readFileTextEnvelope ttoken path = runExceptT $ do
  content <- fileIOExceptT (unFile path) readFileBlocking
  firstExceptT (FileError (unFile path)) $ hoistEither $ do
    te
      <- first TextEnvelopeAesonDecodeError $ Aeson.eitherDecodeStrict' content
...
readFileTextEnvelopeAnyOf types path = runExceptT $ do
  content <- fileIOExceptT (unFile path) readFileBlocking
  firstExceptT (FileError (unFile path)) $ hoistEither $ do
    te
      <- first TextEnvelopeAesonDecodeError $ Aeson.eitherDecodeStrict' content
...
readTextEnvelopeFromFile path = runExceptT $ do
  bs <- fileIOExceptT path readFileBlocking
  firstExceptT (FileError path . TextEnvelopeAesonDecodeError) . hoistEither
...
readTextEnvelopeOfTypeFromFile expectedType path = runExceptT $ do

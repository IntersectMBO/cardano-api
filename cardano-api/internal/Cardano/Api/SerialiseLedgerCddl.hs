...
module Cardano.Api.SerialiseLedgerCddl
  ...
  , serializeByronTx
  , writeByronTxFileTextEnvelopeCddl ) where
...
import           Cardano.Api.SerialiseTextEnvelope
       ( TextEnvelope(..), TextEnvelopeDescr(TextEnvelopeDescr)
       , TextEnvelopeError(..), TextEnvelopeType(TextEnvelopeType)
       , deserialiseFromTextEnvelope, legacyComparison
...
import           Control.Monad.Trans.Except.Extra
       ( firstExceptT, handleIOExceptT, hoistEither, newExceptT, runExceptT )

import qualified Data.Aeson                        as Aeson
import           Data.Aeson.Encode.Pretty
       ( Config(..), defConfig, encodePretty', keyOrder )
...
deserialiseTxLedgerCddl :: forall era. ShelleyBasedEra era
                        -> TextEnvelope
                        -> Either TextEnvelopeError (Tx era)
deserialiseTxLedgerCddl
  era = shelleyBasedEraConstraints era $ deserialiseFromTextEnvelope asType
  where
...
writeByronTxFileTextEnvelopeCddl :: File content Out
                                 ...
                                 -> IO (Either (FileError ()) ())
writeByronTxFileTextEnvelopeCddl path w = runExceptT $ do
  handleIOExceptT (FileIOError (unFile path))
    $ LBS.writeFile (unFile path) txJson
  where
    txJson =
      encodePretty' textEnvelopeCddlJSONConfig (serializeByronTx w) <> "\n"
...
deserialiseByronTxCddl
  :: TextEnvelope -> Either TextEnvelopeCddlError (Byron.ATxAux ByteString)
...
serialiseWitnessLedgerCddl sbe kw =
  ...
  $ serialiseToTextEnvelope
    (Just (TextEnvelopeDescr $ T.unpack $ genDesc kw))
    kw
  where
...
deserialiseWitnessLedgerCddl :: forall era. ShelleyBasedEra era
                             -> TextEnvelope
...
deserialiseWitnessLedgerCddl sbe te =
  ...
  where
    ...
    legacyDecoding :: TextEnvelope
                   ...
                   -> Either TextEnvelopeCddlError (KeyWitness era)
    legacyDecoding
      TextEnvelope { teDescription, teRawCBOR }
      (Left (TextEnvelopeCddlErrCBORDecodingError _)) = case teDescription of
...
writeTxFileTextEnvelopeCddl
  ...
  -> Tx era
  -> IO (Either (FileError ()) ())
writeTxFileTextEnvelopeCddl era path tx = runExceptT $ do
  handleIOExceptT (FileIOError (unFile path))
    $ LBS.writeFile (unFile path) txJson
  where
    txJson =
      encodePretty' textEnvelopeCddlJSONConfig (serialiseTxLedgerCddl era tx)
      <> "\n"
...
writeTxWitnessFileTextEnvelopeCddl
  ...
  -> KeyWitness era
  -> IO (Either (FileError ()) ())
writeTxWitnessFileTextEnvelopeCddl sbe path w = runExceptT $ do
  handleIOExceptT (FileIOError (unFile path))
    $ LBS.writeFile (unFile path) txJson
  where
    txJson =
...
textEnvelopeCddlJSONConfig =
  defConfig
  { confCompare = textEnvelopeCddlJSONKeyOrder
  }
...
data FromSomeTypeCDDL c b where
  FromCDDLTx :: Text -- ^ CDDL type that we want
    -> (InAnyShelleyBasedEra Tx -> b)
    -> FromSomeTypeCDDL TextEnvelope b

  FromCDDLWitness :: Text -- ^ CDDL type that we want
...
deserialiseFromTextEnvelopeCddlAnyOf types teCddl =
  case List.find matching types of
    Nothing -> Left (TextEnvelopeCddlTypeError expectedTypes actualType)
    ...
    Just (FromCDDLTx ttoken f) -> do
      ...
      f . InAnyShelleyBasedEra era
        <$> mapLeft
          textEnvelopeErrorToTextEnvelopeCddlError
          (deserialiseTxLedgerCddl era teCddl)
  ...
  where
    ...
    matching (FromCDDLTx ttoken _f)      =
      TextEnvelopeType (T.unpack ttoken) `legacyComparison` teType teCddl
    matching (FromCDDLWitness ttoken _f) =
      TextEnvelopeType (T.unpack ttoken) `legacyComparison` teType teCddl
...
cddlTypeToEra = \case
  "Witnessed Tx ShelleyEra"
    -> return $ AnyShelleyBasedEra ShelleyBasedEraShelley
  "Witnessed Tx AllegraEra"
    -> return $ AnyShelleyBasedEra ShelleyBasedEraAllegra
  ...
  "Witnessed Tx BabbageEra"
    -> return $ AnyShelleyBasedEra ShelleyBasedEraBabbage
  ...
  "Unwitnessed Tx ConwayEra"
    -> return $ AnyShelleyBasedEra ShelleyBasedEraConway
...
readFileTextEnvelopeCddlAnyOf types path = runExceptT $ do
...
readTextEnvelopeCddlFromFile
  :: FilePath -> IO (Either (FileError TextEnvelopeCddlError) TextEnvelope)
readTextEnvelopeCddlFromFile path = runExceptT $ do

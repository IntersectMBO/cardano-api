...
textEnvelopeErrorToTextEnvelopeCddlError =
  \case
    TextEnvelopeTypeError expectedTypes actualType ->
      TextEnvelopeCddlTypeError (map (T.pack . show) expectedTypes) (T.pack $ show actualType)
...
{-# DEPRECATED
serialiseTxLedgerCddl "Use 'serialiseToTextEnvelope' from 'Cardano.Api.SerialiseTextEnvelope' instead."
 #-}
...
{-# DEPRECATED
deserialiseTxLedgerCddl "Use 'deserialiseFromTextEnvelope' from 'Cardano.Api.SerialiseTextEnvelope' instead."
 #-}

deserialiseTxLedgerCddl ::
     forall era. ShelleyBasedEra era -> TextEnvelope -> Either TextEnvelopeError (Tx era)
deserialiseTxLedgerCddl era = shelleyBasedEraConstraints era $ deserialiseFromTextEnvelope asType
  where
    asType :: AsType (Tx era)
    asType = shelleyBasedEraConstraints era $ proxyToAsType Proxy

writeByronTxFileTextEnvelopeCddl ::
     File content Out -> Byron.ATxAux ByteString -> IO (Either (FileError ()) ())
...
serializeByronTx tx =
  TextEnvelope
    {teType = "Tx ByronEra", teDescription = "Ledger Cddl Format", teRawCBOR = CBOR.recoverBytes tx}
...
writeTxFileTextEnvelopeCddl ::
     () => ShelleyBasedEra era -> File content Out -> Tx era -> IO (Either (FileError ()) ())
...
writeTxWitnessFileTextEnvelopeCddl ::
     ShelleyBasedEra era -> File () Out -> KeyWitness era -> IO (Either (FileError ()) ())
...
textEnvelopeCddlJSONConfig = defConfig {confCompare = textEnvelopeCddlJSONKeyOrder}
...
deserialiseFromTextEnvelopeCddlAnyOf ::
     [FromSomeTypeCDDL TextEnvelope b] -> TextEnvelope -> Either TextEnvelopeCddlError b
...
readTextEnvelopeCddlFromFile path =
  runExceptT $ do
    ...
    firstExceptT (FileError path . TextEnvelopeCddlAesonDecodeError path) . hoistEither

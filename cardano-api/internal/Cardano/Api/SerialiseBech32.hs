...
module Cardano.Api.SerialiseBech32
  ...
  , deserialiseAnyOfFromBech32 ) where
...
deserialiseFromBech32
  :: SerialiseAsBech32 a => AsType a -> Text -> Either Bech32DecodeError a
deserialiseFromBech32 asType bech32Str = do
  ( prefix
    , dataPart ) <- Bech32.decodeLenient bech32Str ?!. Bech32DecodingError
...
deserialiseAnyOfFromBech32 :: forall b. [ FromSomeType SerialiseAsBech32 b ]
                           -> Text
...
deserialiseAnyOfFromBech32 types bech32Str = do
  ...
  FromSomeType actualType fromType <- findForPrefix actualPrefix
  ...
  where
    ...
    findForPrefix prefix = List.find (\(FromSomeType t _) -> prefix
                                      `elem` bech32PrefixesPermitted t) types

    permittedPrefixes :: Set Text
    permittedPrefixes =
      Set.fromList
      $ concat
        [ bech32PrefixesPermitted ttoken | FromSomeType ttoken _f <- types ]
...
instance Error Bech32DecodeError where
  prettyError = \case
    ...
    Bech32UnexpectedPrefix actual permitted -> mconcat

...
estimateOrCalculateBalancedTxBody
  ...
  stakeDelegDeposits
  drepDelegDeposits
  changeAddr = case feeEstMode of
  CalculateWithSpendableUTxO utxo systemstart ledgerEpochInfo mOverride -> first
    AutoBalanceCalculationError
    $ makeTransactionBodyAutoBalance
      era
      systemstart
      ledgerEpochInfo
  ...
  EstimateWithoutSpendableUTxO
    ...
    (TotalReferenceScriptsSize totalRefScriptsSize) -> forShelleyBasedEraInEon
    era
    (Left
     $ AutoBalanceEstimationError TxFeeEstimationOnlyMaryOnwardsSupportedError)
...
estimateBalancedTxBody
  ...
  let certificates = case txCertificates txbodycontent1 of
        TxCertificatesNone       -> []
      ...
      totalDeposits =
        ...
        in  sum
              [ maryEraOnwardsConstraints w
                $ L.getTotalDepositsTxCerts
                  pparams
                  assumeStakePoolHasNotBeenRegistered
                  certificates
              , mconcat $ map (^. L.pProcDepositL) $ toList proposalProcedures ]

      availableUTxOValue =
        mconcat [ totalUTxOValue, negateValue (lovelaceToValue totalDeposits) ]

  let change =
        toLedgerValue w
        $ calculateChangeValue sbe availableUTxOValue txbodycontent1
  ...
  let ( dummyCollRet, dummyTotColl ) =
        maybeDummyTotalCollAndCollReturnOutput sbe txbodycontent changeaddr
  ...
  txbody1ForFeeEstimateOnly <- first TxFeeEstimationxBodyError
  ...
  let fee =
      ...
      ( retColl, reqCol ) =
        caseShelleyToAlonzoOrBabbageEraOnwards
          ...
          (\w' -> calcReturnAndTotalCollateral
             w'
             fee
             pparams
  ...
  txbody2 <- first TxFeeEstimationxBodyError
    $ -- TODO: impossible to fail now
    createAndValidateTransactionBody
      sbe
      txbodycontent1
  ...
  let fakeUTxO =
        createFakeUTxO sbe txbodycontent1 $ selectLovelace availableUTxOValue
      balance  =
        evaluateTransactionBalance
          sbe
          pparams
  ...
  first TxFeeEstimationBalanceError
    $ balanceCheck sbe pparams changeaddr balance

  -- Step 6. Check all txouts have the min required UTxO value
  forM_ (txOuts txbodycontent1) $ \txout
    -> first TxFeeEstimationBalanceError $ checkMinUTxOValue sbe txout pparams
  ...
  txbody3 <- first TxFeeEstimationFinalConstructionError
...
evaluateTransactionFee sbe pp txbody keywitcount byronwitcount refScriptsSize =
  ...
  $ case makeSignedTransaction' (toCardanoEra sbe) [] txbody of
    ShelleyTx _ tx -> L.estimateMinFeeTx
      pp
      tx
      (fromIntegral keywitcount)
      (fromIntegral byronwitcount)
      refScriptsSize
...
estimateTransactionKeyWitnessCount
  ...
  + case txInsCollateral of
    TxInsCollateral _ txins -> length txins
    _ -> 0
  + case txExtraKeyWits of
    TxExtraKeyWitnesses _ khs -> length khs
    _ -> 0
...
data ScriptExecutionError
  ...
  | ScriptErrorMissingScript ScriptWitnessIndex -- The invalid pointer
...
instance Error ScriptExecutionError where
  prettyError = \case
    ...
    ScriptErrorNotPlutusWitnessedTxIn scriptWitness scriptHash -> mconcat
    ...
    ScriptErrorRedeemerPointsToUnknownScriptHash scriptWitness -> mconcat
      [ pretty (renderScriptWitnessIndex scriptWitness)
      , " points to a script hash that is not known." ]

    ScriptErrorMissingScript rdmrPtr resolveable -> mconcat
      [ "The redeemer pointer: " <> pshow rdmrPtr <> " points to a Plutus "
      , "script that does not exist.\n"
      , "The pointers that can be resolved are: " <> pshow resolveable ]

    ScriptErrorMissingCostModel
      language -> "No cost model was found for language " <> pshow language

    ScriptErrorTranslationError
      e -> "Error translating the transaction context: " <> pshow e
...
instance Error (TransactionValidityError era) where
  prettyError = \case
    TransactionValidityIntervalError pastTimeHorizon -> mconcat
      ...
      where
        ...
        timeHorizonSlots Consensus.PastHorizon { Consensus.pastHorizonSummary }
          | eraSummaries@(_ : _) <- pastHorizonSummary
            ...
            <- (Consensus.eraSafeZone . Consensus.eraParams . last)
              eraSummaries =
            fromIntegral slots

          | otherwise = 0 -- This should be impossible.

    TransactionValidityCostModelError cModels err -> mconcat
...
evaluateTransactionExecutionUnits era systemstart epochInfo pp utxo txbody =
  case makeSignedTransaction' era [] txbody of
    ShelleyTx sbe tx' -> evaluateTransactionExecutionUnitsShelley
      sbe
      systemstart
      epochInfo
      pp
  ...
  where
    ...
    fromLedgerScriptExUnitsMap
      ...
      -> Map
        ...
        (Either
           ...
           ( EvalTxExecutionUnitsLog, Alonzo.ExUnits ))
      -> Map
        ScriptWitnessIndex
        (Either
           ScriptExecutionError
    ...
    fromAlonzoScriptExecutionError aOnwards =
      shelleyBasedEraConstraints sbe $ \case
        ...
        L.RedeemerPointsToUnknownScriptHash
          rdmrPtr -> ScriptErrorRedeemerPointsToUnknownScriptHash
          $ toScriptIndex aOnwards rdmrPtr
        ...
        L.MissingScript indexOfScriptWitnessedItem resolveable
          -> let scriptWitnessedItemIndex =
                   toScriptIndex aOnwards indexOfScriptWitnessedItem
        ...
        L.ContextError e -> alonzoEraOnwardsConstraints aOnwards
...
extractPlutusScriptAndLanguage :: Alonzo.AlonzoEraScript (ShelleyLedgerEra era)
...
evaluateTransactionBalance
  ...
  where
    ...
    lookupDRepDeposit drepCred = Map.lookup drepCred drepDelegDeposits
...
instance Error (TxBodyErrorAutoBalance era) where
  prettyError = \case
    TxBodyError err -> prettyError err

    TxBodyScriptExecutionError failures -> mconcat
    ...
    TxBodyErrorAdaBalanceNegative lovelace -> mconcat
    ...
    TxBodyErrorAdaBalanceTooSmall changeOutput minUTxO balance -> mconcat
      ...
      , "Offending output (change output): "
        <> pretty (prettyRenderTxOut changeOutput)
        <> "\n"
    ...
    TxBodyErrorValidityInterval err -> prettyError err

    TxBodyErrorMinUTxONotMet txout minUTxO -> mconcat
      [ "Minimum UTxO threshold not met for tx output: "
        <> pretty (prettyRenderTxOut txout)
        <> "\n"
      , "Minimum required UTxO: " <> pretty minUTxO ]

    TxBodyErrorNonAdaAssetsUnbalanced val -> "Non-Ada assets are unbalanced: "
      <> pretty (renderValue val)

    TxBodyErrorScriptWitnessIndexMissingFromExecUnitsMap sIndex eUnitsMap
      -> mconcat
        [ "ScriptWitnessIndex (redeemer pointer): "
          <> pshow sIndex
...
handleExUnitsErrors ScriptInvalid failuresMap exUnitsMap
  ...
  | otherwise =
    Right $ Map.map (\_ -> ExecutionUnits 0 0) failuresMap <> exUnitsMap
...
makeTransactionBodyAutoBalance
  ...
  txbodycontent
  changeaddr
  mnkeys = shelleyBasedEraConstraints sbe $ do
  -- Our strategy is to:
  ...
  exUnitsMap' <- case Map.mapEither id exUnitsMap of
    ( failures, exUnitsMap' ) -> handleExUnitsErrors
  ...
  let change =
        forShelleyBasedEraInEon
          ...
          mempty
          (\w -> toLedgerValue w
           $ calculateChangeValue sbe totalValueAtSpendableUTxO txbodycontent1)
  ...
  txbody1 <- first TxBodyError
    ...
    createAndValidateTransactionBody
      ...
      -- addresses.
  let nkeys =
        fromMaybe (estimateTransactionKeyWitnessCount txbodycontent1) mnkeys
      fee = calculateMinTxFee sbe pp utxo txbody1 nkeys
      ( retColl, reqCol ) =
        caseShelleyToAlonzoOrBabbageEraOnwards
          (const ( TxReturnCollateralNone, TxTotalCollateralNone ))
          (\w -> let collIns = case txInsCollateral txbodycontent of
                       TxInsCollateral _ collIns' -> collIns'
                       TxInsCollateralNone        -> mempty
                     collateralOuts =
                       catMaybes
                         [ Map.lookup txin (unUTxO utxo) | txin <- collIns ]
                     totalPotentialCollateral =
                       mconcat
                       $ map (\(TxOut _ txOutVal _ _)
                              -> txOutValueToLovelace txOutVal) collateralOuts
...
balanceCheck sbe bpparams changeaddr balance
  | txOutValueToLovelace balance == 0 && onlyAda (txOutValueToValue balance) =
    return ()
  | txOutValueToLovelace balance < 0 =
    Left . TxBodyErrorAdaBalanceNegative $ txOutValueToLovelace balance
  | otherwise =
    case checkMinUTxOValue
      sbe
      (TxOut changeaddr balance TxOutDatumNone ReferenceScriptNone)
      bpparams of
      Left (TxBodyErrorMinUTxONotMet txOutAny minUTxO) -> Left
        $ TxBodyErrorAdaBalanceTooSmall
          txOutAny
          minUTxO
          (txOutValueToLovelace balance)
      Left err -> Left err
      Right _ -> Right ()
...
calculateIncomingUTxOValue :: Monoid (Ledger.Value (ShelleyLedgerEra era))
                           => [ TxOut ctx era ]
                           -> Ledger.Value (ShelleyLedgerEra era)
calculateIncomingUTxOValue providedUtxoOuts =
  mconcat
    [ v | (TxOut _ (TxOutValueShelleyBased _ v) _ _) <- providedUtxoOuts ]
...
calcReturnAndTotalCollateral _ _ _ TxInsCollateralNone _ _ _ _ =
...
calcReturnAndTotalCollateral
  ...
  let totalCollateralLovelace = totalAvailableAda
      ...
      totalCollateral =
        TxTotalCollateral retColSup . L.rationalToCoinViaCeiling $ reqAmt % 100
  ...
  case ( txReturnCollateral, txTotalCollateral ) of
    ...
    ( TxReturnCollateralNone, TxTotalCollateralNone )
      -> if totalCollateralLovelace * 100 >= requiredCollateral
        then ( TxReturnCollateral
                 retColSup
                 (TxOut
                    cAddr
                    (lovelaceToTxOutValue
                       (babbageEraOnwardsToShelleyBasedEra retColSup)
                       returnCollateral)
                    TxOutDatumNone
                    ReferenceScriptNone)
             , totalCollateral )
...
calculateCreatedUTOValue sbe txbodycontent =
  mconcat
    [ fromLedgerValue sbe v | (TxOut _ (TxOutValueShelleyBased _ v) _ _)
    <- txOuts txbodycontent ]
...
createFakeUTxO
  :: ShelleyBasedEra era -> TxBodyContent BuildTx era -> Coin -> UTxO era
createFakeUTxO sbe txbodycontent totalAdaInUTxO =
  let singleTxIn  =
        maybe [] (return . fst)
        $ List.uncons [ txin | ( txin, _ ) <- txIns txbodycontent ]
      singleTxOut =
        maybe
          []
...
updateTxOut
  :: ShelleyBasedEra era -> Coin -> TxOut CtxUTxO era -> TxOut CtxUTxO era
updateTxOut sbe updatedValue txout =
  let ledgerout =
        shelleyBasedEraConstraints sbe
        $ toShelleyTxOut sbe txout & L.coinTxOutL .~ updatedValue
...
maybeDummyTotalCollAndCollReturnOutput
  sbe
  TxBodyContent { txInsCollateral, txReturnCollateral, txTotalCollateral }
  cAddr = case txInsCollateral of
  TxInsCollateralNone -> ( TxReturnCollateralNone, TxTotalCollateralNone )
  TxInsCollateral {}  -> forShelleyBasedEraInEon
    sbe
    ( TxReturnCollateralNone, TxTotalCollateralNone )
    (\w
     -> let dummyRetCol =
              TxReturnCollateral
                ...
                (TxOut
                   ...
                   ReferenceScriptNone)
            dummyTotCol =
              TxTotalCollateral w (L.Coin (2 ^ (32 :: Integer) - 1))
  ...
  where
    ...
    f idx (PlutusScriptWitness langInEra version script datum redeemer _) =
      case Map.lookup idx exUnitsMap of
        Nothing      -> Left
          $ TxBodyErrorScriptWitnessIndexMissingFromExecUnitsMap idx exUnitsMap
        Just exunits -> Right
          $ PlutusScriptWitness langInEra version script datum redeemer exunits
...
mapTxScriptWitnesses
  f
  txbodycontent
  @TxBodyContent { txIns, txWithdrawals, txCertificates, txMintValue } = do
  ...
  where
    ...
    mapScriptWitnessesTxIns txins =
      ...
      in  traverse (\( txIn, eWitness ) -> case eWitness of
                      Left e    -> Left e
                      Right wit -> Right ( txIn, wit )) mappedScriptWitnesses
    ...
    mapScriptWitnessesWithdrawals (TxWithdrawals supported withdrawals) =
      let mappedWithdrawals
          ...
          mappedWithdrawals =
            ...
            | -- The withdrawals are indexed in the map order by stake credential
              ( ix, ( addr, withdrawal, BuildTxWith wit ) )
              <- zip [ 0 .. ] (orderStakeAddrs withdrawals)
            , let mappedWitness =
                    adjustWitness (f (ScriptWitnessIndexWithdrawal ix)) wit ]
      in  TxWithdrawals supported
          <$> traverse
            (\( sAddr, ll, eWitness ) -> case eWitness of
               Left e    -> Left e
      ...
      where
        ...
        adjustWitness g (ScriptWitness ctx witness') =
          ScriptWitness ctx <$> g witness'

    mapScriptWitnessesCertificates
      :: TxCertificates BuildTx era
      -> Either (TxBodyErrorAutoBalance era) (TxCertificates BuildTx era)
    mapScriptWitnessesCertificates
      TxCertificatesNone = Right TxCertificatesNone
    mapScriptWitnessesCertificates
      (TxCertificates supported certs (BuildTxWith witnesses)) =
      let mappedScriptWitnesses :: [ ( StakeCredential
                                     , Either
                                         (TxBodyErrorAutoBalance era)
          ...
          mappedScriptWitnesses =
            ...
            , ScriptWitness ctx witness <- maybeToList
      ...
      in  TxCertificates supported certs . BuildTxWith . Map.fromList
          <$> traverse
            (\( sCred, eScriptWitness ) -> case eScriptWitness of
               Left e    -> Left e
    ...
    mapScriptWitnessesMinting
      (TxMintValue supported value (BuildTxWith witnesses)) =
      ...
      in  do
            final <- traverse
              (\( pid, eScriptWitness ) -> case eScriptWitness of
                 Left e    -> Left e

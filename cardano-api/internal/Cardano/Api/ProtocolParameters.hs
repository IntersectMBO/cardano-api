...
instance IsShelleyBasedEra era => Show (LedgerProtocolParameters era) where
  show (LedgerProtocolParameters pp) = shelleyBasedEraConstraints (shelleyBasedEra @era) $ show pp
...
instance IsShelleyBasedEra era => Eq (LedgerProtocolParameters era) where
  LedgerProtocolParameters a == LedgerProtocolParameters b =
    shelleyBasedEraConstraints (shelleyBasedEra @era) $ a == b

{-# DEPRECATED
convertToLedgerProtocolParameters "Use the ledger's PParams (from module Cardano.Api.Ledger) type instead of ProtocolParameters. The type will be removed after Chang hard fork."
 #-}
...
convertToLedgerProtocolParameters sbe pp = LedgerProtocolParameters <$> toLedgerPParams sbe pp
  ...
  case eraPParamsUpdate of
    ShelleyEraBasedProtocolParametersUpdate c depAfterMary depAfterBabbage depAfterAlonzo ->
      let Ledger.PParamsUpdate common = createCommonPParamsUpdate c
          ...
          Ledger.PParamsUpdate depAfterAlonzo' =
            createDeprecatedAfterAlonzoPParams sbe depAfterAlonzo
    ...
    AlonzoEraBasedProtocolParametersUpdate c depAfterAlonzoA introInAlon depAfterBabbage ->
      let Ledger.PParamsUpdate common = createCommonPParamsUpdate c
          Ledger.PParamsUpdate withProtVer = createPreConwayProtocolVersionUpdate depAfterBabbage
          Ledger.PParamsUpdate preAl' =
            createPParamsUpdateIntroducedInAlonzo AlonzoEraOnwardsAlonzo introInAlon
          Ledger.PParamsUpdate depAfterAlonzoA' =
            createDeprecatedAfterAlonzoPParams sbe depAfterAlonzoA
    ...
    BabbageEraBasedProtocolParametersUpdate c introInAlonzo depAfterBabbage introInBabbage ->
      let Ledger.PParamsUpdate common = createCommonPParamsUpdate c
          Ledger.PParamsUpdate withProtVer = createPreConwayProtocolVersionUpdate depAfterBabbage
          Ledger.PParamsUpdate inAlonzoPParams =
            createPParamsUpdateIntroducedInAlonzo AlonzoEraOnwardsBabbage introInAlonzo
          Ledger.PParamsUpdate inBAb =
            createIntroducedInBabbagePParams BabbageEraOnwardsBabbage introInBabbage
    ...
    ConwayEraBasedProtocolParametersUpdate c introInAlonzo introInBabbage introInConway ->
      let Ledger.PParamsUpdate common = createCommonPParamsUpdate c
          Ledger.PParamsUpdate inAlonzoPParams =
            createPParamsUpdateIntroducedInAlonzo AlonzoEraOnwardsConway introInAlonzo
          Ledger.PParamsUpdate inBab =
            createIntroducedInBabbagePParams BabbageEraOnwardsConway introInBabbage
...
createDeprecatedAfterMaryPParams ::
  ...
  => MaxMaryEra (ShelleyLedgerEra era) =>
       ShelleyBasedEra era -> DeprecatedAfterMaryPParams era -> Ledger.PParamsUpdate
...
createDeprecatedAfterAlonzoPParams ::
  ...
  => MaxAlonzoEra (ShelleyLedgerEra era) =>
       ShelleyBasedEra era -> ShelleyToAlonzoPParams era -> Ledger.PParamsUpdate
                                                              (ShelleyLedgerEra era)
...
--
{-# DEPRECATED
ProtocolParameters "Use the ledger's PParams (from module Cardano.Api.Ledger) type instead of ProtocolParameters. The type will be removed after Chang hard fork."
 #-}
...
instance SerialiseAsRawBytes PraosNonce where
  serialiseToRawBytes (PraosNonce h) = Crypto.hashToBytes h
...
instance ToCBOR ExecutionUnitPrices where
  toCBOR ExecutionUnitPrices {priceExecutionSteps, priceExecutionMemory} =
    CBOR.encodeListLen 2 <> toCBOR priceExecutionSteps <> toCBOR priceExecutionMemory

instance FromCBOR ExecutionUnitPrices where
  fromCBOR = do
    CBOR.enforceSize "ExecutionUnitPrices" 2
    ExecutionUnitPrices <$> fromCBOR <*> fromCBOR
...
instance FromJSON ExecutionUnitPrices where
  parseJSON =
    withObject "ExecutionUnitPrices" $ \o ->
      ExecutionUnitPrices <$> o .: "priceSteps" <*> o .: "priceMemory"

toAlonzoPrices :: ExecutionUnitPrices -> Either ProtocolParametersConversionError Alonzo.Prices
toAlonzoPrices ExecutionUnitPrices {priceExecutionSteps, priceExecutionMemory} = do
...
fromAlonzoCostModels :: Plutus.CostModels -> Map AnyPlutusScriptVersion CostModel
fromAlonzoCostModels cModels =
  Map.fromList . map (bimap fromAlonzoScriptLanguage fromAlonzoCostModel)
    $ Map.toList
...
toAlonzoCostModel (CostModel m) l =
  first (PpceInvalidCostModel (CostModel m)) $ Alonzo.mkCostModel l m
...
instance ToCBOR UpdateProposal where
  toCBOR (UpdateProposal ppup epochno) = CBOR.encodeListLen 2 <> toCBOR ppup <> toCBOR epochno

instance FromCBOR UpdateProposal where
  fromCBOR = do
    CBOR.enforceSize "ProtocolParametersUpdate" 2
    UpdateProposal <$> fromCBOR <*> fromCBOR

makeShelleyUpdateProposal ::
     ProtocolParametersUpdate -> [Hash GenesisKey] -> EpochNo -> UpdateProposal
...
toShelleyCommonPParamsUpdate ProtocolParametersUpdate { protocolUpdateMaxBlockHeaderSize
...
toShelleyPParamsUpdate ::
  ...
  => ProtocolParametersUpdate
  -> Either ProtocolParametersConversionError (PParamsUpdate ledgerera)
toShelleyPParamsUpdate protocolParametersUpdate@ProtocolParametersUpdate { protocolUpdateProtocolVersion
...
toAlonzoCommonPParamsUpdate ::
  ...
  => ProtocolParametersUpdate
  -> Either ProtocolParametersConversionError (PParamsUpdate ledgerera)
toAlonzoCommonPParamsUpdate protocolParametersUpdate@ProtocolParametersUpdate { protocolUpdateCostModels
...
toAlonzoPParamsUpdate protocolParametersUpdate@ProtocolParametersUpdate { protocolUpdateProtocolVersion
                                                                        , protocolUpdateDecentralization
                                                                        } = do
...
toBabbageCommonPParamsUpdate protocolParametersUpdate@ProtocolParametersUpdate {protocolUpdateUTxOCostPerByte} = do
  ppuAlonzoCommon <- toAlonzoCommonPParamsUpdate protocolParametersUpdate
  let ppuBabbage =
        ppuAlonzoCommon
          & ppuCoinsPerUTxOByteL
              .~ fmap CoinPerByte (noInlineMaybeToStrictMaybe protocolUpdateUTxOCostPerByte)
  pure ppuBabbage
...
toBabbagePParamsUpdate protocolParametersUpdate@ProtocolParametersUpdate {protocolUpdateProtocolVersion} = do
  ppuBabbageCommon <- toBabbageCommonPParamsUpdate protocolParametersUpdate
  protVer <- mapM mkProtVer protocolUpdateProtocolVersion
  let ppuBabbage = ppuBabbageCommon & ppuProtocolVersionL .~ noInlineMaybeToStrictMaybe protVer
  pure ppuBabbage
...
boundRationalEither ::
     Ledger.BoundedRational b => String -> Rational -> Either ProtocolParametersConversionError b
...
fromLedgerUpdate ::
  ...
  => Ledger.EraCrypto ledgerera ~ StandardCrypto =>
       ShelleyBasedEra era -> Ledger.Update ledgerera -> UpdateProposal
...
fromLedgerProposedPPUpdates ::
     forall era ledgerera. ShelleyLedgerEra era ~ ledgerera
  => Ledger.EraCrypto ledgerera ~ StandardCrypto =>
       ShelleyBasedEra era -> Ledger.ProposedPPUpdates ledgerera -> Map
                                                                      (Hash GenesisKey)
                                                                      ProtocolParametersUpdate
...
fromLedgerPParamsUpdate ::
     ShelleyBasedEra era -> Ledger.PParamsUpdate (ShelleyLedgerEra era) -> ProtocolParametersUpdate
...
fromShelleyCommonPParamsUpdate ::
     EraPParams ledgerera => PParamsUpdate ledgerera -> ProtocolParametersUpdate
fromShelleyCommonPParamsUpdate ppu =
  ProtocolParametersUpdate
    ...
    , protocolUpdatePoolPledgeInfluence =
        Ledger.unboundRational <$> strictMaybeToMaybe (ppu ^. ppuA0L)
    , protocolUpdateMonetaryExpansion =
        Ledger.unboundRational <$> strictMaybeToMaybe (ppu ^. ppuRhoL)
...
fromShelleyPParamsUpdate ppu =
  (fromShelleyCommonPParamsUpdate ppu)
    ...
    , protocolUpdateDecentralization = Ledger.unboundRational <$> strictMaybeToMaybe (ppu ^. ppuDL)
    , protocolUpdateExtraPraosEntropy =
        fromLedgerNonce <$> strictMaybeToMaybe (ppu ^. ppuExtraEntropyL)
    , protocolUpdateMinUTxOValue = strictMaybeToMaybe (ppu ^. ppuMinUTxOValueL)
    }

fromAlonzoCommonPParamsUpdate ::
     AlonzoEraPParams ledgerera => PParamsUpdate ledgerera -> ProtocolParametersUpdate
fromAlonzoCommonPParamsUpdate ppu =
  (fromShelleyCommonPParamsUpdate ppu)
    { protocolUpdateCostModels =
        maybe mempty fromAlonzoCostModels (strictMaybeToMaybe (ppu ^. ppuCostModelsL))
    , protocolUpdatePrices = fromAlonzoPrices <$> strictMaybeToMaybe (ppu ^. ppuPricesL)
    , protocolUpdateMaxTxExUnits =
        fromAlonzoExUnits <$> strictMaybeToMaybe (ppu ^. ppuMaxTxExUnitsL)
    , protocolUpdateMaxBlockExUnits =
        fromAlonzoExUnits <$> strictMaybeToMaybe (ppu ^. ppuMaxBlockExUnitsL)
...
fromAlonzoPParamsUpdate ::
     Ledger.Crypto crypto => PParamsUpdate (Ledger.AlonzoEra crypto) -> ProtocolParametersUpdate
...
fromBabbageCommonPParamsUpdate ::
     BabbageEraPParams ledgerera => PParamsUpdate ledgerera -> ProtocolParametersUpdate
fromBabbageCommonPParamsUpdate ppu =
  (fromAlonzoCommonPParamsUpdate ppu)
    { protocolUpdateUTxOCostPerByte =
        unCoinPerByte <$> strictMaybeToMaybe (ppu ^. ppuCoinsPerUTxOByteL)
    }

fromBabbagePParamsUpdate ::
     Ledger.Crypto crypto => PParamsUpdate (Ledger.BabbageEra crypto) -> ProtocolParametersUpdate
...
toShelleyCommonPParams ProtocolParameters { protocolParamProtocolVersion
...
toShelleyPParams ::
  ...
  => ProtocolParameters
  -> Either ProtocolParametersConversionError (PParams ledgerera)
toShelleyPParams protocolParameters@ProtocolParameters { protocolParamDecentralization
...
toAlonzoCommonPParams ::
  ...
  => ProtocolParameters
  -> Either ProtocolParametersConversionError (PParams ledgerera)
toAlonzoCommonPParams protocolParameters@ProtocolParameters { protocolParamCostModels
                                                            , protocolParamPrices
  ...
  prices <- requireParam "protocolParamPrices" toAlonzoPrices protocolParamPrices
  maxTxExUnits <- requireParam "protocolParamMaxTxExUnits" Right protocolParamMaxTxExUnits
  maxBlockExUnits <- requireParam "protocolParamMaxBlockExUnits" Right protocolParamMaxBlockExUnits
  maxValueSize <- requireParam "protocolParamMaxBlockExUnits" Right protocolParamMaxValueSize
  collateralPercent <-
...
toAlonzoPParams protocolParameters@ProtocolParameters {protocolParamDecentralization} = do
  ...
  d <-
    requireParam
      ...
      (boundRationalEither "D")
      protocolParamDecentralization
  let ppAlonzo = ppAlonzoCommon & ppDL .~ d
  pure ppAlonzo
...
toBabbagePParams ::
  ...
  => ProtocolParameters
  -> Either ProtocolParametersConversionError (PParams ledgerera)
toBabbagePParams protocolParameters@ProtocolParameters {protocolParamUTxOCostPerByte} = do
  ppAlonzoCommon <- toAlonzoCommonPParams protocolParameters
  utxoCostPerByte <- requireParam "protocolParamUTxOCostPerByte" Right protocolParamUTxOCostPerByte
  let ppBabbage = ppAlonzoCommon & ppCoinsPerUTxOByteL .~ CoinPerByte utxoCostPerByte
  pure ppBabbage
...
--
{-# DEPRECATED
fromLedgerPParams "Use the ledger's PParams (from module Cardano.Api.Ledger) type instead of ProtocolParameters. The type will be removed after Chang hard fork."
 #-}

fromLedgerPParams ::
     ShelleyBasedEra era -> Ledger.PParams (ShelleyLedgerEra era) -> ProtocolParameters
...
{-# DEPRECATED
fromShelleyCommonPParams "Use the ledger's PParams (from module Cardano.Api.Ledger) type instead of ProtocolParameters. The type will be removed after Chang hard fork."
 #-}

fromShelleyCommonPParams :: EraPParams ledgerera => PParams ledgerera -> ProtocolParameters
fromShelleyCommonPParams pp =
  ProtocolParameters
    { protocolParamProtocolVersion =
        case pp ^. ppProtocolVersionL of
          Ledger.ProtVer a b -> (Ledger.getVersion a, b)
...
{-# DEPRECATED
fromShelleyPParams "Use the ledger's PParams (from module Cardano.Api.Ledger) type instead of ProtocolParameters. The type will be removed after Chang hard fork."
 #-}
...
{-# DEPRECATED
fromAlonzoPParams "Use the ledger's PParams (from module Cardano.Api.Ledger) type instead of ProtocolParameters. The type will be removed after Chang hard fork."
 #-}

fromAlonzoPParams :: AlonzoEraPParams ledgerera => PParams ledgerera -> ProtocolParameters
...
{-# DEPRECATED
fromExactlyAlonzoPParams "Use the ledger's PParams (from module Cardano.Api.Ledger) type instead of ProtocolParameters. The type will be removed after Chang hard fork."
 #-}
...
{-# DEPRECATED
fromBabbagePParams "Use the ledger's PParams (from module Cardano.Api.Ledger) type instead of ProtocolParameters. The type will be removed after Chang hard fork."
 #-}

fromBabbagePParams :: BabbageEraPParams ledgerera => PParams ledgerera -> ProtocolParameters
...
{-# DEPRECATED
fromConwayPParams "Use the ledger's PParams (from module Cardano.Api.Ledger) type instead of ProtocolParameters. The type will be removed after Chang hard fork."
 #-}

fromConwayPParams :: BabbageEraPParams ledgerera => PParams ledgerera -> ProtocolParameters
fromConwayPParams = fromBabbagePParams

{-# DEPRECATED
checkProtocolParameters "Use the ledger's PParams (from module Cardano.Api.Ledger) type instead of ProtocolParameters. The type will be removed after Chang hard fork. PParams natively enforce these checks."
 #-}
...
instance Error ProtocolParametersConversionError where
  prettyError =
    \case
      ...
      PpceMissingParameter name -> "Missing parameter: " <> pretty name

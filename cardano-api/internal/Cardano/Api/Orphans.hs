...
deriving anyclass instance ( ToJSON (L.PredicateFailure (L.EraRule "UTXOW" ledgerera))
...
deriving anyclass instance (ToJSON (L.PredicateFailure (L.EraRule "LEDGER" ledgerera))) =>
...
stakeSnapshotsToPair Consensus.StakeSnapshots { Consensus.ssStakeSnapshots
                                              , Consensus.ssMarkTotal
                                              , Consensus.ssSetTotal
                                              , Consensus.ssGoTotal
                                              } =
  [ "pools" .= ssStakeSnapshots
  , "total" .= object ["stakeMark" .= ssMarkTotal, "stakeSet" .= ssSetTotal, "stakeGo" .= ssGoTotal]
  ]
...
stakeSnapshotToPair Consensus.StakeSnapshot { Consensus.ssMarkPool
                                            , Consensus.ssSetPool
                                            , Consensus.ssGoPool
                                            } =
...
instance ToJSON (OneEraHash xs) where
  toJSON = toJSON . Text.decodeLatin1 . Base16.encode . SBS.fromShort . getOneEraHash
...
instance ToJSON (HeaderHash blk) => ToJSON (Tip blk) where
  ...
  toJSON (Tip slotNo headerHash blockNo) =
    Aeson.object ["slotNo" .= slotNo, "headerHash" .= headerHash, "blockNo" .= blockNo]

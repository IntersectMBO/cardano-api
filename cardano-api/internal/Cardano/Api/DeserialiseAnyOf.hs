...
module Cardano.Api.DeserialiseAnyOf
  ...
  , mapSomeAddressVerificationKey ) where
...
renderInputDecodeError = \case
  InputTextEnvelopeError textEnvErr -> prettyError textEnvErr
  InputBech32DecodeError decodeErr -> prettyError decodeErr
...
deserialiseInput :: forall a. AsType a
...
deserialiseInput asType acceptedFormats inputBs =
  ...
  where
      ...
      let textEnvRes :: Either TextEnvelopeError a
          textEnvRes =
            ...
            =<< first
              ...
              (Aeson.eitherDecodeStrict' inputBs)
      case textEnvRes of
        Right res -> DeserialiseInputSuccess res

        -- The input was valid a text envelope, but there was a type mismatch
        -- error.
        Left err@TextEnvelopeTypeError {} -> DeserialiseInputError
          (InputTextEnvelopeError err)
    ...
    deserialiseBech32 = case deserialiseFromBech32 asType inputText of
      Right res -> DeserialiseInputSuccess res
    ...
    deserialiseHex
      | isValidHex inputBs = case deserialiseFromRawBytesHex asType inputBs of
    ...
    isValidHex x =
      all (`elem` hexAlpha) (toLower <$> BSC.unpack x) && even (BSC.length x)
      where
        hexAlpha :: [ Char ]
...
-- envelope format.
deserialiseInputAnyOf :: forall b. [ FromSomeType SerialiseAsBech32 b ]
...
data SomeAddressVerificationKey
  ...
  | AGenesisDelegateExtendedVerificationKey (VerificationKey
...
renderSomeAddressVerificationKey = \case
  ...
  AGenesisDelegateExtendedVerificationKey vk ->
    ...
    let genDelegKey  =
        ...
        stakePoolKey =
          castVerificationKey genDelegKey :: VerificationKey StakePoolKey
...
deserialiseAnyVerificationKey bs =
  case deserialiseAnyVerificationKeyBech32 bs of
    Right vk -> Right vk
    Left _e  -> case deserialiseAnyVerificationKeyTextEnvelope bs of
      Right vk -> Right vk
...
deserialiseAnyVerificationKeyBech32 =
  ...
  where
    ...
    allBech32VerKey =
      ...
      , FromSomeType
          (AsVerificationKey AsDRepExtendedKey)
          ADRepExtendedVerificationKey
      , FromSomeType
          (AsVerificationKey AsCommitteeColdKey)
          ACommitteeColdVerificationKey
      , FromSomeType
          (AsVerificationKey AsCommitteeColdExtendedKey)
          ACommitteeColdExtendedVerificationKey
      , FromSomeType
          (AsVerificationKey AsCommitteeHotKey)
          ACommitteeHotVerificationKey
      , FromSomeType
          (AsVerificationKey AsCommitteeHotExtendedKey)

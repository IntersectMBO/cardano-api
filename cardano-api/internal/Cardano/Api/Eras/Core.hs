...
maybeEon ::
     () => Eon eon => IsCardanoEra era => Maybe (eon era) -- ^ The eon if supported in the era
...
monoidForEraInEonA ::
     () => Eon eon => Applicative f => Monoid a => CardanoEra era -> (eon era -> f a) -> f a
monoidForEraInEonA sbe = forEraInEon sbe (pure mempty)

-- ----------------------------------------------------------------------------
-- Era and eon existential types
data EraInEon eon where
  EraInEon :: (Typeable era, Typeable (eon era), Eon eon) => eon era -> EraInEon eon

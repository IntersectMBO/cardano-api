...
module Cardano.Api.Convenience.Query
  ...
  , renderQueryConvenienceError ) where
...
renderQueryConvenienceError
  (QueryEraMismatch (EraMismatch ledgerEraName' otherEraName')) =
  ...
  <> otherEraName'
  <> " era."
renderQueryConvenienceError ByronEraNotSupported = "Byron era not supported"
renderQueryConvenienceError
...
queryStateForBalancedTx era allTxIns certs = runExceptT $ do
  sbe <- requireShelleyBasedEra era & onNothing (left ByronEraNotSupported)
  ...
  eraHistory <- lift queryEraHistory & onLeft (left . QceUnsupportedNtcVersion)
  ...
  stakeDelegDeposits <- monoidForEraInEonA era $ \beo -> lift
    (queryStakeDelegDeposits beo stakeCreds)
    & onLeft (left . QceUnsupportedNtcVersion)
    & onLeft (left . QueryEraMismatch)

  drepDelegDeposits <- monoidForEraInEonA era $ \con -> Map.map drepDeposit
    <$> (lift (queryDRepState con drepCreds)
         & onLeft (left . QceUnsupportedNtcVersion)
         & onLeft (left . QueryEraMismatch))

  featuredTxTreasuryValueM <- caseShelleyToBabbageOrConwayEraOnwards
    (const $ pure Nothing)
    (\cOnwards -> do
       L.AccountState { L.asTreasury } <- lift (queryAccountState cOnwards)
         & onLeft (left . QceUnsupportedNtcVersion)
         & onLeft (left . QueryEraMismatch)

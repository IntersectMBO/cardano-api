...
instance Eq (Tx era) where
  (==) (ShelleyTx sbe txA) (ShelleyTx _ txB) =
...
instance Show (Tx era) where
  showsPrec p (ShelleyTx ShelleyBasedEraShelley tx) =
    showParen (p >= 11)
    $ showString "ShelleyTx ShelleyBasedEraShelley " . showsPrec 11 tx

  showsPrec p (ShelleyTx ShelleyBasedEraAllegra tx) =
    showParen (p >= 11)
    $ showString "ShelleyTx ShelleyBasedEraAllegra " . showsPrec 11 tx

  showsPrec p (ShelleyTx ShelleyBasedEraMary tx)    =
    showParen (p >= 11)
    $ showString "ShelleyTx ShelleyBasedEraMary " . showsPrec 11 tx

  showsPrec p (ShelleyTx ShelleyBasedEraAlonzo tx)  =
    showParen (p >= 11)
    $ showString "ShelleyTx ShelleyBasedEraAlonzo " . showsPrec 11 tx

  showsPrec p (ShelleyTx ShelleyBasedEraBabbage tx) =
    showParen (p >= 11)
    $ showString "ShelleyTx ShelleyBasedEraBabbage " . showsPrec 11 tx

  showsPrec p (ShelleyTx ShelleyBasedEraConway tx)  =
    showParen (p >= 11)
    $ showString "ShelleyTx ShelleyBasedEraConway " . showsPrec 11 tx
...
serialiseShelleyBasedTx
  :: forall ledgerera. L.EraTx ledgerera => L.Tx ledgerera -> ByteString
serialiseShelleyBasedTx = Plain.serialize'

deserialiseShelleyBasedTx :: forall ledgerera tx'. L.EraTx ledgerera
                          => (L.Tx ledgerera -> tx')
                          -> ByteString
...
instance IsShelleyBasedEra era => HasTextEnvelope (Tx era) where
  textEnvelopeType _ = case shelleyBasedEra :: ShelleyBasedEra era of
...
data TxBody era where
  ShelleyTxBody :: ShelleyBasedEra era
    ...
    -> TxScriptValidity
      era -- ^ Mark script as expected to pass or fail validation
    -> TxBody era
...
instance IsShelleyBasedEra era => SerialiseAsCBOR (TxBody era) where
  serialiseToCBOR body =
    serialiseToCBOR $ signShelleyTransaction shelleyBasedEra body mempty
...
data TxBodyScriptData era where
  ...
  TxBodyScriptData :: AlonzoEraOnwardsConstraints era
...
deriving instance L.EraCrypto (ShelleyLedgerEra era) ~ StandardCrypto
  => Show (TxBodyScriptData era)
...
txScriptValidityToScriptValidity TxScriptValidityNone                =
  ScriptValid
txScriptValidityToScriptValidity (TxScriptValidity _ scriptValidity) =
  scriptValidity

txScriptValidityToIsValid :: TxScriptValidity era -> L.IsValid
txScriptValidityToIsValid =
  scriptValidityToIsValid . txScriptValidityToScriptValidity
...
instance Show (KeyWitness era) where
  ...
  showsPrec p (ShelleyKeyWitness ShelleyBasedEraShelley tx) =
    showParen (p >= 11)
    $ showString "ShelleyKeyWitness ShelleyBasedEraShelley " . showsPrec 11 tx

  showsPrec p (ShelleyKeyWitness ShelleyBasedEraAllegra tx) =
    showParen (p >= 11)
    $ showString "ShelleyKeyWitness ShelleyBasedEraAllegra " . showsPrec 11 tx

  showsPrec p (ShelleyKeyWitness ShelleyBasedEraMary tx) =
    showParen (p >= 11)
    $ showString "ShelleyKeyWitness ShelleyBasedEraMary " . showsPrec 11 tx

  showsPrec p (ShelleyKeyWitness ShelleyBasedEraAlonzo tx) =
    showParen (p >= 11)
    $ showString "ShelleyKeyWitness ShelleyBasedEraAlonzo " . showsPrec 11 tx

  showsPrec p (ShelleyKeyWitness ShelleyBasedEraBabbage tx) =
    showParen (p >= 11)
    $ showString "ShelleyKeyWitness ShelleyBasedEraBabbage " . showsPrec 11 tx

  showsPrec p (ShelleyKeyWitness ShelleyBasedEraConway tx) =
    showParen (p >= 11)
    $ showString "ShelleyKeyWitness ShelleyBasedEraConway " . showsPrec 11 tx
...
instance IsCardanoEra era => SerialiseAsCBOR (KeyWitness era) where
  ...
  deserialiseFromCBOR _ bs = case cardanoEra :: CardanoEra era of
...
decodeShelleyBasedWitness :: forall era. L.Era (ShelleyLedgerEra era)
                          => ShelleyBasedEra era
                          -> ByteString
  ...
  where
    ...
    decode = do
      ...
      case t of
        ...
        _ -> CBOR.cborError
          $ CBOR.DecoderErrorUnknownTag "Shelley Witness" (fromIntegral t)

instance IsCardanoEra era => HasTextEnvelope (KeyWitness era) where
  textEnvelopeType _ = case cardanoEra :: CardanoEra era of
...
makeShelleySignature :: Crypto.SignableRepresentation tosign
                     => tosign
                     -> ShelleySigningKey
...
makeShelleySignature tosign (ShelleyExtendedSigningKey sk) =
  ...
  where
    fromXSignature
      :: Crypto.HD.XSignature -> Shelley.SignedDSIGN StandardCrypto b
    fromXSignature =
      Crypto.SignedDSIGN
...
getTxWitnesses (ShelleyTx sbe tx') =
  caseShelleyToMaryOrAlonzoEraOnwards
    ...
    sbe
  where
    getShelleyTxWitnesses :: forall ledgerera. L.EraTx ledgerera
    ...
    getShelleyTxWitnesses tx =
      map
        ...
        (Set.elems (tx ^. L.witsTxL . L.bootAddrTxWitsL))
      ++ map
        (ShelleyKeyWitness sbe)
        (Set.elems (tx ^. L.witsTxL . L.addrTxWitsL))
...
makeSignedTransaction
  witnesses
  (ShelleyTxBody sbe txbody txscripts txscriptdata txmetadata scriptValidity) =
  case sbe of
  ...
  where
    txCommon :: forall ledgerera. ShelleyLedgerEra era ~ ledgerera
             => L.EraCrypto ledgerera ~ StandardCrypto
             => L.EraTx ledgerera
             => L.Tx ledgerera
    txCommon =
      ...
      .~ (L.mkBasicTxWits
          ...
          & L.scriptTxWitsL
          .~ Map.fromList
            [ ( Ledger.hashScript @ledgerera sw, sw ) | sw <- txscripts ]
          & L.bootAddrTxWitsL
    ...
    alonzoSignedTransaction =
      ...
      where
        ( datums, redeemers ) = case txscriptdata of
...
makeByronKeyWitness nw txbody =
  ...
  in 
      ...
      case byronKeyFormat :: ByronKeyFormat key of
        ByronLegacyKeyFormat -> \(ByronSigningKeyLegacy sk)
...
makeShelleyBootstrapWitness
  ...
  -> SigningKey ByronKey
  -> KeyWitness era
makeShelleyBootstrapWitness sbe nwOrAddr txBody sk = case txBody of
...
makeShelleyBasedBootstrapWitness sbe nwOrAddr txbody (ByronSigningKey sk) =
  ...
  where
    ...
    txhash =
      ...
      $ Ledger.extractHash (Ledger.hashAnnotated txbody)
    ...
    eitherNwOrAddr = case nwOrAddr of
...
makeShelleyKeyWitness sbe = \case
  ShelleyTxBody _ txbody _ _ _ _ -> shelleyBasedEraConstraints sbe
    $ let txhash :: Shelley.Hash StandardCrypto Ledger.EraIndependentTxBody
          txhash =
            Ledger.extractHash @StandardCrypto (Ledger.hashAnnotated txbody)
      in 
           -- To allow sharing of the txhash computation across many signatures we
           -- define and share the txhash outside the lambda for the signing key:
          \wsk -> let sk        = toShelleySigningKey wsk

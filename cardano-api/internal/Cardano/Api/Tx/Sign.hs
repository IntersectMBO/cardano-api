...
module Cardano.Api.Tx.Sign
  ...
  , -- * Data family instances
    AsType(AsTx, AsByronTx, AsShelleyTx, AsMaryTx, AsAllegraTx,
       AsAlonzoTx, AsKeyWitness, AsByronWitness, AsShelleyWitness, AsTxId,
       AsTxBody, AsByronTxBody, AsShelleyTxBody, AsMaryTxBody)
  , TxBody(..)
  , TxScriptValidity(..)
...
--
data Tx era where
  ShelleyTx :: ShelleyBasedEra era -> L.Tx (ShelleyLedgerEra era) -> Tx era
...
instance Eq (Tx era) where
  (==) (ShelleyTx sbe txA) (ShelleyTx _ txB) = shelleyBasedEraConstraints sbe $ txA == txB

-- The GADT in the ShelleyTx case requires a custom instance
instance Show (Tx era) where
  showsPrec p (ShelleyTx ShelleyBasedEraShelley tx) =
    showParen (p >= 11) $ showString "ShelleyTx ShelleyBasedEraShelley " . showsPrec 11 tx
  showsPrec p (ShelleyTx ShelleyBasedEraAllegra tx) =
    showParen (p >= 11) $ showString "ShelleyTx ShelleyBasedEraAllegra " . showsPrec 11 tx
  showsPrec p (ShelleyTx ShelleyBasedEraMary tx) =
    showParen (p >= 11) $ showString "ShelleyTx ShelleyBasedEraMary " . showsPrec 11 tx
  showsPrec p (ShelleyTx ShelleyBasedEraAlonzo tx) =
    showParen (p >= 11) $ showString "ShelleyTx ShelleyBasedEraAlonzo " . showsPrec 11 tx
  showsPrec p (ShelleyTx ShelleyBasedEraBabbage tx) =
    showParen (p >= 11) $ showString "ShelleyTx ShelleyBasedEraBabbage " . showsPrec 11 tx
  showsPrec p (ShelleyTx ShelleyBasedEraConway tx) =
    showParen (p >= 11) $ showString "ShelleyTx ShelleyBasedEraConway " . showsPrec 11 tx
...
{-# DEPRECATED
AsByronTx "Use AsTx AsByronEra instead."
 #-}
...
{-# DEPRECATED
AsShelleyTx "Use AsTx AsShelleyEra instead."
 #-}
...
instance IsShelleyBasedEra era => SerialiseAsCBOR (Tx era) where
  serialiseToCBOR (ShelleyTx sbe tx) = shelleyBasedEraConstraints sbe $ serialiseShelleyBasedTx tx
...
serialiseShelleyBasedTx ::
     forall ledgerera. L.EraTx ledgerera
  => L.Tx ledgerera
  -> ByteString
serialiseShelleyBasedTx = Plain.serialize'

deserialiseShelleyBasedTx ::
     forall ledgerera tx'. L.EraTx ledgerera
  => (L.Tx ledgerera -> tx')
  -> ByteString
...
instance Eq (TxBody era) where
  (==) (ShelleyTxBody sbe txbodyA txscriptsA redeemersA txmetadataA scriptValidityA) (ShelleyTxBody _ txbodyB txscriptsB redeemersB txmetadataB scriptValidityB) =
    caseShelleyToMaryOrAlonzoEraOnwards
      (const $ txbodyA == txbodyB && txscriptsA == txscriptsB && txmetadataA == txmetadataB)
      (const
         $ txbodyA == txbodyB
...
instance Show (TxBody era) where
  showsPrec p (ShelleyTxBody ShelleyBasedEraShelley txbody txscripts redeemers txmetadata scriptValidity) =
    showParen
      (p >= 11)
  ...
  showsPrec p (ShelleyTxBody ShelleyBasedEraAllegra txbody txscripts redeemers txmetadata scriptValidity) =
    showParen
      (p >= 11)
      (showString "ShelleyTxBody ShelleyBasedEraAllegra "
         ...
         . showChar ' '
         . showsPrec 11 scriptValidity)
  showsPrec p (ShelleyTxBody ShelleyBasedEraMary txbody txscripts redeemers txmetadata scriptValidity) =
    showParen
      (p >= 11)
      (showString "ShelleyTxBody ShelleyBasedEraMary "
         ...
         . showChar ' '
         . showsPrec 11 scriptValidity)
  showsPrec p (ShelleyTxBody ShelleyBasedEraAlonzo txbody txscripts redeemers txmetadata scriptValidity) =
    showParen
      (p >= 11)
      (showString "ShelleyTxBody ShelleyBasedEraAlonzo "
         ...
         . showChar ' '
         . showsPrec 11 scriptValidity)
  showsPrec p (ShelleyTxBody ShelleyBasedEraBabbage txbody txscripts redeemers txmetadata scriptValidity) =
    showParen
      (p >= 11)
      (showString "ShelleyTxBody ShelleyBasedEraBabbage "
         ...
         . showChar ' '
         . showsPrec 11 scriptValidity)
  showsPrec p (ShelleyTxBody ShelleyBasedEraConway txbody txscripts redeemers txmetadata scriptValidity) =
    showParen
      (p >= 11)
...
data TxBodyScriptData era where
  ...
  TxBodyScriptData
    :: AlonzoEraOnwardsConstraints era=> AlonzoEraOnwards era
...
data KeyWitness era where
  ByronKeyWitness :: Byron.TxInWitness -> KeyWitness ByronEra
  ShelleyBootstrapWitness
    :: ShelleyBasedEra era -> Shelley.BootstrapWitness StandardCrypto -> KeyWitness era
  ShelleyKeyWitness
    :: ShelleyBasedEra era -> L.WitVKey Shelley.Witness StandardCrypto -> KeyWitness era

-- The GADT in the Shelley cases requires a custom instance
instance Eq (KeyWitness era) where
  (==) (ByronKeyWitness wA) (ByronKeyWitness wB) = wA == wB
  (==) (ShelleyBootstrapWitness _ wA) (ShelleyBootstrapWitness _ wB) = wA == wB
  (==) (ShelleyKeyWitness _ wA) (ShelleyKeyWitness _ wB) = wA == wB
  (==) _ _ = False
...
instance Show (KeyWitness era) where
  showsPrec p (ByronKeyWitness tx) =
    showParen (p >= 11) $ showString "ByronKeyWitness " . showsPrec 11 tx
  showsPrec p (ShelleyBootstrapWitness ShelleyBasedEraShelley tx) =
    showParen (p >= 11)
      $ showString "ShelleyBootstrapWitness ShelleyBasedEraShelley " . showsPrec 11 tx
  showsPrec p (ShelleyBootstrapWitness ShelleyBasedEraAllegra tx) =
    showParen (p >= 11)
      $ showString "ShelleyBootstrapWitness ShelleyBasedEraAllegra " . showsPrec 11 tx
  showsPrec p (ShelleyBootstrapWitness ShelleyBasedEraMary tx) =
    showParen (p >= 11)
      $ showString "ShelleyBootstrapWitness ShelleyBasedEraMary " . showsPrec 11 tx
  showsPrec p (ShelleyBootstrapWitness ShelleyBasedEraAlonzo tx) =
    showParen (p >= 11)
      $ showString "ShelleyBootstrapWitness ShelleyBasedEraAlonzo " . showsPrec 11 tx
  showsPrec p (ShelleyBootstrapWitness ShelleyBasedEraBabbage tx) =
    showParen (p >= 11)
      $ showString "ShelleyBootstrapWitness ShelleyBasedEraBabbage " . showsPrec 11 tx
  showsPrec p (ShelleyBootstrapWitness ShelleyBasedEraConway tx) =
    showParen (p >= 11)
      $ showString "ShelleyBootstrapWitness ShelleyBasedEraConway " . showsPrec 11 tx
  showsPrec p (ShelleyKeyWitness ShelleyBasedEraShelley tx) =
    showParen (p >= 11) $ showString "ShelleyKeyWitness ShelleyBasedEraShelley " . showsPrec 11 tx
  showsPrec p (ShelleyKeyWitness ShelleyBasedEraAllegra tx) =
    showParen (p >= 11) $ showString "ShelleyKeyWitness ShelleyBasedEraAllegra " . showsPrec 11 tx
  showsPrec p (ShelleyKeyWitness ShelleyBasedEraMary tx) =
    showParen (p >= 11) $ showString "ShelleyKeyWitness ShelleyBasedEraMary " . showsPrec 11 tx
  showsPrec p (ShelleyKeyWitness ShelleyBasedEraAlonzo tx) =
    showParen (p >= 11) $ showString "ShelleyKeyWitness ShelleyBasedEraAlonzo " . showsPrec 11 tx
  showsPrec p (ShelleyKeyWitness ShelleyBasedEraBabbage tx) =
    showParen (p >= 11) $ showString "ShelleyKeyWitness ShelleyBasedEraBabbage " . showsPrec 11 tx
  showsPrec p (ShelleyKeyWitness ShelleyBasedEraConway tx) =
    showParen (p >= 11) $ showString "ShelleyKeyWitness ShelleyBasedEraConway " . showsPrec 11 tx

instance HasTypeProxy era => HasTypeProxy (KeyWitness era) where
  data AsType (KeyWitness era) =
    AsKeyWitness (AsType era)
...
instance IsCardanoEra era => SerialiseAsCBOR (KeyWitness era) where
  serialiseToCBOR (ByronKeyWitness wit) = Plain.serialize' wit
  serialiseToCBOR (ShelleyKeyWitness sbe wit) =
    CBOR.serialize' (eraProtVerLow sbe) $ encodeShelleyBasedKeyWitness wit
  serialiseToCBOR (ShelleyBootstrapWitness sbe wit) =
    CBOR.serialize' (eraProtVerLow sbe) $ encodeShelleyBasedBootstrapWitness wit
  deserialiseFromCBOR _ bs =
    case cardanoEra :: CardanoEra era of
      ByronEra -> ByronKeyWitness <$> Plain.decodeFull' bs
...
encodeShelleyBasedKeyWitness wit = CBOR.encodeListLen 2 <> CBOR.encodeWord 0 <> CBOR.encCBOR wit
...
decodeShelleyBasedWitness ::
     forall era. L.Era (ShelleyLedgerEra era)
  => ShelleyBasedEra era
  -> ByteString
  -> Either CBOR.DecoderError (KeyWitness era)
decodeShelleyBasedWitness sbe =
  CBOR.decodeFullAnnotator (L.eraProtVerLow @(ShelleyLedgerEra era)) "Shelley Witness" decode
    . LBS.fromStrict
  where
    ...
    decode = do
      ...
      case t of
        ...
        _ -> CBOR.cborError $ CBOR.DecoderErrorUnknownTag "Shelley Witness" (fromIntegral t)
...
pattern Tx txbody ws <- (getTxBodyAndWitnesses -> (txbody, ws))
  where Tx txbody ws = makeSignedTransaction ws txbody
...
makeShelleySignature tosign (ShelleyNormalSigningKey sk) = Crypto.signedDSIGN () tosign sk
makeShelleySignature tosign (ShelleyExtendedSigningKey sk) =
  ...
  where
    fromXSignature :: Crypto.HD.XSignature -> Shelley.SignedDSIGN StandardCrypto b
    fromXSignature =
      Crypto.SignedDSIGN
...
makeSignedByronTransaction witnesses txbody =
  Byron.annotateTxAux
    $ Byron.mkTxAux (unAnnotated txbody) (Vector.fromList [w | ByronKeyWitness w <- witnesses])

-- order of signing keys must match txins
signByronTransaction ::
     NetworkId -> Annotated Byron.Tx ByteString -> [SigningKey ByronKey] -> Byron.ATxAux ByteString
signByronTransaction nw txbody sks = makeSignedByronTransaction witnesses txbody
  where
    witnesses = map (makeByronKeyWitness nw txbody) sks

-- signing keys is a set
signShelleyTransaction ::
     () => ShelleyBasedEra era -> TxBody era -> [ShelleyWitnessSigningKey] -> Tx era
signShelleyTransaction sbe txbody sks = makeSignedTransaction witnesses txbody
  where
...
getTxWitnessesByron (Byron.ATxAux {Byron.aTaWitness = witnesses}) =
  map ByronKeyWitness . Vector.toList . unAnnotated $ witnesses
...
getTxWitnesses (ShelleyTx sbe tx') =
  ...
  where
    getShelleyTxWitnesses ::
         forall ledgerera. L.EraTx ledgerera
      => L.EraCrypto ledgerera ~ StandardCrypto => L.Tx ledgerera -> [KeyWitness era]
    ...
    getAlonzoTxWitnesses ::
         forall ledgerera. L.EraCrypto ledgerera ~ StandardCrypto
      => L.EraTx ledgerera => L.Tx ledgerera -> [KeyWitness era]
    getAlonzoTxWitnesses = getShelleyTxWitnesses

makeSignedTransaction :: forall era. [KeyWitness era] -> TxBody era -> Tx era
makeSignedTransaction witnesses (ShelleyTxBody sbe txbody txscripts txscriptdata txmetadata scriptValidity) =
  case sbe of
  ...
  where
    txCommon ::
      ...
      => L.EraCrypto ledgerera ~ StandardCrypto => L.EraTx ledgerera => L.Tx ledgerera
    txCommon =
      L.mkBasicTx txbody
        & L.witsTxL
            .~ (L.mkBasicTxWits
                  ...
                  & L.scriptTxWitsL
                      .~ Map.fromList [(Ledger.hashScript @ledgerera sw, sw) | sw <- txscripts]
                  & L.bootAddrTxWitsL .~ Set.fromList [w | ShelleyBootstrapWitness _ w <- witnesses])
        & L.auxDataTxL .~ maybeToStrictMaybe txmetadata
    shelleySignedTransaction ::
         forall ledgerera. ShelleyLedgerEra era ~ ledgerera
      => Ledger.EraCrypto ledgerera ~ StandardCrypto => Ledger.EraTx ledgerera => Tx era
    shelleySignedTransaction = ShelleyTx sbe txCommon
    alonzoSignedTransaction ::
         forall ledgerera. ShelleyLedgerEra era ~ ledgerera
      => Ledger.EraCrypto ledgerera ~ StandardCrypto => L.AlonzoEraTx ledgerera => Tx era
    alonzoSignedTransaction =
      ShelleyTx
        sbe
        (txCommon
...
makeByronKeyWitness nw txbody =
  let txhash :: Byron.Hash Byron.Tx
   ...
   in case byronKeyFormat :: ByronKeyFormat key of
        ByronLegacyKeyFormat -> \(ByronSigningKeyLegacy sk) -> witness sk pm txhash
        ByronModernKeyFormat -> \(ByronSigningKey sk) -> witness sk pm txhash
  where
    witness ::
         Byron.SigningKey -> Byron.ProtocolMagicId -> Byron.Hash Byron.Tx -> KeyWitness ByronEra
...
makeShelleyBasedBootstrapWitness sbe nwOrAddr txbody (ByronSigningKey sk) =
  ...
  where
      ...
      Plain.serialize'
        $ Byron.mkAttributes
            Byron.AddrAttributes
              {Byron.aaVKDerivationPath = derivationPath, Byron.aaNetworkMagic = networkMagic}
    ...
    derivationPath = either (const Nothing) (Byron.aaVKDerivationPath . unAddrAttrs) eitherNwOrAddr
    networkMagic :: Byron.NetworkMagic
    networkMagic = either toByronNetworkMagic (Byron.aaNetworkMagic . unAddrAttrs) eitherNwOrAddr
          ...
          -- define and share the txhash outside the lambda for the signing key:
           in \wsk ->
                let sk = toShelleySigningKey wsk
                 ...
                 in ShelleyKeyWitness sbe $ L.WitVKey vk signature

toShelleySigningKey :: ShelleyWitnessSigningKey -> ShelleySigningKey
toShelleySigningKey key =
  case key of
  ...
  -- The cases for extended keys
    ...
    WitnessCommitteeColdExtendedKey (CommitteeColdExtendedSigningKey sk) ->
      ShelleyExtendedSigningKey sk
    WitnessCommitteeHotExtendedKey (CommitteeHotExtendedSigningKey sk) ->
      ShelleyExtendedSigningKey sk
    WitnessDRepExtendedKey (DRepExtendedSigningKey sk) -> ShelleyExtendedSigningKey sk

getShelleyKeyWitnessVerificationKey ::
     ShelleySigningKey -> Shelley.VKey Shelley.Witness StandardCrypto
getShelleyKeyWitnessVerificationKey (ShelleyNormalSigningKey sk) =
  (Shelley.asWitness :: Shelley.VKey Shelley.Payment StandardCrypto -> Shelley.VKey
                                                                         Shelley.Witness
                                                                         StandardCrypto)

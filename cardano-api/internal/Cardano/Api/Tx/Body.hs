...
module Cardano.Api.Tx.Body
  ...
  , TxOut(..)
  , TxOutValue(..)
  , TxOutDatum(TxOutDatumNone, TxOutDatumHash, TxOutDatumInTx,
           TxOutDatumInline)
  , toCtxUTxOTxOut
...
txOutToJsonValue era (TxOut addr val dat refScript) =
  case era of
    ...
    AlonzoEra ->
      object ["address" .= addr, "value" .= val, datHashJsonVal dat, "datum" .= datJsonVal dat]
    BabbageEra ->
      object
        [ "address" .= addr
        , "value" .= val
  ...
  where
    ...
    datHashJsonVal d =
      case d of
        TxOutDatumNone -> "datumhash" .= Aeson.Null
        TxOutDatumHash _ h -> "datumhash" .= toJSON h
        TxOutDatumInTx' _ h _ -> "datumhash" .= toJSON h
        TxOutDatumInline _ datum -> "inlineDatumhash" .= toJSON (hashScriptDataBytes datum)
    ...
    where
            ...
            (alonzoDat, babbageDat) ->
              fail
                $ "Parsed an Alonzo era datum and a Babbage era datum "
                    ...
                    <> show babbageDat
        finalRefScript <-
          case mBabRefScript of
            Nothing -> return r
            Just anyScript -> return $ ReferenceScript BabbageEraOnwardsBabbage anyScript
      ...
      reconcileConway top@(TxOut addr v dat r) babbageDatum mBabRefScript = do
        ...
        finalRefScript <-
          case mBabRefScript of
            Nothing -> return r
            Just anyScript -> return $ ReferenceScript BabbageEraOnwardsConway anyScript
        return $ TxOut addr v finalDat finalRefScript
      alonzoTxOutParser :: AlonzoEraOnwards era -> Aeson.Object -> Aeson.Parser (TxOut CtxTx era)
...
toAlonzoTxOutDatumHashUTxO :: TxOutDatum CtxUTxO era -> StrictMaybe (Plutus.DataHash StandardCrypto)
...
fromShelleyTxOut sbe ledgerTxOut =
  shelleyBasedEraConstraints sbe $ do
    ...
    case sbe of
      ShelleyBasedEraShelley -> TxOut addressInEra txOutValue TxOutDatumNone ReferenceScriptNone
      ShelleyBasedEraAllegra -> TxOut addressInEra txOutValue TxOutDatumNone ReferenceScriptNone
      ShelleyBasedEraMary -> TxOut addressInEra txOutValue TxOutDatumNone ReferenceScriptNone
      ShelleyBasedEraAlonzo -> TxOut addressInEra txOutValue TxOutDatumNone ReferenceScriptNone
      ShelleyBasedEraBabbage ->
        TxOut
          addressInEra
          txOutValue
          (fromBabbageTxOutDatum AlonzoEraOnwardsBabbage BabbageEraOnwardsBabbage datum)
          (case mRefScript of
             SNothing -> ReferenceScriptNone
             SJust refScript -> fromShelleyScriptToReferenceScript ShelleyBasedEraBabbage refScript)
        where datum = ledgerTxOut ^. L.datumTxOutL
              mRefScript = ledgerTxOut ^. L.referenceScriptTxOutL
      ShelleyBasedEraConway ->
        TxOut
          addressInEra
          txOutValue
          (fromBabbageTxOutDatum AlonzoEraOnwardsConway BabbageEraOnwardsConway datum)
          (case mRefScript of
             SNothing -> ReferenceScriptNone
             SJust refScript -> fromShelleyScriptToReferenceScript ShelleyBasedEraConway refScript)
        where datum = ledgerTxOut ^. L.datumTxOutL
              mRefScript = ledgerTxOut ^. L.referenceScriptTxOutL

toAlonzoTxOutDatumHash :: TxOutDatum ctx era -> StrictMaybe (Plutus.DataHash StandardCrypto)
...
fromBabbageTxOutDatum w _ (Plutus.DatumHash dh) = TxOutDatumHash w $ ScriptDataHash dh
...
data TxOutValue era where
  TxOutValueByron :: L.Coin -> TxOutValue era
  TxOutValueShelleyBased
    :: (Eq (Ledger.Value (ShelleyLedgerEra era)), Show (Ledger.Value (ShelleyLedgerEra era)))=> ShelleyBasedEra
         era
  ...
  parseJSON =
    ...
    where
      ...
      decodeAssetId (polid, Aeson.Object assetNameHm) = do
        ...
        pure . valueFromList $ map (first $ AssetId polId) aNameQuantity
      decodeAssetId ("lovelace", Aeson.Number sci) =
        case toBoundedInteger sci of
          Just (ll :: Word64) -> pure $ valueFromList [(AdaAssetId, Quantity $ toInteger ll)]
...
lovelaceToTxOutValue :: () => ShelleyBasedEra era -> L.Coin -> TxOutValue era
lovelaceToTxOutValue era ll =
  shelleyBasedEraConstraints era $ TxOutValueShelleyBased era $ A.mkAdaValue era ll
...
prettyRenderTxOut (TxOutInAnyEra _ (TxOut (AddressInEra _ addr) txOutVal _ _)) =
  serialiseAddress (toAddressAny addr) <> " + " <> renderValue (txOutValueToValue txOutVal)
...
data TxTotalCollateral era where
  ...
  TxTotalCollateral :: BabbageEraOnwards era -> L.Coin -> TxTotalCollateral era
...
data TxOutDatum ctx era where
  ...
  TxOutDatumHash :: AlonzoEraOnwards era -> Hash ScriptData -> TxOutDatum ctx era
     ...
     --
  TxOutDatumInTx'
    :: AlonzoEraOnwards era -> Hash ScriptData -> HashableScriptData -> TxOutDatum CtxTx era
...
pattern TxOutDatumInTx w d <- TxOutDatumInTx' w _ d
  where TxOutDatumInTx w d = TxOutDatumInTx' w (hashScriptDataBytes d) d
...
data TxProposalProcedures build era where
  ...
  TxProposalProcedures
    :: Ledger.EraPParams (ShelleyLedgerEra era)=> OSet (L.ProposalProcedure (ShelleyLedgerEra era))
    -> BuildTxWith
         build
...
setTxCurrentTreasuryValue ::
     Maybe (Featured ConwayEraOnwards era L.Coin)
  -> TxBodyContent build era
  -> TxBodyContent build era
...
getTxIdByron (Byron.ATxAux {Byron.aTaTx = txbody}) =
  TxId
    ...
    $ txbody
  where
    impossible = error "getTxIdByron: byron and shelley hash sizes do not match"

-- | Calculate the transaction identifier for a 'TxBody'.
--
getTxId :: TxBody era -> TxId
getTxId (ShelleyTxBody sbe tx _ _ _ _) = shelleyBasedEraConstraints sbe $ getTxIdShelley sbe tx
...
getTxIdShelley _ tx =
  TxId . Crypto.castHash . (\(Ledger.TxId txhash) -> SafeHash.extractHash txhash)
...
instance Error TxBodyError where
  prettyError =
    \case
      TxBodyEmptyTxIns -> "Transaction body has no inputs"
      TxBodyEmptyTxInsCollateral ->
        "Transaction body has no collateral inputs, but uses Plutus scripts"
      TxBodyEmptyTxOuts -> "Transaction body has no outputs"
      TxBodyOutputNegative (Quantity q) txout ->
        "Negative quantity (" <> pretty q <> ") in transaction output: " <> pretty txout
      TxBodyOutputOverflow (Quantity q) txout ->
        "Quantity too large (" <> pretty q <> " >= 2^64) in transaction output: " <> pretty txout
      ...
      TxBodyMetadataError errs ->
        mconcat
          ...
          , mconcat $ List.intersperse "; " [pretty k <> ": " <> prettyError err | (k, err) <- errs]
          ]
      TxBodyMintAdaError -> "Transaction cannot mint ada, only non-ada assets"
      TxBodyMissingProtocolParams ->
...
createTransactionBody ::
     () => ShelleyBasedEra era -> TxBodyContent BuildTx era -> Either TxBodyError (TxBody era)
createTransactionBody sbe bc =
  shelleyBasedEraConstraints sbe $ do
    ...
    setMint <-
      monoidForEraInEonA era $ \w -> pure $ Endo $ A.mintTxBodyL w .~ convMintValue apiMintValue
    setScriptIntegrityHash <-
      monoidForEraInEonA era $ \w ->
        pure
          $ Endo
          $ A.scriptIntegrityHashTxBodyL w
              .~ getScriptIntegrityHash apiProtocolParameters languages sData
    setCollateralInputs <-
      monoidForEraInEonA era $ \w -> pure $ Endo $ A.collateralInputsTxBodyL w .~ collTxIns
    setReqSignerHashes <-
      monoidForEraInEonA era $ \w ->
        pure $ Endo $ A.reqSignerHashesTxBodyL w .~ convExtraKeyWitnesses apiExtraKeyWitnesses
    setReferenceInputs <-
      monoidForEraInEonA era $ \w -> pure $ Endo $ A.referenceInputsTxBodyL w .~ refTxIns
    setCollateralReturn <-
      monoidForEraInEonA era $ \w -> pure $ Endo $ A.collateralReturnTxBodyL w .~ returnCollateral
    setTotalCollateral <-
      monoidForEraInEonA era $ \w -> pure $ Endo $ A.totalCollateralTxBodyL w .~ totalCollateral
    let ledgerTxBody =
...
getScriptIntegrityHash apiProtocolParameters languages =
  \case
...
validateTxBodyContent :: ShelleyBasedEra era -> TxBodyContent BuildTx era -> Either TxBodyError ()
validateTxBodyContent sbe txBodContent@TxBodyContent { txIns
                                                     , txInsCollateral
                                                     , txOuts
                                                     , txProtocolParams
                                                     , txMintValue
...
validateProtocolParameters txProtocolParams languages =
  case txProtocolParams of
    BuildTxWith Nothing
      | not (Set.null languages) -> Left TxBodyMissingProtocolParams
...
validateTxIns txIns = sequence_ [inputIndexDoesNotExceedMax txIns, txBodyContentHasTxIns txIns]

validateTxInsCollateral :: TxInsCollateral era -> Set Plutus.Language -> Either TxBodyError ()
validateTxInsCollateral txInsCollateral languages =
  case txInsCollateral of
    TxInsCollateralNone -> unless (Set.null languages) (Left TxBodyEmptyTxInsCollateral)
    TxInsCollateral _ collateralTxIns -> guardShelleyTxInsOverflow collateralTxIns
...
outputDoesNotExceedMax :: () => CardanoEra era -> Value -> TxOut CtxTx era -> Either TxBodyError ()
...
positiveOutput :: () => CardanoEra era -> Value -> TxOut CtxTx era -> Either TxBodyError ()
...
fromLedgerProposalProcedures sbe body =
  forShelleyBasedEraInEonMaybe sbe $ \w ->
    conwayEraOnwardsConstraints w
      $ Featured w
      $ TxProposalProcedures (body ^. L.proposalProceduresTxBodyL) ViewTx
...
fromLedgerVotingProcedures sbe body =
  forShelleyBasedEraInEonMaybe sbe $ \w ->
    conwayEraOnwardsConstraints w
      $ Featured w
      $ TxVotingProcedures (body ^. L.votingProceduresTxBodyL) ViewTx
...
fromLedgerTreasuryDonation sbe body =
  forShelleyBasedEraInEonMaybe sbe $ \w ->
    conwayEraOnwardsConstraints w $ Featured w (body ^. L.treasuryDonationTxBodyL)
...
fromLedgerTxIns sbe body =
  [(fromShelleyTxIn input, ViewTx) | input <- Set.toList (inputs_ sbe body)]
  where
    inputs_ ::
         ShelleyBasedEra era
      -> Ledger.TxBody (ShelleyLedgerEra era)
...
fromLedgerTxInsCollateral ::
     forall era. ShelleyBasedEra era -> Ledger.TxBody (ShelleyLedgerEra era) -> TxInsCollateral era
...
fromLedgerTxOuts sbe body scriptdata =
  case sbe of
    ...
    ShelleyBasedEraAllegra ->
      [fromShelleyTxOut sbe txout | txout <- toList (body ^. L.outputsTxBodyL)]
    ShelleyBasedEraMary -> [fromShelleyTxOut sbe txout | txout <- toList (body ^. L.outputsTxBodyL)]
    ShelleyBasedEraAlonzo ->
      [fromAlonzoTxOut AlonzoEraOnwardsAlonzo txout | txout <- toList (body ^. L.outputsTxBodyL)]
    ShelleyBasedEraBabbage ->
      [ fromBabbageTxOut BabbageEraOnwardsBabbage txdatums txouts
      | let txdatums = selectTxDatums scriptdata
      , txouts <- toList (body ^. L.outputsTxBodyL)
      ]
    ShelleyBasedEraConway ->
      [ fromBabbageTxOut BabbageEraOnwardsConway txdatums txouts
      | let txdatums = selectTxDatums scriptdata
      , txouts <- toList (body ^. L.outputsTxBodyL)
      ]

selectTxDatums ::
     TxBodyScriptData era -> Map (L.DataHash StandardCrypto) (L.Data (ShelleyLedgerEra era))
selectTxDatums TxBodyNoScriptData = Map.empty
selectTxDatums (TxBodyScriptData _ (Alonzo.TxDats' datums) _) = datums

fromAlonzoTxOut :: () => AlonzoEraOnwards era -> L.TxOut (ShelleyLedgerEra era) -> TxOut CtxTx era
...
fromLedgerTxTotalCollateral ::
     ShelleyBasedEra era -> Ledger.TxBody (ShelleyLedgerEra era) -> TxTotalCollateral era
...
fromLedgerTxReturnCollateral ::
     ShelleyBasedEra era -> Ledger.TxBody (ShelleyLedgerEra era) -> TxReturnCollateral CtxTx era
...
fromLedgerTxFee :: ShelleyBasedEra era -> Ledger.TxBody (ShelleyLedgerEra era) -> TxFee era
fromLedgerTxFee sbe body = shelleyBasedEraConstraints sbe $ TxFeeExplicit sbe $ body ^. L.feeTxBodyL

fromLedgerTxValidityLowerBound :: ShelleyBasedEra era -> A.TxBody era -> TxValidityLowerBound era
...
fromLedgerTxValidityUpperBound :: ShelleyBasedEra era -> A.TxBody era -> TxValidityUpperBound era
...
fromLedgerAuxiliaryData ShelleyBasedEraAllegra (L.AllegraTxAuxData ms ss) =
  (fromShelleyMetadata ms, fromShelleyBasedScript ShelleyBasedEraAllegra <$> toList ss)
fromLedgerAuxiliaryData ShelleyBasedEraMary (L.AllegraTxAuxData ms ss) =
  (fromShelleyMetadata ms, fromShelleyBasedScript ShelleyBasedEraMary <$> toList ss)
fromLedgerAuxiliaryData ShelleyBasedEraAlonzo txAuxData =
  ( fromShelleyMetadata (L.atadMetadata txAuxData)
  , fromShelleyBasedScript ShelleyBasedEraAlonzo <$> toList (L.getAlonzoTxAuxDataScripts txAuxData))
fromLedgerAuxiliaryData ShelleyBasedEraBabbage txAuxData =
  ( fromShelleyMetadata (L.atadMetadata txAuxData)
  , fromShelleyBasedScript ShelleyBasedEraBabbage <$> toList (L.getAlonzoTxAuxDataScripts txAuxData))
fromLedgerAuxiliaryData ShelleyBasedEraConway txAuxData =
  ( fromShelleyMetadata (L.atadMetadata txAuxData)
  , fromShelleyBasedScript ShelleyBasedEraConway <$> toList (L.getAlonzoTxAuxDataScripts txAuxData))
...
fromLedgerTxAuxiliaryData sbe (Just auxData) = (metadata, auxdata)
  where
    metadata =
      if null ms
        then TxMetadataNone
        else TxMetadataInEra sbe $ TxMetadata ms
    auxdata =
      caseShelleyEraOnlyOrAllegraEraOnwards
...
fromLedgerTxExtraKeyWitnesses ::
     ShelleyBasedEra era -> Ledger.TxBody (ShelleyLedgerEra era) -> TxExtraKeyWitnesses era
...
fromLedgerTxWithdrawals ::
     ShelleyBasedEra era -> Ledger.TxBody (ShelleyLedgerEra era) -> TxWithdrawals ViewTx era
...
fromLedgerTxCertificates ::
     ShelleyBasedEra era -> Ledger.TxBody (ShelleyLedgerEra era) -> TxCertificates ViewTx era
...
maybeFromLedgerTxUpdateProposal ::
     () => ShelleyBasedEra era -> Ledger.TxBody (ShelleyLedgerEra era) -> TxUpdateProposal era
...
fromLedgerTxMintValue ::
     ShelleyBasedEra era -> Ledger.TxBody (ShelleyLedgerEra era) -> TxMintValue ViewTx era
...
makeByronTransactionBody txIns txOuts = do
  ...
  outs'' <- traverse (\out -> toByronTxOut out ?! classifyRangeError out) outs'
  return
    $ CBOR.reAnnotate CBOR.byronProtVer
    $ Annotated (Byron.UnsafeTx ins'' outs'' (Byron.mkAttributes ())) ()
  where
    maxByronTxInIx :: Word
...
convExtraKeyWitnesses txExtraKeyWits =
  case txExtraKeyWits of
    ...
    TxExtraKeyWitnesses _ khs -> Set.fromList [Shelley.asWitness kh | PaymentKeyHash kh <- khs]
       ...
       let redeemers =
             Alonzo.Redeemers
               $ Map.fromList
                   ...
                   | (idx, AnyScriptWitness (PlutusScriptWitness _ _ _ _ d e)) <- scriptWitnesses
                   , Just i <- [fromScriptWitnessIndex w idx]
                   ]
           datums =
             Alonzo.TxDats
               $ Map.fromList [(L.hashData d', d') | d <- scriptdata, let d' = toAlonzoData d]
           scriptdata :: [HashableScriptData]
           scriptdata =
             [d | TxOut _ _ (TxOutDatumInTx _ d) _ <- txOuts]
               ++ [ d
                  | (_, AnyScriptWitness (PlutusScriptWitness _ _ _ (ScriptDatumForTxIn d) _ _)) <-
                      scriptWitnesses
                  ]
...
makeShelleyTransactionBody sbe@ShelleyBasedEraShelley txbodycontent@TxBodyContent { txIns
                                                                                  , txOuts
                                                                                  , txFee
                                                                                  , txValidityUpperBound
                                                                                  , txMetadata
  ...
  let txbody =
        (mkCommonTxBody sbe txIns txOuts txFee txWithdrawals txAuxData
          ...
          ^. A.txBodyL
  return $ ShelleyTxBody sbe txbody scripts_ TxBodyNoScriptData txAuxData TxScriptValidityNone
  where
    scripts_ :: [Ledger.Script StandardShelley]
    scripts_ =
      catMaybes
        [ toShelleyScript <$> scriptWitnessScript scriptwitness
        | (_, AnyScriptWitness scriptwitness) <- collectTxBodyScriptWitnesses sbe txbodycontent
        ]
    txAuxData :: Maybe (L.TxAuxData StandardShelley)
    txAuxData = toAuxiliaryData sbe txMetadata TxAuxScriptsNone
makeShelleyTransactionBody sbe@ShelleyBasedEraAllegra txbodycontent@TxBodyContent { txIns
                                                                                  , txOuts
                                                                                  , txFee
                                                                                  , txValidityLowerBound
                                                                                  , txValidityUpperBound
...
makeShelleyTransactionBody sbe@ShelleyBasedEraMary txbodycontent@TxBodyContent { txIns
                                                                               , txOuts
                                                                               , txFee
                                                                               , txValidityLowerBound
                                                                               , txValidityUpperBound
  ...
  let txbody =
        (mkCommonTxBody sbe txIns txOuts txFee txWithdrawals txAuxData
          ...
          ^. A.txBodyL
  return $ ShelleyTxBody sbe txbody scripts TxBodyNoScriptData txAuxData TxScriptValidityNone
  where
    ...
    txAuxData = toAuxiliaryData sbe txMetadata txAuxScripts
makeShelleyTransactionBody sbe@ShelleyBasedEraAlonzo txbodycontent@TxBodyContent { txIns
                                                                                 , txInsCollateral
                                                                                 , txOuts
                                                                                 , txFee
                                                                                 , txValidityLowerBound
  ...
  where
    ...
    datums = Alonzo.TxDats $ Map.fromList [(L.hashData d, d) | d <- toAlonzoData <$> scriptdata]
    scriptdata :: [HashableScriptData]
    scriptdata =
      [d | TxOut _ _ (TxOutDatumInTx _ d) _ <- txOuts]
        ++ [ d
    ...
    redeemers =
      Alonzo.Redeemers
        $ Map.fromList
            [ (i, (toAlonzoData d, toAlonzoExUnits e))
            | (idx, AnyScriptWitness (PlutusScriptWitness _ _ _ _ d e)) <- witnesses
    ...
    txAuxData = toAuxiliaryData sbe txMetadata txAuxScripts
makeShelleyTransactionBody sbe@ShelleyBasedEraBabbage txbodycontent@TxBodyContent { txIns
                                                                                  , txInsCollateral
                                                                                  , txInsReference
                                                                                  , txReturnCollateral
                                                                                  , txTotalCollateral
  ...
  return
    $ ShelleyTxBody
        ...
        txbody
        scripts
        (TxBodyScriptData AlonzoEraOnwardsBabbage datums redeemers)
        txAuxData
        txScriptValidity
  where
    ...
    datums =
      Alonzo.TxDats $ Map.fromList [(L.hashData d', d') | d <- scriptdata, let d' = toAlonzoData d]
    scriptdata :: [HashableScriptData]
    scriptdata =
      [d | TxOut _ _ (TxOutDatumInTx _ d) _ <- txOuts]
        ++ [ d
    ...
    languages :: Set Plutus.Language
    languages =
      Set.fromList $ catMaybes [getScriptLanguage sw | (_, AnyScriptWitness sw) <- witnesses]
    ...
    txAuxData = toAuxiliaryData sbe txMetadata txAuxScripts
makeShelleyTransactionBody sbe@ShelleyBasedEraConway txbodycontent@TxBodyContent { txIns
                                                                                 , txInsCollateral
                                                                                 , txInsReference
                                                                                 , txReturnCollateral
                                                                                 , txTotalCollateral
  ...
  let txbody =
        (mkCommonTxBody sbe txIns txOuts txFee txWithdrawals txAuxData
           ...
           & A.proposalProceduresTxBodyL cOn
               .~ convProposalProcedures
                    (maybe TxProposalProceduresNone unFeatured txProposalProcedures)
  ...
  return
    $ ShelleyTxBody
        ...
        txbody
        scripts
        (TxBodyScriptData AlonzoEraOnwardsConway datums redeemers)
        txAuxData
        txScriptValidity
  where
  ...
  toJSON =
    \case
      ScriptWitnessIndexTxIn n ->
        object ["kind" .= Aeson.String "ScriptWitnessIndexTxIn", "value" .= n]
      ScriptWitnessIndexMint n ->
        object ["kind" .= Aeson.String "ScriptWitnessIndexMint", "value" .= n]
      ScriptWitnessIndexCertificate n ->
        object ["kind" .= Aeson.String "ScriptWitnessIndexCertificate", "value" .= n]
      ScriptWitnessIndexWithdrawal n ->
        object ["kind" .= Aeson.String "ScriptWitnessIndexWithdrawal", "value" .= n]
      ScriptWitnessIndexVoting n ->
        object ["kind" .= Aeson.String "ScriptWitnessIndexVoting", "value" .= n]
      ScriptWitnessIndexProposing n ->
        object ["kind" .= Aeson.String "ScriptWitnessIndexProposing", "value" .= n]
...
toScriptIndex ::
     AlonzoEraOnwards era -> L.PlutusPurpose L.AsIx (ShelleyLedgerEra era) -> ScriptWitnessIndex
...
toScriptIndexAlonzo :: L.AlonzoPlutusPurpose L.AsIx (ShelleyLedgerEra era) -> ScriptWitnessIndex
...
toScriptIndexConway :: L.ConwayPlutusPurpose L.AsIx (ShelleyLedgerEra era) -> ScriptWitnessIndex
...
collectTxBodyScriptWitnesses ::
  ...
  -> TxBodyContent BuildTx era
  -> [(ScriptWitnessIndex, AnyScriptWitness era)]
collectTxBodyScriptWitnesses _ TxBodyContent { txIns
                                             , txWithdrawals
                                             , txCertificates
                                             , txMintValue
                                             , txVotingProcedures
  ...
  where
    ...
    scriptWitnessesTxIns txins =
      ...
      | (ix, (_, BuildTxWith (ScriptWitness _ witness))) <- zip [0 ..] (orderTxIns txins)
      ]
    scriptWitnessesWithdrawals ::
         TxWithdrawals BuildTx era -> [(ScriptWitnessIndex, AnyScriptWitness era)]
    ...
    scriptWitnessesCertificates ::
         TxCertificates BuildTx era -> [(ScriptWitnessIndex, AnyScriptWitness era)]
    ...
    scriptWitnessesVoting ::
         TxVotingProcedures BuildTx era -> [(ScriptWitnessIndex, AnyScriptWitness era)]
    ...
    scriptWitnessesProposing ::
         TxProposalProcedures BuildTx era -> [(ScriptWitnessIndex, AnyScriptWitness era)]
...
toShelleyWithdrawal withdrawals =
  L.Withdrawals
    $ Map.fromList [(toShelleyStakeAddr stakeAddr, value) | (stakeAddr, value, _) <- withdrawals]
...
fromShelleyWithdrawal (L.Withdrawals withdrawals) =
  [(fromShelleyStakeAddr stakeAddr, value, ViewTx) | (stakeAddr, value) <- Map.assocs withdrawals]
...
toAuxiliaryData sbe txMetadata txAuxScripts =
  let ms =
      ...
      ss =
        case txAuxScripts of
          TxAuxScriptsNone -> []
          TxAuxScripts _ ss' -> map toShelleyScript ss'
   in case sbe of
        ShelleyBasedEraShelley -> guard (not (Map.null ms)) $> L.ShelleyTxAuxData ms
        ShelleyBasedEraAllegra ->
          guard (not (Map.null ms && null ss)) $> L.AllegraTxAuxData ms (Seq.fromList ss)
        ShelleyBasedEraMary ->
          guard (not (Map.null ms && null ss)) $> L.AllegraTxAuxData ms (Seq.fromList ss)
        ShelleyBasedEraAlonzo -> guard (not (Map.null ms && null ss)) $> L.mkAlonzoTxAuxData ms ss
        ShelleyBasedEraBabbage -> guard (not (Map.null ms && null ss)) $> L.mkAlonzoTxAuxData ms ss
        ShelleyBasedEraConway -> guard (not (Map.null ms && null ss)) $> L.mkAlonzoTxAuxData ms ss
...
genesisUTxOPseudoTxIn nw (GenesisUTxOKeyHash kh)
 ...
 = fromShelleyTxIn (Shelley.initialFundsPseudoTxIn addr)
  where
    ...
    addr = L.Addr (toShelleyNetwork nw) (Shelley.KeyHashObj kh) Shelley.StakeRefNull
...
getReferenceInputsSizeForTxIds beo utxo txIds =
  babbageEraOnwardsConstraints beo $ do
...
scriptDataToInlineDatum d = L.Datum . L.dataToBinaryData $ toAlonzoData d

binaryDataToScriptData ::
     L.Era ledgerera => BabbageEraOnwards era -> L.BinaryData ledgerera -> HashableScriptData
binaryDataToScriptData BabbageEraOnwardsBabbage d = fromAlonzoData $ L.binaryDataToData d
binaryDataToScriptData BabbageEraOnwardsConway d = fromAlonzoData $ L.binaryDataToData d

...
instance (IsShelleyBasedEra era) => FromCBOR (Certificate era) where
  fromCBOR =
    shelleyBasedEraConstraints (shelleyBasedEra @era)
    $ fromShelleyCertificate shelleyBasedEra
...
instance (IsShelleyBasedEra era) => HasTextEnvelope (Certificate era) where
  ...
  textEnvelopeDefaultDescr cert = case cert of
    ShelleyRelatedCertificate
      _
      (Ledger.ShelleyTxCertDelegCert Ledger.ShelleyRegCert {})
      -> "Stake address registration"
    ShelleyRelatedCertificate
      _
      (Ledger.ShelleyTxCertDelegCert Ledger.ShelleyUnRegCert {})
      -> "Stake address deregistration"
    ShelleyRelatedCertificate
      _
      (Ledger.ShelleyTxCertDelegCert Ledger.ShelleyDelegCert {})
    ...
    ShelleyRelatedCertificate _ (Ledger.ShelleyTxCertPool Ledger.RegPool {})
      -> "Pool registration"
    ShelleyRelatedCertificate _ Ledger.ShelleyTxCertGenesisDeleg {}
    ...
    ConwayCertificate
      _
      (Ledger.ConwayTxCertGov Ledger.ConwayAuthCommitteeHotKey {})
      -> "Constitution committee member hot key registration"
    ConwayCertificate
      _
      (Ledger.ConwayTxCertGov Ledger.ConwayResignCommitteeColdKey {})
    ...
    ConwayCertificate _ (Ledger.ConwayTxCertPool Ledger.RegPool {})
      -> "Pool registration"
    ConwayCertificate _ (Ledger.ConwayTxCertPool Ledger.RetirePool {})
      -> "Pool retirement"
...
makeStakeAddressUnregistrationCertificate
  :: StakeAddressRequirements era -> Certificate era
makeStakeAddressUnregistrationCertificate req = case req of
  StakeAddrRegistrationConway cOnwards deposit scred
    -> conwayEraOnwardsConstraints cOnwards
    $ ConwayCertificate cOnwards
...
makeStakeAddressDelegationCertificate
  :: StakeDelegationRequirements era -> Certificate era
makeStakeAddressDelegationCertificate = \case
  ...
  StakeDelegationRequirementsPreConway atMostBabbage scred pid
    ...
    $ Ledger.mkDelegStakeTxCert
      (toShelleyStakeCredential scred)
      (unStakePoolKeyHash pid)
...
makeStakePoolRetirementCertificate
  :: () => StakePoolRetirementRequirements era -> Certificate era
makeStakePoolRetirementCertificate req = case req of
...
makeDrepRegistrationCertificate
  ...
  -> Certificate era
makeDrepRegistrationCertificate
  (DRepRegistrationRequirements conwayOnwards vcred deposit)
  anchor =
...
data CommitteeHotKeyAuthorizationRequirements era where
  CommitteeHotKeyAuthorizationRequirements :: ConwayEraOnwards era
    -> Ledger.Credential
      Ledger.ColdCommitteeRole
      (EraCrypto (ShelleyLedgerEra era))
    -> Ledger.Credential
      Ledger.HotCommitteeRole
      (EraCrypto (ShelleyLedgerEra era))
...
makeCommitteeColdkeyResignationCertificate
  (CommitteeColdkeyResignationRequirements cOnwards coldKeyCred anchor) =
  ConwayCertificate cOnwards . Ledger.ConwayTxCertGov
...
makeDrepUnregistrationCertificate
  (DRepUnregistrationRequirements conwayOnwards vcred deposit) =
  ConwayCertificate conwayOnwards . Ledger.ConwayTxCertGov
...
getTxCertWitness sbe ledgerCert =
  ...
  $ case Ledger.getVKeyWitnessTxCert ledgerCert of
    Just keyHash -> Just
      ...
      $ StakeKeyHash
      $ Ledger.coerceKeyRole keyHash
    Nothing      -> StakeCredentialByScript . fromShelleyScriptHash
...
selectStakeCredentialWitness = \case
  ShelleyRelatedCertificate stbEra shelleyCert -> shelleyToBabbageEraConstraints
    stbEra
    $ getTxCertWitness
      (shelleyToBabbageEraToShelleyBasedEra stbEra)
      shelleyCert
  ConwayCertificate cEra conwayCert            -> conwayEraOnwardsConstraints
    cEra
...
filterUnRegCreds = fmap fromShelleyStakeCredential . \case
  ShelleyRelatedCertificate stbEra shelleyCert
    -> shelleyToBabbageEraConstraints stbEra $ case shelleyCert of
  ...
  ConwayCertificate cEra conwayCert
    -> conwayEraOnwardsConstraints cEra $ case conwayCert of
      Ledger.RegTxCert _ -> Nothing
      Ledger.UnRegTxCert cred -> Just cred
      Ledger.RegPoolTxCert _ -> Nothing
      Ledger.RetirePoolTxCert _ _ -> Nothing
...
filterUnRegDRepCreds
  :: Certificate era
  -> Maybe (Ledger.Credential Ledger.DRepRole Ledger.StandardCrypto)
...
toShelleyCertificate
  :: () => Certificate era -> Ledger.TxCert (ShelleyLedgerEra era)
...
toShelleyPoolParams
  ...
  , Ledger.ppMetadata =
      toShelleyPoolMetadata <$> Ledger.maybeToStrictMaybe stakePoolMetadata
  }
  where
    ...
    toShelleyDnsName name =
      fromMaybe
        (error "toShelleyDnsName: invalid dns name. TODO: proper validation")
...
fromShelleyPoolParams
  :: Ledger.PoolParams StandardCrypto -> StakePoolParameters
fromShelleyPoolParams
  Ledger.PoolParams
  { Ledger.ppId
  , Ledger.ppVrf
  ...
  , stakePoolMetadata =
      fromShelleyPoolMetadata <$> Ledger.strictMaybeToMaybe ppMetadata
  }
  where
    ...
    fromShelleyPoolMetadata
      :: Ledger.PoolMetadata -> StakePoolMetadataReference

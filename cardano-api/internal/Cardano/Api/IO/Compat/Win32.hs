...
import           System.Directory
       ( emptyPermissions, readable, setPermissions )
...
writeSecretsImpl
  :: FilePath -> [ Char ] -> [ Char ] -> (a -> ByteString) -> [ a ] -> IO ()
writeSecretsImpl outDir prefix suffix secretOp xs =
  forM_ (zip xs [ 0 :: Int .. ]) $ \( secret, nr ) -> do
    let filename =
          outDir </> prefix <> "." <> printf "%03d" nr <> "." <> suffix
...
checkVrfFilePermissionsImpl (File vrfPrivKey) = do
  ...
  when
    (attribs `hasPermission` genericPermissions)

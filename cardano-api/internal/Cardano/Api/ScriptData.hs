...
instance SerialiseAsCBOR HashableScriptData where
  ...
  deserialiseFromCBOR AsHashableScriptData bs =
    HashableScriptData bs <$> CBOR.decodeFullDecoder "ScriptData" fromCBOR (LBS.fromStrict bs)
...
instance SerialiseAsRawBytes (Hash ScriptData) where
  serialiseToRawBytes (ScriptDataHash dh) = Crypto.hashToBytes (Ledger.extractHash dh)
...
instance SerialiseAsCBOR ScriptData where
  ...
  deserialiseFromCBOR AsScriptData bs =
    CBOR.decodeFullDecoder "ScriptData" fromCBOR (LBS.fromStrict bs) :: Either
      CBOR.DecoderError
      ScriptData
...
toAlonzoData =
  either (\e -> error $ "toAlonzoData: " <> show e) Plutus.binaryDataToData
...
fromAlonzoData d =
  HashableScriptData (Ledger.originalBytes d) (fromPlutusData $ Plutus.getPlutusData d)

toPlutusData :: ScriptData -> PlutusAPI.Data
toPlutusData (ScriptDataConstructor int xs) = PlutusAPI.Constr int [toPlutusData x | x <- xs]
toPlutusData (ScriptDataMap kvs) = PlutusAPI.Map [(toPlutusData k, toPlutusData v) | (k, v) <- kvs]
toPlutusData (ScriptDataList xs) = PlutusAPI.List [toPlutusData x | x <- xs]
...
fromPlutusData (PlutusAPI.Constr int xs) = ScriptDataConstructor int [fromPlutusData x | x <- xs]
fromPlutusData (PlutusAPI.Map kvs) =
  ScriptDataMap [(fromPlutusData k, fromPlutusData v) | (k, v) <- kvs]
fromPlutusData (PlutusAPI.List xs) = ScriptDataList [fromPlutusData x | x <- xs]
...
validateScriptData d =
  ...
  where
    ...
    collect (ScriptDataList xs) = foldMap collect xs
    collect (ScriptDataMap kvs) = foldMap (\(k, v) -> collect k <> collect v) kvs
    -- Constr tags do need to be less than a Word64
    collect (ScriptDataConstructor n xs) =
      [ScriptDataConstructorOutOfRange n | n > fromIntegral (maxBound :: Word64) || n < 0]
        <> foldMap collect xs
...
instance Error ScriptDataRangeError where
  prettyError (ScriptDataConstructorOutOfRange n) =
    mconcat
      ["Constructor numbers in script data value ", pretty n, " is outside the range 0 .. 2^64-1."]
...
--
scriptDataFromJson ::
     ScriptDataJsonSchema -> Aeson.Value -> Either ScriptDataJsonError HashableScriptData
...
--
scriptDataToJson :: ScriptDataJsonSchema -> HashableScriptData -> Aeson.Value
...
scriptDataToJsonNoSchema = conv . getScriptData
  where
    ...
    conv (ScriptDataBytes bs)
      ...
      , Text.all Char.isPrint s = Aeson.String s
      | otherwise = Aeson.String (bytesPrefix <> Text.decodeLatin1 (Base16.encode bs))
    conv (ScriptDataList vs) = Aeson.Array (Vector.fromList (map conv vs))
    conv (ScriptDataMap kvs) = Aeson.object [(convKey k, conv v) | (k, v) <- kvs]
    conv (ScriptDataConstructor n vs) =
      Aeson.Array
        $ Vector.fromList
            [Aeson.Number (fromInteger n), Aeson.Array (Vector.fromList (map conv vs))]
    ...
    convKey (ScriptDataBytes bs) =
      Aeson.fromText $ bytesPrefix <> Text.decodeLatin1 (Base16.encode bs)
    convKey v = Aeson.fromText . Text.Lazy.toStrict . Aeson.Text.encodeToLazyText . conv $ v

scriptDataFromJsonNoSchema :: Aeson.Value -> Either ScriptDataJsonSchemaError HashableScriptData
scriptDataFromJsonNoSchema = fmap (\sd -> HashableScriptData (serialiseToCBOR sd) sd) . conv
  where
    conv :: Aeson.Value -> Either ScriptDataJsonSchemaError ScriptData
    ...
    conv (Aeson.String s)
      ...
      , not (BSC.any (\c -> c >= 'A' && c <= 'F') bs') = Right (ScriptDataBytes bs)
      | otherwise = Right (ScriptDataBytes (Text.encodeUtf8 s))
    conv (Aeson.Array vs) = fmap ScriptDataList . traverse conv $ Vector.toList vs
...
instance Error ScriptDataJsonBytesError where
  ...
  prettyError (ScriptDataJsonBytesErrorInvalid e) = "ScriptData is invalid: " <> prettyError e
...
scriptDataToJsonDetailedSchema = conv . getScriptData
  where
    conv :: ScriptData -> Aeson.Value
    conv (ScriptDataNumber n) = singleFieldObject "int" . Aeson.Number $ fromInteger n
    conv (ScriptDataBytes bs) =
      singleFieldObject "bytes" . Aeson.String $ Text.decodeLatin1 (Base16.encode bs)
    conv (ScriptDataList vs) =
      singleFieldObject "list" . Aeson.Array $ Vector.fromList (map conv vs)
...
scriptDataFromJsonDetailedSchema ::
     Aeson.Value -> Either ScriptDataJsonSchemaError HashableScriptData
scriptDataFromJsonDetailedSchema = fmap (\sd -> HashableScriptData (serialiseToCBOR sd) sd) . conv
  where
    ...
    conv (Aeson.Object m) =
      case List.sort $ KeyMap.toList m of
        ...
        [("bytes", Aeson.String s)]
          | Right bs <- Base16.decode (Text.encodeUtf8 s) -> Right (ScriptDataBytes bs)
        [("list", Aeson.Array vs)] -> fmap ScriptDataList . traverse conv $ Vector.toList vs
        [("map", Aeson.Array kvs)] ->
          fmap ScriptDataMap . traverse convKeyValuePair $ Vector.toList kvs
        [("constructor", Aeson.Number d), ("fields", Aeson.Array vs)] ->
          case Scientific.floatingOrInteger d :: Either Double Integer of
            Left n -> Left (ScriptDataJsonNumberNotInteger n)
            Right n -> fmap (ScriptDataConstructor n) . traverse conv $ Vector.toList vs
        (key, v):_
    ...
    convKeyValuePair :: Aeson.Value -> Either ScriptDataJsonSchemaError (ScriptData, ScriptData)
...
instance Error ScriptDataJsonSchemaError where
  prettyError =
    \case
      ScriptDataJsonNullNotAllowed -> "JSON null values are not supported."
      ScriptDataJsonBoolNotAllowed -> "JSON bool values are not supported."

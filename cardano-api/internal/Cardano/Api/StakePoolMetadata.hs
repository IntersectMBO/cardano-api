...
instance SerialiseAsRawBytes (Hash StakePoolMetadata) where
  ...
  deserialiseFromRawBytes (AsHash AsStakePoolMetadata) bs =
    maybeToRight
      (SerialiseAsRawBytesError "Unable to deserialise Hash StakePoolMetadata")
...
instance FromJSON StakePoolMetadata where
  parseJSON = Aeson.withObject "StakePoolMetadata" $ \obj -> StakePoolMetadata
    <$> parseName obj
...
instance Error StakePoolMetadataValidationError where
  prettyError = \case
    StakePoolMetadataJsonDecodeError errStr              -> pretty errStr
    StakePoolMetadataInvalidLengthError maxLen actualLen -> mconcat

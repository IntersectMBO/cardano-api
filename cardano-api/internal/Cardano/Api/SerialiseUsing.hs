...
instance SerialiseAsRawBytes a => FromCBOR (UsingRawBytes a) where
  fromCBOR = do
    ...
    case deserialiseFromRawBytes ttoken bs of
      ...
      Left (SerialiseAsRawBytesError msg) ->
        fail ("cannot deserialise as a " ++ tname ++ ".  The error was: " ++ msg)
    where
...
instance SerialiseAsRawBytes a => FromJSON (UsingRawBytesHex a) where
  parseJSON =
    Aeson.withText tname $ either fail pure . deserialiseFromRawBytesBase16 . Text.encodeUtf8
    where
      tname = (tyConName . typeRepTyCon . typeRep) (Proxy :: Proxy a)

instance SerialiseAsRawBytes a => ToJSONKey (UsingRawBytesHex a) where
  toJSONKey = Aeson.toJSONKeyText $ \(UsingRawBytesHex x) -> serialiseToRawBytesHexText x

instance SerialiseAsRawBytes a => FromJSONKey (UsingRawBytesHex a) where
  fromJSONKey =
    Aeson.FromJSONKeyTextParser $ either fail pure . deserialiseFromRawBytesBase16 . Text.encodeUtf8
  ...
  case Base16.decode str of
    Right raw ->
      case deserialiseFromRawBytes ttoken raw of
        ...
        Left (SerialiseAsRawBytesError msg) ->
          Left ("cannot deserialise " ++ show str ++ ".  The error was: " <> msg)
...
instance SerialiseAsBech32 a => IsString (UsingBech32 a) where
  fromString str =
    case deserialiseFromBech32 ttoken (Text.pack str) of
      ...
      Left e -> error $ docToString $ "fromString: " <> pretty str <> ": " <> prettyError e
    where
      ttoken :: AsType a

...
fromConsensusGenTx ::
     () => Consensus.CardanoBlock L.StandardCrypto ~ block => Consensus.GenTx block -> TxInMode
fromConsensusGenTx =
  \case
    Consensus.HardForkGenTx (Consensus.OneEraGenTx (Z tx')) -> TxInByronSpecial tx'
...
toConsensusGenTx ::
     () => Consensus.CardanoBlock L.StandardCrypto ~ block => TxInMode -> Consensus.GenTx block
toConsensusGenTx (TxInByronSpecial gtx) = Consensus.HardForkGenTx (Consensus.OneEraGenTx (Z gtx))
...
toConsensusTxId ::
  ...
  => Consensus.CardanoBlock L.StandardCrypto ~ block =>
       TxIdInMode -> Consensus.TxId (Consensus.GenTx block)
...
data TxValidationError era where
  ByronTxValidationError :: Consensus.ApplyTxErr Consensus.ByronBlock -> TxValidationError era
  ...
  showsPrec p =
    \case
      ByronTxValidationError err ->
        showParen (p >= 11) (showString "ByronTxValidationError " . showsPrec 11 err)
...
instance ToJSON (TxValidationError era) where
  toJSON =
    \case
      ByronTxValidationError err ->
        Aeson.object ["kind" .= Aeson.String "ByronTxValidationError", "error" .= toJSON err]
      ShelleyTxValidationError sbe err ->
        shelleyBasedEraConstraints sbe
          $ Aeson.object
...
data TxValidationErrorInCardanoMode where
  TxValidationErrorInCardanoMode :: () => TxValidationError era -> TxValidationErrorInCardanoMode
  ...
  toJSON =
    \case
      TxValidationErrorInCardanoMode err ->
        Aeson.object
          ["tag" .= Aeson.String "TxValidationErrorInCardanoMode", "contents" .= toJSON err]
      TxValidationEraMismatch err ->
        Aeson.object
          ["tag" .= Aeson.String "TxValidationEraMismatch", "contents" .= toJSON (textShow err)]
...
fromConsensusApplyTxErr =
  \case
    Consensus.ApplyTxErrByron err -> TxValidationErrorInCardanoMode $ ByronTxValidationError err

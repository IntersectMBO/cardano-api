...
data TxInMode where
  ...
  --
  TxInByronSpecial :: Consensus.GenTx Consensus.ByronBlock -> TxInMode
  ...
  Consensus.HardForkGenTx (Consensus.OneEraGenTx (S (Z tx')))
    -> let Consensus.ShelleyTx _txid shelleyEraTx = tx'
       in  TxInMode
             ...
             (ShelleyTx ShelleyBasedEraShelley shelleyEraTx)
  Consensus.HardForkGenTx (Consensus.OneEraGenTx (S (S (Z tx'))))
    -> let Consensus.ShelleyTx _txid shelleyEraTx = tx'
       in  TxInMode
             ShelleyBasedEraAllegra
             (ShelleyTx ShelleyBasedEraAllegra shelleyEraTx)
  Consensus.HardForkGenTx (Consensus.OneEraGenTx (S (S (S (Z tx')))))
    -> let Consensus.ShelleyTx _txid shelleyEraTx = tx'
       in  TxInMode
             ShelleyBasedEraMary
             (ShelleyTx ShelleyBasedEraMary shelleyEraTx)
  Consensus.HardForkGenTx (Consensus.OneEraGenTx (S (S (S (S (Z tx'))))))
    -> let Consensus.ShelleyTx _txid shelleyEraTx = tx'
       in  TxInMode
             ShelleyBasedEraAlonzo
             (ShelleyTx ShelleyBasedEraAlonzo shelleyEraTx)
  Consensus.HardForkGenTx (Consensus.OneEraGenTx (S (S (S (S (S (Z tx')))))))
    -> let Consensus.ShelleyTx _txid shelleyEraTx = tx'
       in  TxInMode
             ShelleyBasedEraBabbage
             (ShelleyTx ShelleyBasedEraBabbage shelleyEraTx)
  Consensus.HardForkGenTx
    (Consensus.OneEraGenTx (S (S (S (S (S (S (Z tx'))))))))
    -> let Consensus.ShelleyTx _txid shelleyEraTx = tx'
       in  TxInMode
             ShelleyBasedEraConway
             (ShelleyTx ShelleyBasedEraConway shelleyEraTx)
...
toConsensusTxId (TxIdInMode ByronEra txid) =
  Consensus.HardForkGenTxId . Consensus.OneEraGenTxId . Z
...
toConsensusTxId (TxIdInMode ShelleyEra txid) =
  Consensus.HardForkGenTxId
    (Consensus.OneEraGenTxId (S (Z (Consensus.WrapGenTxId txid'))))
  where
...
toConsensusTxId (TxIdInMode AllegraEra txid) =
  Consensus.HardForkGenTxId
    (Consensus.OneEraGenTxId (S (S (Z (Consensus.WrapGenTxId txid')))))
  where
...
toConsensusTxId (TxIdInMode MaryEra txid) =
  Consensus.HardForkGenTxId
    (Consensus.OneEraGenTxId (S (S (S (Z (Consensus.WrapGenTxId txid'))))))
  where
...
toConsensusTxId (TxIdInMode BabbageEra txid) =
  Consensus.HardForkGenTxId
    (Consensus.OneEraGenTxId
       (S (S (S (S (S (Z (Consensus.WrapGenTxId txid'))))))))
  where
...
toConsensusTxId (TxIdInMode ConwayEra txid) =
  Consensus.HardForkGenTxId
    (Consensus.OneEraGenTxId
       (S (S (S (S (S (S (Z (Consensus.WrapGenTxId txid')))))))))
  where
...
data TxValidationError era where
  ByronTxValidationError
    :: Consensus.ApplyTxErr Consensus.ByronBlock -> TxValidationError era
...
instance Show (TxValidationError era) where
  showsPrec p = \case
    ByronTxValidationError err       -> showParen
      (p >= 11)
      (showString "ByronTxValidationError " . showsPrec 11 err)

    ShelleyTxValidationError sbe err -> shelleyBasedEraConstraints sbe
      $ showParen
        (p >= 11)
...
instance ToJSON (TxValidationError era) where
  toJSON = \case
    ByronTxValidationError err       -> Aeson.object
...
data TxValidationErrorInCardanoMode where
  TxValidationErrorInCardanoMode
    :: () => TxValidationError era -> TxValidationErrorInCardanoMode
...
instance ToJSON TxValidationErrorInCardanoMode where
  toJSON = \case
    TxValidationErrorInCardanoMode err -> Aeson.object
      [ "tag" .= Aeson.String "TxValidationErrorInCardanoMode"
      , "contents" .= toJSON err ]
    TxValidationEraMismatch err        -> Aeson.object
...
fromConsensusApplyTxErr = \case
  ...
  Consensus.ApplyTxErrShelley err  -> TxValidationErrorInCardanoMode
    $ ShelleyTxValidationError ShelleyBasedEraShelley err
  Consensus.ApplyTxErrAllegra err  -> TxValidationErrorInCardanoMode
    $ ShelleyTxValidationError ShelleyBasedEraAllegra err
  Consensus.ApplyTxErrMary err     -> TxValidationErrorInCardanoMode
    $ ShelleyTxValidationError ShelleyBasedEraMary err
  Consensus.ApplyTxErrAlonzo err   -> TxValidationErrorInCardanoMode
    $ ShelleyTxValidationError ShelleyBasedEraAlonzo err
  Consensus.ApplyTxErrBabbage err  -> TxValidationErrorInCardanoMode
    $ ShelleyTxValidationError ShelleyBasedEraBabbage err
  Consensus.ApplyTxErrConway err   -> TxValidationErrorInCardanoMode
    $ ShelleyTxValidationError ShelleyBasedEraConway err
  Consensus.ApplyTxErrWrongEra err -> TxValidationEraMismatch err

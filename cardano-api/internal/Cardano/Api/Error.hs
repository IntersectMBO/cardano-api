...
instance Error ErrorAsException where
  prettyError (ErrorAsException e) = prettyError e

instance Show ErrorAsException where
  show (ErrorAsException e) = docToString $ prettyError e

instance Exception ErrorAsException where
  displayException (ErrorAsException e) = docToString $ prettyError e
...
instance Error e => Error (FileError e) where
  prettyError = \case
    FileErrorTempFile targetPath tempPath h -> vsep
      [ "Error creating temporary file at: " <> pretty tempPath
      , "Target path: " <> pretty targetPath
      , "Handle: " <> pshow h ]
    FileDoesNotExistError path -> "Error file not found at: " <> pretty path
    FileIOError path ioe
      -> pretty path <> ": " <> pretty (displayException ioe)
    FileError path e -> pretty path <> ": " <> prettyError e

instance Error IOException where
  prettyError = pretty . show

fileIOExceptT
  :: MonadIO m => FilePath -> (FilePath -> IO s) -> ExceptT (FileError e) m s

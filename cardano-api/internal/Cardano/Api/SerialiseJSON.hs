...
instance Error JsonDecodeError where
  prettyError (JsonDecodeError err) = pretty err
...
deserialiseFromJSON _proxy = either (Left . JsonDecodeError) Right . Aeson.eitherDecodeStrict'
...
readFileJSON ttoken path =
  runExceptT $ do
    ...
    firstExceptT (FileError path) $ hoistEither $ deserialiseFromJSON ttoken content

writeFileJSON :: ToJSON a => FilePath -> a -> IO (Either (FileError ()) ())
writeFileJSON path x =
  runExceptT $ handleIOExceptT (FileIOError path) $ BS.writeFile path (serialiseToJSON x)

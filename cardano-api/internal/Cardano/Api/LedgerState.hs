...
module Cardano.Api.LedgerState
  ...
  , LedgerState(.., LedgerStateByron, LedgerStateShelley,
            LedgerStateAllegra, LedgerStateMary, LedgerStateAlonzo,
            LedgerStateBabbage, LedgerStateConway)
  , encodeLedgerState
  , decodeLedgerState
  , initialLedgerState
  , applyBlock
...
instance Error InitialLedgerStateError where
  prettyError =
    \case
      ILSEConfigFile err -> "Failed to read or parse the network config file:" <+> pretty err
      ILSEGenesisFile err ->
        "Failed to read or parse a genesis file linked from the network config file:"
          <+> prettyError err
...
instance Error LedgerStateError where
  prettyError =
    \case
      ...
      ApplyBlockHashMismatch err ->
        "Applying a block did not result in the expected block hash:" <+> pretty err
      ApplyBlockError hardForkLedgerError ->
        "Applying a block resulted in an error:" <+> pshow hardForkLedgerError
      InvalidRollback oldestSupported rollbackPoint ->
        "Encountered a rollback larger than the security parameter. Attempted to roll back to"
          <+> pshow rollbackPoint <> ", but oldest supported slot is"
...
applyBlock env oldState validationMode = applyBlock' env oldState validationMode . toConsensusBlock

pattern LedgerStateByron :: Ledger.LedgerState Byron.ByronBlock -> LedgerState
pattern LedgerStateByron st <- LedgerState (Consensus.LedgerStateByron st)

pattern LedgerStateShelley :: Ledger.LedgerState Shelley.StandardShelleyBlock -> LedgerState
pattern LedgerStateShelley st <- LedgerState (Consensus.LedgerStateShelley st)

pattern LedgerStateAllegra :: Ledger.LedgerState Shelley.StandardAllegraBlock -> LedgerState
pattern LedgerStateAllegra st <- LedgerState (Consensus.LedgerStateAllegra st)

pattern LedgerStateMary :: Ledger.LedgerState Shelley.StandardMaryBlock -> LedgerState
pattern LedgerStateMary st <- LedgerState (Consensus.LedgerStateMary st)

pattern LedgerStateAlonzo :: Ledger.LedgerState Shelley.StandardAlonzoBlock -> LedgerState
pattern LedgerStateAlonzo st <- LedgerState (Consensus.LedgerStateAlonzo st)

pattern LedgerStateBabbage :: Ledger.LedgerState Shelley.StandardBabbageBlock -> LedgerState
pattern LedgerStateBabbage st <- LedgerState (Consensus.LedgerStateBabbage st)

pattern LedgerStateConway :: Ledger.LedgerState Shelley.StandardConwayBlock -> LedgerState
pattern LedgerStateConway st <- LedgerState (Consensus.LedgerStateConway st)

{-# COMPLETE LedgerStateByron, LedgerStateShelley, LedgerStateAllegra, LedgerStateMary, LedgerStateAlonzo, LedgerStateBabbage, LedgerStateConway #-}
...
foldBlocks ::
  ...
  -- ^ The initial accumulator state.
                                    -> (Env -> LedgerState -> [LedgerEvent] -> BlockInMode -> a -> IO
                                                                                                     ( a
                                                                                                     , FoldStatus))
...
foldBlocks nodeConfigFilePath socketPath validationMode state0 accumulate =
  ...
  -- cardano-ledger-specs repo.
    let byronConfig =
        ...
        networkId' =
          case Cardano.Chain.Genesis.configReqNetMagic byronConfig of
  ...
  -- Connect to the node.
    ...
    liftIO $ connectToLocalNode connectInfo (protocols stateIORef errorIORef env ledgerState)
    ...
    chainSyncClient pipelineSize stateIORef errorIORef env ledgerState0 =
      ...
      where
        ...
        clientNextN n knownLedgerStates =
          CSP.ClientStNext
            { CSP.recvMsgRollForward =
                \blockInMode@(BlockInMode _ (Block (BlockHeader slotNo _ currBlockNo) _)) serverChainTip -> do
                  let newLedgerStateE =
                        applyBlock
                          env
                          (maybe
                  ...
                  case newLedgerStateE of
                    ...
                    Right newLedgerState -> do
                      let (knownLedgerStates', committedStates) =
                            pushLedgerState env knownLedgerStates slotNo newLedgerState blockInMode
                          ...
                          ledgerStateSingleFold (_, (ledgerState, ledgerEvents), At currBlock) = do
                            ...
                            (newState, foldStatus) <-
                              accumulate env ledgerState ledgerEvents currBlock accumulatorState
                      ...
                      case finalFoldStatus of
                          ...
                          let !ioRefErr =
                                DebugError . force
                                  $ unlines
                                      ...
                                      , "knownLedgerStates: "
                                          <> show (extractHistory knownLedgerStates)
                        ...
                        ContinueFold ->
                          return
                            $ clientIdle_RequestMoreN newClientTip newServerTip n knownLedgerStates'
            , CSP.recvMsgRollBackward =
                \chainPoint serverChainTip -> do
                  ...
                  return
                    (clientIdle_RequestMoreN newClientTip newServerTip n truncatedKnownLedgerStates)
            }
        ...
        clientIdle_DoneNwithMaybeError n errorMay =
          case n of
            Succ predN ->
...
chainSyncClientWithLedgerState ::
  ...
  -> CS.ChainSyncClient BlockInMode ChainPoint ChainTip m a
  ...
  where
        ...
        (\blkInMode tip ->
           CS.ChainSyncClient
             $ goClientStIdle (Left err)
                 <$> CS.runChainSyncClient (recvMsgRollForward (blkInMode, Left err) tip))
        (\point tip ->
           CS.ChainSyncClient
    ...
    goClientStIntersect history (CS.ClientStIntersect recvMsgIntersectFound recvMsgIntersectNotFound) =
      CS.ClientStIntersect
        ...
        (\tip ->
           CS.ChainSyncClient
             (goClientStIdle history <$> CS.runChainSyncClient (recvMsgIntersectNotFound tip)))
...
chainSyncClientPipelinedWithLedgerState ::
  ...
  -> CSP.ChainSyncClientPipelined
       ...
       m
       a
  -> CSP.ChainSyncClientPipelined BlockInMode ChainPoint ChainTip m a
chainSyncClientPipelinedWithLedgerState env ledgerState0 validationMode (CSP.ChainSyncClientPipelined clientTop) =
  ...
  where
    ...
    goClientPipelinedStIdle history n client =
      case client of
        CSP.SendMsgRequestNext a b -> CSP.SendMsgRequestNext a (goClientStNext history n b)
        CSP.SendMsgRequestNextPipelined m a ->
          CSP.SendMsgRequestNextPipelined m (goClientPipelinedStIdle history (Succ n) a)
        CSP.SendMsgFindIntersect ps a ->
          CSP.SendMsgFindIntersect ps (goClientPipelinedStIntersect history n a)
        CSP.CollectResponse a b ->
          case n of
            Succ nPrev ->
              CSP.CollectResponse
    ...
    goClientStNext (Left err) n (CSP.ClientStNext recvMsgRollForward recvMsgRollBackward) =
      CSP.ClientStNext
        (\blkInMode tip ->
           goClientPipelinedStIdle (Left err) n <$> recvMsgRollForward (blkInMode, Left err) tip)
        (\point tip -> goClientPipelinedStIdle (Left err) n <$> recvMsgRollBackward point tip)
           ...
           let newLedgerStateE =
                 case Seq.lookup 0 history of
                   ...
                   Just (_, Right (oldLedgerState, _), _) ->
                     applyBlock env oldLedgerState validationMode blkInMode
               (history', _) = pushLedgerState env history slotNo newLedgerStateE blkInMode
            in goClientPipelinedStIdle (Right history') n
...
pushLedgerState env hist sn st block =
  Seq.splitAt (fromIntegral $ envSecurityParam env + 1) ((sn, st, At block) Seq.:<| hist)
 ...
 genCfg =
  case genCfg of
    GenesisCardano _ bCfg _ transCfg
      | Cardano.Crypto.ProtocolMagic.unProtocolMagicId
          (Cardano.Chain.Genesis.configProtocolMagicId bCfg)
      ...
      | otherwise ->
        let topLevelConfig = Consensus.pInfoConfig $ fst $ mkProtocolInfoCardano genCfg
         in Right
              $ Env
      ...
      where shelleyGenesis = transCfg ^. Ledger.tcShelleyGenesisL
...
readByteString :: MonadError Text m => MonadIO m => FilePath -> Text -> m ByteString
readByteString fp cfgType =
  (liftEither <=< liftIO)
    $ catch (Right <$> BS.readFile fp)
    $ \(_ :: IOException) ->
        return
          $ Left
          $ mconcat ["Cannot read the ", cfgType, " configuration file at : ", Text.pack fp]

initLedgerStateVar :: GenesisConfig -> LedgerState
initLedgerStateVar genesisConfig =
  LedgerState {clsState = Ledger.ledgerState $ Consensus.pInfoInitLedger $ fst protocolInfo}
  where
...
getAnyNewEpochState ::
     ShelleyBasedEra era -> LedgerState -> Either LedgerStateError AnyNewEpochState
getAnyNewEpochState sbe (LedgerState ls) = AnyNewEpochState sbe <$> getNewEpochState sbe ls
...
getNewEpochState era x = do
  ...
  case era of
    ShelleyBasedEraShelley ->
      case x of
        Consensus.LedgerStateShelley current -> pure $ Shelley.shelleyLedgerState current
        _ -> Left err
    ShelleyBasedEraAllegra ->
      case x of
        Consensus.LedgerStateAllegra current -> pure $ Shelley.shelleyLedgerState current
        _ -> Left err
    ShelleyBasedEraMary ->
      case x of
        Consensus.LedgerStateMary current -> pure $ Shelley.shelleyLedgerState current
        _ -> Left err
    ShelleyBasedEraAlonzo ->
      case x of
        Consensus.LedgerStateAlonzo current -> pure $ Shelley.shelleyLedgerState current
        _ -> Left err
    ShelleyBasedEraBabbage ->
      case x of
        Consensus.LedgerStateBabbage current -> pure $ Shelley.shelleyLedgerState current
        _ -> Left err
    ShelleyBasedEraConway ->
      case x of
        Consensus.LedgerStateConway current -> pure $ Shelley.shelleyLedgerState current
        _ -> Left err

encodeLedgerState ::
     Consensus.CardanoCodecConfig Consensus.StandardCrypto -> LedgerState -> CBOR.Encoding
encodeLedgerState ccfg (LedgerState st) =
  encodeDisk @(Consensus.CardanoBlock Consensus.StandardCrypto) ccfg st

decodeLedgerState ::
     Consensus.CardanoCodecConfig Consensus.StandardCrypto -> forall s. CBOR.Decoder s LedgerState
...
toLedgerStateEvents lr = (ledgerState, ledgerEvents)
  where
    ...
    ledgerEvents =
      mapMaybe (toLedgerEvent . WrapLedgerEvent @(Consensus.CardanoBlock Consensus.StandardCrypto))
        $ Ledger.lrEvents lr
...
mkProtocolInfoCardano ::
     GenesisConfig
  -> ( Consensus.ProtocolInfo (Consensus.CardanoBlock Consensus.StandardCrypto)
    ...
    Consensus.CardanoProtocolParams
      { Consensus.paramsByron =
          Consensus.ProtocolParamsByron
            ...
            , Consensus.byronMaxTxCapacityOverrides =
...
readCardanoGenesisConfig ::
     MonadIOTransError GenesisConfigError t m => NodeConfig -> t m GenesisConfig
...
instance Error GenesisConfigError where
  prettyError =
    \case
      ...
      NEByronConfig fp ce ->
        mconcat ["Failed reading Byron genesis file ", pretty fp, ": ", pshow ce]
      NEShelleyConfig fp txt ->
        mconcat ["Failed reading Shelley genesis file ", pretty fp, ": ", pretty txt]
      NEAlonzoConfig fp txt ->
        mconcat ["Failed reading Alonzo genesis file ", pretty fp, ": ", pretty txt]
      NEConwayConfig fp txt ->
        mconcat ["Failed reading Conway genesis file ", pretty fp, ": ", pretty txt]
      NECardanoConfig err ->
        mconcat ["With Cardano protocol, Byron/Shelley config mismatch:\n", "   ", pretty err]

readByronGenesisConfig ::
     MonadIOTransError GenesisConfigError t m => NodeConfig -> t m Cardano.Chain.Genesis.Config
readByronGenesisConfig enc = do
  let file = unFile $ ncByronGenesisFile enc
  genHash <-
    liftEither . first NEError
...
readShelleyGenesisConfig ::
     MonadIOTransError GenesisConfigError t m => NodeConfig -> t m ShelleyConfig
...
readAlonzoGenesisConfig ::
     MonadIOTransError GenesisConfigError t m => NodeConfig -> t m AlonzoGenesis
...
readShelleyGenesis (File file) expectedGenesisHash = do
  ...
  genesis <-
    liftEither . first (ShelleyGenesisDecodeError file . Text.pack)
...
renderShelleyGenesisError sge =
  case sge of
    ShelleyGenesisReadError fp err ->
      mconcat ["There was an error reading the genesis file: ", Text.pack fp, " Error: ", err]
    ...
    ShelleyGenesisDecodeError fp err ->
      mconcat ["There was an error parsing the genesis file: ", Text.pack fp, " Error: ", err]
...
renderConwayGenesisError sge =
  case sge of
    ConwayGenesisFileMissing ->
      mconcat ["\"ConwayGenesisFile\" is missing from node configuration. "]
    ConwayGenesisHashMissing fp ->
      mconcat ["\"ConwayGenesisHash\" is missing from node configuration: ", Text.pack fp]
...
envSecurityParam env = k
  where
    Consensus.SecurityParam k = HFC.hardForkConsensusConfigK $ envConsensusConfig env
...
tickThenReapplyCheckHash cfg block lsb =
  if Consensus.blockPrevHash block == Ledger.ledgerTipHash lsb
    then Right . toLedgerStateEvents $ Ledger.tickThenReapplyLedgerResult cfg block lsb
    else Left
           ...
           $ mconcat
               ...
               , textShow
                   $ Slot.unSlotNo
                   $ Slot.fromWithOrigin (Slot.SlotNo 0) (Ledger.ledgerTipSlot lsb)
               , " hash "
               , renderByteArray $ unChainHash $ Ledger.ledgerTipHash lsb
...
instance Api.Error LeadershipError where
  prettyError =
    \case
      LeaderErrDecodeLedgerStateFailure -> "Failed to successfully decode ledger state"
      ...
      LeaderErrGenesisSlot -> "Leadership schedule currently cannot be calculated from genesis"
      LeaderErrStakePoolHasNoStake poolId -> "The stake pool: " <> pshow poolId <> " has no stake"
      LeaderErrDecodeProtocolEpochStateFailure decoderError ->
      ...
      LeaderErrStakeDistribUnstable curSlot stableAfterSlot stabWindow predictedLastSlot ->
        mconcat
          ...
          , "the following epoch's leadership schedule. Please wait until : "
              <> pshow stableAfterSlot
      ...
      LeaderErrCandidateNonceStillEvolving -> "Candidate nonce is still evolving"
...
nextEpochEligibleLeadershipSlots sbe sGen serCurrEpochState ptclState poolid (VrfSigningKey vrfSkey) pp eInfo (cTip, currentEpoch) =
  shelleyBasedEraConstraints sbe $ do
    (_, currentEpochLastSlot) <-
      first LeaderErrSlotRangeCalculationFailure $ Slot.epochInfoRange eInfo currentEpoch
    ...
    let stabilityWindowR :: Rational
        stabilityWindowR =
          fromIntegral (3 * sgSecurityParam sGen) / Ledger.unboundRational (sgActiveSlotsCoeff sGen)
    ...
    chainDepState <- first LeaderErrDecodeProtocolStateFailure $ decodeProtocolState ptclState
    ...
    when (evolvingNonce == candidateNonce) $ Left LeaderErrCandidateNonceStillEvolving
    ...
    CurrentEpochState cEstate <-
      first LeaderErrDecodeProtocolEpochStateFailure $ decodeCurrentEpochState sbe serCurrEpochState
    ...
    let slotRangeOfInterest :: Core.EraPParams ledgerera => Core.PParams ledgerera -> Set SlotNo
        slotRangeOfInterest pp' =
          Set.filter (not . Ledger.isOverlaySlot firstSlotOfEpoch (pp' ^. Core.ppDG))
  ...
  where
    globals =
      shelleyBasedEraConstraints sbe $ constructGlobals sGen eInfo $ pp ^. Core.ppProtocolVersionL
...
isLeadingSlotsTPraos ::
  ...
  => Crypto.Signable v Ledger.Seed =>
       Crypto.VRFAlgorithm v =>
         Crypto.ContextVRF v ~ () =>
           Set SlotNo -> PoolId -> Map
                                     (SL.KeyHash 'SL.StakePool Consensus.StandardCrypto)
                                     (SL.IndividualPoolStake Consensus.StandardCrypto) -> Consensus.Nonce -> Crypto.SignKeyVRF
                                                                                                               v -> Ledger.ActiveSlotCoeff -> Either
                                                                                                                                                LeadershipError
                                                                                                                                                (Set
                                                                                                                                                   SlotNo)
...
constructGlobals ::
     ShelleyGenesis Consensus.StandardCrypto -> EpochInfo (Either Text) -> Ledger.ProtVer -> Globals
...
data AnyNewEpochState where
  AnyNewEpochState
    :: ShelleyBasedEra era -> ShelleyAPI.NewEpochState (ShelleyLedgerEra era) -> AnyNewEpochState
...
foldEpochState ::
  ...
  -- ^ The final state
foldEpochState nodeConfigFilePath socketPath validationMode terminationEpoch initialResult checkCondition =
  handleExceptions $ do
  -- NOTE this was originally implemented with a non-pipelined client then
  -- changed to a pipelined client for a modest speedup:
  --  * Non-pipelined: 1h  0m  19s
  ...
  -- Connect to the node.
    ...
    modifyError FoldBlocksIOException
      $ liftIO
      $ connectToLocalNode connectInfo (protocols stateMv errorIORef env ledgerState)
    liftIO (readIORef errorIORef) >>= \case
      Just err -> throwError $ FoldBlocksApplyBlockError err
    ...
    chainSyncClient pipelineSize stateMv errorIORef' env ledgerState0 =
      ...
      where
        ...
        clientNextN n knownLedgerStates =
          CSP.ClientStNext
            { CSP.recvMsgRollForward =
                \blockInMode@(BlockInMode era (Block (BlockHeader slotNo _ currBlockNo) _)) serverChainTip -> do
                  let newLedgerStateE =
                        applyBlock
                          env
                          (maybe
                    ...
                    Just sbe ->
                      case newLedgerStateE of
                        ...
                        Right new@(newLedgerState, ledgerEvents) -> do
                          let (knownLedgerStates', _) =
                                pushLedgerState env knownLedgerStates slotNo new blockInMode
                              newClientTip = At currBlockNo
                              newServerTip = fromChainTip serverChainTip
                          case getNewEpochState sbe $ clsState newLedgerState of
                            ...
                            Right lState -> do
                              ...
                              -- code will deadlock!
                              condition <-
                                bracket (takeMVar stateMv) (tryPutMVar stateMv) $ \(_prevCondition, previousState) -> do
                                  updatedState@(!newCondition, !_) <-
                                    runStateT
                                      (checkCondition newEpochState slotNo currBlockNo)
                                      previousState
...
handleExceptions :: MonadIOTransError FoldBlocksError t m => ExceptT FoldBlocksError IO a -> t m a
handleExceptions = liftEither <=< liftIO . runExceptT . flip catches handlers
  where
    handlers =
      [Handler $ throwError . FoldBlocksIOException, Handler $ throwError . FoldBlocksMuxError]

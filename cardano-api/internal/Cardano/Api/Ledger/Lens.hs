...
mkBasicTxOut ::
  ...
  -> L.TxOut (ShelleyLedgerEra era)
mkBasicTxOut sbe addr value = shelleyBasedEraConstraints sbe $ L.mkBasicTxOut addr value

mkAdaValue :: ShelleyBasedEra era -> L.Coin -> L.Value (ShelleyLedgerEra era)
mkAdaValue sbe coin =
  caseShelleyToAllegraOrMaryEraOnwards (const coin) (const (L.MaryValue coin mempty)) sbe

adaAssetL :: ShelleyBasedEra era -> Lens' (L.Value (ShelleyLedgerEra era)) L.Coin
adaAssetL sbe =
  caseShelleyToAllegraOrMaryEraOnwards adaAssetShelleyToAllegraEraL adaAssetMaryEraOnwardsL sbe

adaAssetShelleyToAllegraEraL ::
     ShelleyToAllegraEra era -> Lens' (L.Value (ShelleyLedgerEra era)) L.Coin
adaAssetShelleyToAllegraEraL w = shelleyToAllegraEraConstraints w $ lens id const
...
multiAssetL w =
  maryEraOnwardsConstraints w
    $ lens (\(L.MaryValue _ ma) -> ma) (\(L.MaryValue c _) ma -> L.MaryValue c ma)

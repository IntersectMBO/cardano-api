...
module Cardano.Api.LedgerEvents.Rule.BBODY.UTXOW
  ...
  , handlePreAlonzoUTxOWEvent ) where
...
handlePreAlonzoUTxOWEvent
  ...
  => Shelley.ShelleyUtxowEvent ledgerera
  -> Maybe LedgerEvent
handlePreAlonzoUTxOWEvent (Shelley.UtxoEvent e) = case e of
  Shelley.TotalDeposits {} -> Nothing
  Shelley.UpdateEvent (Shelley.PpupNewEpoch _) -> Nothing
  Shelley.TxUTxODiff _ _ -> Nothing

handleAllegraMaryUTxOWEvent
  :: Event (Ledger.Core.EraRule "UTXO" ledgerera)
  ~ Allegra.AllegraUtxoEvent ledgerera
  => Event (Ledger.Core.EraRule "PPUP" ledgerera) ~ Shelley.PpupEvent ledgerera
  => Shelley.ShelleyUtxowEvent ledgerera
  -> Maybe LedgerEvent
handleAllegraMaryUTxOWEvent (Shelley.UtxoEvent e) = case e of

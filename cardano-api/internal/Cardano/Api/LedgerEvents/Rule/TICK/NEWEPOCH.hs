...
module Cardano.Api.LedgerEvents.Rule.TICK.NEWEPOCH
  ...
  , handleConwayNEWEPOCHEvents ) where
...
type LatestTickEventConstraints ledgerera =
  ...
  , Event (Core.EraRule "EPOCH" ledgerera)
    ~ Shelley.ShelleyEpochEvent ledgerera
  , Event (Core.EraRule "POOLREAP" ledgerera)
    ~ Shelley.ShelleyPoolreapEvent ledgerera )

handleLedgerTICKEvents :: EraCrypto ledgerera ~ StandardCrypto
                       => LatestTickEventConstraints ledgerera
                       => ShelleyTickEvent ledgerera
                       -> Maybe LedgerEvent
handleLedgerTICKEvents (TickNewEpochEvent newEpochEvent) =
  handleShelleyNEWEPOCHEvents newEpochEvent
handleLedgerTICKEvents (TickRupdEvent rewardUpdate)      =
  handleLedgerRUPDEvents rewardUpdate
...
handleShelleyNEWEPOCHEvents
  ...
  -> Maybe LedgerEvent
handleShelleyNEWEPOCHEvents shelleyNewEpochEvent = case shelleyNewEpochEvent of
  Shelley.DeltaRewardEvent {} -> Nothing
  Shelley.RestrainedRewards {} -> Nothing
  Shelley.TotalRewardEvent epochNo rewardsMap -> Just
    $ RewardsDistribution
      epochNo
      (Map.mapKeys fromShelleyStakeCredential rewardsMap)
...
handleEpochEvents
  ...
  -> Maybe LedgerEvent
handleEpochEvents (PoolReapEvent e) = case e of
  RetiredPools { refundPools, unclaimedPools, epochNo } -> Just . PoolReap
    $ PoolReapDetails
      epochNo
...
handleConwayNEWEPOCHEvents
  ...
  => Event (Core.EraRule "POOLREAP" ledgerera) ~ ShelleyPoolreapEvent ledgerera
  => Event (Core.EraRule "RUPD" ledgerera) ~ RupdEvent StandardCrypto
  => ConwayNewEpochEvent ledgerera
  -> Maybe LedgerEvent
handleConwayNEWEPOCHEvents conwayNewEpochEvent = case conwayNewEpochEvent of
  Conway.DeltaRewardEvent rewardUpdate -> case rewardUpdate of
    RupdEvent epochNum rewards -> Just
      $ IncrementalRewardsDistribution
        epochNum
        (Map.mapKeys fromShelleyStakeCredential rewards)
  Conway.RestrainedRewards {} -> Nothing
  Conway.TotalRewardEvent epochNo rewardsMap -> Just
    $ RewardsDistribution
      epochNo
      (Map.mapKeys fromShelleyStakeCredential rewardsMap)
  Conway.EpochEvent epochEvent -> case epochEvent of
    Conway.EpochBoundaryRatifyState ratifyState -> Just
      $ EpochBoundaryRatificationState (AnyRatificationState ratifyState)
    Conway.PoolReapEvent poolReap -> case poolReap of
      RetiredPools { refundPools, unclaimedPools, epochNo } -> Just . PoolReap
        $ PoolReapDetails
          epochNo
          (convertRetiredPoolsMap refundPools)

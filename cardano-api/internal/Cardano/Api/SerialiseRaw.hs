...
module Cardano.Api.SerialiseRaw
  ...
  , serialiseToRawBytesHexText ) where
...
data RawBytesHexError
  = RawBytesHexErrorBase16DecodeFail ByteString -- ^ original input
                                     String -- ^ error message
  | RawBytesHexErrorRawBytesDecodeFail ByteString                -- ^ original input
...
instance Error RawBytesHexError where
  prettyError = \case
    ...
    RawBytesHexErrorRawBytesDecodeFail
      input
      asType
      (SerialiseAsRawBytesError e) -> "Failed to deserialise "
      <> pretty (toText input)
      <> " as "
      <> pshow asType
      <> ". "

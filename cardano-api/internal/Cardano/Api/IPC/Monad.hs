...
newtype LocalStateQueryExpr block point query r m a =
  LocalStateQueryExpr
  ...
  deriving ( Functor, Applicative, Monad, MonadReader NodeToClientVersion
           , MonadIO )
...
setupLocalStateQueryExpr waitDone mPointVar' resultVar' ntcVersion f =
  ...
  { Net.Query.recvMsgAcquired = runContT
      (runReaderT (runLocalStateQueryExpr f) ntcVersion)
      $ \result -> do
...
getNtcVersion
  :: LocalStateQueryExpr block point QueryInMode r IO NodeToClientVersion
...
queryExpr q = do
  ...
  if ntcVersion >= minNtcVersion
    then fmap Right . LocalStateQueryExpr . ReaderT $ \_ -> ContT $ \f -> pure

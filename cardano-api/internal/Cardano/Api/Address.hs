...
module Cardano.Api.Address
  ...
  , -- * Data family instances
    AsType(AsByronAddr, AsShelleyAddr, AsByronAddress,
       AsShelleyAddress, AsAddress, AsAddressAny, AsAddressInEra,
       AsStakeAddress)
  , -- * Helpers
    isKeyAddress
  ) where
...
data Address addrtype
     ...
     --
 where
  ByronAddress :: Byron.Address -> Address ByronAddr
...
instance SerialiseAsRawBytes (Address ByronAddr) where
  serialiseToRawBytes (ByronAddress addr) =
    Shelley.serialiseAddr . Shelley.AddrBootstrap . Shelley.BootstrapAddress $ addr
  deserialiseFromRawBytes (AsAddress AsByronAddr) bs =
    case Shelley.decodeAddr bs :: Maybe (Shelley.Addr StandardCrypto) of
      ...
      Just Shelley.Addr {} ->
        Left (SerialiseAsRawBytesError "Unable to deserialise Address ByronAddr")
      Just (Shelley.AddrBootstrap (Shelley.BootstrapAddress addr)) -> Right (ByronAddress addr)

instance SerialiseAsRawBytes (Address ShelleyAddr) where
  serialiseToRawBytes (ShelleyAddress nw pc scr) = Shelley.serialiseAddr (Shelley.Addr nw pc scr)
...
instance SerialiseAddress (Address ByronAddr) where
  serialiseAddress addr@ByronAddress {} =
    Text.decodeLatin1 . Base58.encodeBase58 Base58.bitcoinAlphabet . serialiseToRawBytes $ addr
...
instance SerialiseAddress (Address ShelleyAddr) where
  serialiseAddress addr@ShelleyAddress {} = serialiseToBech32 addr
  deserialiseAddress (AsAddress AsShelleyAddr) t =
    either (const Nothing) Just $ deserialiseFromBech32 (AsAddress AsShelleyAddr) t
...
makeByronAddress nw (ByronVerificationKey vk) =
  ByronAddress $ Byron.makeVerKeyAddress (toByronNetworkMagic nw) vk

makeShelleyAddress :: NetworkId -> PaymentCredential -> StakeAddressReference -> Address ShelleyAddr
makeShelleyAddress nw pc scr =
  ShelleyAddress (toShelleyNetwork nw) (toShelleyPaymentCredential pc) (toShelleyStakeReference scr)
...
instance SerialiseAsRawBytes AddressAny where
  ...
  deserialiseFromRawBytes AsAddressAny bs =
    case Shelley.decodeAddr bs of
      ...
      Just (Shelley.Addr nw pc scr) -> Right (AddressShelley (ShelleyAddress nw pc scr))
...
fromShelleyAddrToAny (Shelley.Addr nw pc scr) = AddressShelley $ ShelleyAddress nw pc scr
...
lexPlausibleAddressString = Text.pack <$> Parsec.many1 (Parsec.satisfy isPlausibleAddressChar)
  where
    -- Covers both base58 and bech32 (with constrained prefixes)
    isPlausibleAddressChar c = isAsciiLower c || isAsciiUpper c || isDigit c || c == '_'

instance Eq (AddressInEra era) where
  (==) (AddressInEra ByronAddressInAnyEra addr1) (AddressInEra ByronAddressInAnyEra addr2) =
    addr1 == addr2
  (==) (AddressInEra ShelleyAddressInEra {} addr1) (AddressInEra ShelleyAddressInEra {} addr2) =
    addr1 == addr2
  (==) (AddressInEra ByronAddressInAnyEra _) (AddressInEra ShelleyAddressInEra {} _) = False
  (==) (AddressInEra ShelleyAddressInEra {} _) (AddressInEra ByronAddressInAnyEra _) = False

instance Ord (AddressInEra era) where
  compare (AddressInEra ByronAddressInAnyEra addr1) (AddressInEra ByronAddressInAnyEra addr2) =
    compare addr1 addr2
  compare (AddressInEra ShelleyAddressInEra {} addr1) (AddressInEra ShelleyAddressInEra {} addr2) =
    compare addr1 addr2
  compare (AddressInEra ByronAddressInAnyEra _) (AddressInEra ShelleyAddressInEra {} _) = LT
  compare (AddressInEra ShelleyAddressInEra {} _) (AddressInEra ByronAddressInAnyEra _) = GT
...
instance IsCardanoEra era => SerialiseAsRawBytes (AddressInEra era) where
  serialiseToRawBytes (AddressInEra ByronAddressInAnyEra addr) = serialiseToRawBytes addr
  serialiseToRawBytes (AddressInEra ShelleyAddressInEra {} addr) = serialiseToRawBytes addr
...
instance IsCardanoEra era => SerialiseAddress (AddressInEra era) where
  serialiseAddress (AddressInEra ByronAddressInAnyEra addr) = serialiseAddress addr
  serialiseAddress (AddressInEra ShelleyAddressInEra {} addr) = serialiseAddress addr
...
shelleyAddressInEra sbe = AddressInEra (ShelleyAddressInEra sbe)

anyAddressInShelleyBasedEra :: () => ShelleyBasedEra era -> AddressAny -> AddressInEra era
anyAddressInShelleyBasedEra sbe =
  \case
...
anyAddressInEra era =
  \case
    AddressByron addr -> Right (AddressInEra ByronAddressInAnyEra addr)
    AddressShelley addr ->
      forEraInEon
        era
...
makeByronAddressInEra nw vk = byronAddressInEra (makeByronAddress nw vk)
...
makeShelleyAddressInEra ::
  ...
  -> StakeAddressReference
  -> AddressInEra era
makeShelleyAddressInEra sbe nw pc scr = shelleyAddressInEra sbe (makeShelleyAddress nw pc scr)

-- ----------------------------------------------------------------------------
-- Stake addresses
--
data StakeAddress where
  StakeAddress :: Shelley.Network -> Shelley.StakeCredential StandardCrypto -> StakeAddress
...
instance ToJSON StakeCredential where
  toJSON =
    Aeson.object . \case
      StakeCredentialByKey keyHash -> ["stakingKeyHash" .= serialiseToRawBytesHexText keyHash]
      StakeCredentialByScript scriptHash ->
...
instance SerialiseAddress StakeAddress where
  serialiseAddress addr@StakeAddress {} = serialiseToBech32 addr
  deserialiseAddress AsStakeAddress t =
    either (const Nothing) Just $ deserialiseFromBech32 AsStakeAddress t
...
instance FromJSON StakeAddress where
  parseJSON =
    withText "StakeAddress" $ \str ->
      case deserialiseAddress AsStakeAddress str of
        Nothing -> fail $ "Error while deserialising StakeAddress: " <> Text.unpack str
        Just sAddr -> pure sAddr

makeStakeAddress :: NetworkId -> StakeCredential -> StakeAddress
makeStakeAddress nw sc = StakeAddress (toShelleyNetwork nw) (toShelleyStakeCredential sc)
...
toShelleyAddr (AddressInEra (ShelleyAddressInEra _) (ShelleyAddress nw pc scr)) =
...
toShelleyStakeAddr (StakeAddress nw sc) =
  Shelley.RewardAccount {Shelley.raNetwork = nw, Shelley.raCredential = sc}

toShelleyPaymentCredential :: PaymentCredential -> Shelley.PaymentCredential StandardCrypto
toShelleyPaymentCredential (PaymentCredentialByKey (PaymentKeyHash kh)) = Shelley.KeyHashObj kh
toShelleyPaymentCredential (PaymentCredentialByScript sh) =
  Shelley.ScriptHashObj (toShelleyScriptHash sh)

toShelleyStakeCredential :: StakeCredential -> Shelley.StakeCredential StandardCrypto
toShelleyStakeCredential (StakeCredentialByKey (StakeKeyHash kh)) = Shelley.KeyHashObj kh
toShelleyStakeCredential (StakeCredentialByScript sh) =
  Shelley.ScriptHashObj (toShelleyScriptHash sh)

toShelleyStakeReference :: StakeAddressReference -> Shelley.StakeReference StandardCrypto
...
toShelleyStakeReference NoStakeAddress = Shelley.StakeRefNull

fromShelleyAddrIsSbe :: () => ShelleyBasedEra era -> Shelley.Addr StandardCrypto -> AddressInEra era
fromShelleyAddrIsSbe sbe =
  \case
    Shelley.AddrBootstrap (Shelley.BootstrapAddress addr) ->
      AddressInEra ByronAddressInAnyEra (ByronAddress addr)
    Shelley.Addr nw pc scr -> AddressInEra (ShelleyAddressInEra sbe) (ShelleyAddress nw pc scr)

fromShelleyAddr :: ShelleyBasedEra era -> Shelley.Addr StandardCrypto -> AddressInEra era
fromShelleyAddr _ (Shelley.AddrBootstrap (Shelley.BootstrapAddress addr)) =
  AddressInEra ByronAddressInAnyEra (ByronAddress addr)
fromShelleyAddr sBasedEra (Shelley.Addr nw pc scr) =
  AddressInEra (ShelleyAddressInEra sBasedEra) (ShelleyAddress nw pc scr)
...
fromShelleyStakeCredential :: Shelley.StakeCredential StandardCrypto -> StakeCredential
...
fromShelleyPaymentCredential (Shelley.KeyHashObj kh) = PaymentCredentialByKey (PaymentKeyHash kh)
fromShelleyPaymentCredential (Shelley.ScriptHashObj sh) = PaymentCredentialByScript (ScriptHash sh)

fromShelleyStakeReference :: Shelley.StakeReference StandardCrypto -> StakeAddressReference
...
fromShelleyStakeReference Shelley.StakeRefNull = NoStakeAddress

...
data Address addrtype where
  ...
  --
  ShelleyAddress :: Shelley.Network
...
instance SerialiseAsRawBytes (Address ByronAddr) where
  serialiseToRawBytes (ByronAddress addr) =
    Shelley.serialiseAddr . Shelley.AddrBootstrap . Shelley.BootstrapAddress
    $ addr

  deserialiseFromRawBytes (AsAddress AsByronAddr) bs =
    case Shelley.decodeAddr bs :: Maybe (Shelley.Addr StandardCrypto) of
      Nothing -> Left
        (SerialiseAsRawBytesError "Unable to deserialise Address ByronAddr")
      Just Shelley.Addr {} -> Left
        (SerialiseAsRawBytesError "Unable to deserialise Address ByronAddr")
      Just (Shelley.AddrBootstrap (Shelley.BootstrapAddress addr)) -> Right
        (ByronAddress addr)
...
instance SerialiseAsRawBytes (Address ShelleyAddr) where
  ...
  deserialiseFromRawBytes (AsAddress AsShelleyAddr) bs =
    case Shelley.decodeAddr bs of
      ...
      Just Shelley.AddrBootstrap {} -> Left
        (SerialiseAsRawBytesError
           "Unable to deserialise bootstrap Address ShelleyAddr")
...
instance FromJSON (Address ByronAddr) where
  parseJSON = Aeson.withText "Address" $ \txt -> maybe
...
instance SerialiseAsRawBytes AddressAny where
  ...
  deserialiseFromRawBytes AsAddressAny bs = case Shelley.decodeAddr bs of
    Nothing -> Left
      (SerialiseAsRawBytesError "Unable to deserialise AddressAny")
    Just (Shelley.AddrBootstrap (Shelley.BootstrapAddress addr)) -> Right
...
data AddressInEra era where
  AddressInEra
    :: AddressTypeInEra addrtype era -> Address addrtype -> AddressInEra era
...
instance IsCardanoEra era => SerialiseAsRawBytes (AddressInEra era) where
  ...
  deserialiseFromRawBytes _ bs =
    first
      (const
         (SerialiseAsRawBytesError "Unable to deserialise AddressInEra era"))
...
instance IsCardanoEra era => SerialiseAddress (AddressInEra era) where
  ...
  deserialiseAddress _ t =
    rightToMaybe . anyAddressInEra cardanoEra
    =<< deserialiseAddress AsAddressAny t
...
anyAddressInEra era = \case
  ...
  AddressShelley addr -> forEraInEon
    era
...
instance SerialiseAsRawBytes StakeAddress where
  ...
  deserialiseFromRawBytes AsStakeAddress bs =
    case Shelley.deserialiseRewardAccount bs of
      Nothing -> Left
        (SerialiseAsRawBytesError "Unable to deserialise StakeAddress")
...
makeStakeAddress nw sc =
  StakeAddress (toShelleyNetwork nw) (toShelleyStakeCredential sc)
...
shelleyPayAddrToPlutusPubKHash
  :: Address ShelleyAddr -> Maybe PlutusAPI.PubKeyHash
shelleyPayAddrToPlutusPubKHash (ShelleyAddress _ payCred _) = case payCred of
...
toShelleyPaymentCredential
  :: PaymentCredential -> Shelley.PaymentCredential StandardCrypto
...
toShelleyStakeCredential
  :: StakeCredential -> Shelley.StakeCredential StandardCrypto
...
toShelleyStakeReference
  :: StakeAddressReference -> Shelley.StakeReference StandardCrypto
...
fromShelleyAddr
  :: ShelleyBasedEra era -> Shelley.Addr StandardCrypto -> AddressInEra era
...
fromShelleyStakeCredential
  :: Shelley.StakeCredential StandardCrypto -> StakeCredential
...
fromShelleyPaymentCredential
  :: Shelley.PaymentCredential StandardCrypto -> PaymentCredential
...
fromShelleyStakeReference
  :: Shelley.StakeReference StandardCrypto -> StakeAddressReference
...
stakeAddressCredential (StakeAddress _ scred) =

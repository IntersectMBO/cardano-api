...
instance SerialiseAsCBOR TxMetadata where
  ...
  deserialiseFromCBOR AsTxMetadata bs =
    TxMetadata . fromShelleyMetadata
      <$> (CBOR.decodeFullDecoder' CBOR.shelleyProtVer "TxMetadata" CBOR.decCBOR bs :: Either
             CBOR.DecoderError
             (Map Word64 Shelley.Metadatum))
...
metaTextChunks =
  TxMetaList
    . chunks txMetadataTextStringMaxByteLength TxMetaText (BS.length . Text.encodeUtf8) utf8SplitAt
  where
        ...
        --
              ->
               let sz = BS.length (Text.encodeUtf8 (Text.singleton char))
                in if len + sz > n
                     then ( n + 1 -- Higher than 'n' to always trigger the predicate
                          , (left, right <> Text.Builder.singleton char))
                     else (len + sz, (left <> Text.Builder.singleton char, right)))
            (0, (mempty, mempty))

-- | Create a 'TxMetadataValue' from a 'ByteString' as a list of chunks of an
-- accaptable size.
metaBytesChunks :: ByteString -> TxMetadataValue
metaBytesChunks = TxMetaList . chunks txMetadataByteStringMaxLength TxMetaBytes BS.length BS.splitAt
...
toShelleyMetadatum (TxMetaList xs) = Shelley.List [toShelleyMetadatum x | x <- xs]
toShelleyMetadatum (TxMetaMap xs) =
  Shelley.Map [(toShelleyMetadatum k, toShelleyMetadatum v) | (k, v) <- xs]
...
fromShelleyMetadatum (Shelley.List xs) = TxMetaList [fromShelleyMetadatum x | x <- xs]
fromShelleyMetadatum (Shelley.Map xs) =
  TxMetaMap [(fromShelleyMetadatum k, fromShelleyMetadatum v) | (k, v) <- xs]
...
chunks maxLength strHoist strLength strSplitAt str
  ...
  | otherwise = [strHoist str | strLength str > 0]
...
validateTxMetadata (TxMetadata m)
 ...
 =
  case [(k, err) | (k, v) <- Map.toList m, err <- validateTxMetadataValue v] of
    [] -> Right ()
    errs -> Left errs
...
validateTxMetadataValue (TxMetaNumber n) =
  ...
  | n > fromIntegral (maxBound :: Word64) || n < negate (fromIntegral (maxBound :: Word64))
  ]
validateTxMetadataValue (TxMetaBytes bs) =
  [TxMetadataBytesTooLong len | let len = BS.length bs, len > txMetadataByteStringMaxLength]
...
validateTxMetadataValue (TxMetaList xs) = foldMap validateTxMetadataValue xs
validateTxMetadataValue (TxMetaMap kvs) =
  foldMap (\(k, v) -> validateTxMetadataValue k <> validateTxMetadataValue v) kvs
...
instance Error TxMetadataRangeError where
  prettyError =
    \case
      TxMetadataNumberOutOfRange n ->
        "Numeric metadata value " <> pretty n <> " is outside the range -(2^64-1) .. 2^64-1."
...
--
metadataFromJson :: TxMetadataJsonSchema -> Aeson.Value -> Either TxMetadataJsonError TxMetadata
metadataFromJson schema
 ...
 =
  \case
    Aeson.Object m ->
      fmap (TxMetadata . Map.fromList) . mapM (uncurry metadataKeyPairFromJson) $ KeyMap.toList m
    _ -> Left TxMetadataJsonToplevelNotMap
  where
    metadataKeyPairFromJson ::
         Aeson.Key -> Aeson.Value -> Either TxMetadataJsonError (Word64, TxMetadataValue)
    metadataKeyPairFromJson k v = do
      k' <- convTopLevelKey k
      v' <- first (TxMetadataJsonSchemaError k' v) (metadataValueFromJson v)
      first (TxMetadataRangeError k' v) (validateMetadataValue v')
      return (k', v')
    ...
    convTopLevelKey (Aeson.toText -> k) =
      case parseAll (pUnsigned <* Atto.endOfInput) k of
        Just n
          | n <= fromIntegral (maxBound :: Word64) -> Right (fromIntegral n)
    ...
    validateMetadataValue v =
      case validateTxMetadataValue v of
        [] -> Right ()
        err:_ -> Left err
    metadataValueFromJson :: Aeson.Value -> Either TxMetadataJsonSchemaError TxMetadataValue
...
--
metadataToJson :: TxMetadataJsonSchema -> TxMetadata -> Aeson.Value
metadataToJson schema =
  \(TxMetadata mdMap) ->
    Aeson.object [(Aeson.fromString (show k), metadataValueToJson v) | (k, v) <- Map.toList mdMap]
  where
...
metadataValueToJsonNoSchema = conv
  where
    ...
    conv (TxMetaBytes bs) = Aeson.String (bytesPrefix <> Text.decodeLatin1 (Base16.encode bs))
    conv (TxMetaText txt) = Aeson.String txt
    conv (TxMetaList vs) = Aeson.Array (Vector.fromList (map conv vs))
    conv (TxMetaMap kvs) = Aeson.object [(convKey k, conv v) | (k, v) <- kvs]
    ...
    convKey (TxMetaBytes bs) = Aeson.fromText $ bytesPrefix <> Text.decodeLatin1 (Base16.encode bs)
    convKey (TxMetaText txt) = Aeson.fromText txt
    convKey v = Aeson.fromText . Text.Lazy.toStrict . Aeson.Text.encodeToLazyText . conv $ v

metadataValueFromJsonNoSchema :: Aeson.Value -> Either TxMetadataJsonSchemaError TxMetadataValue
metadataValueFromJsonNoSchema = conv
  where
    ...
    conv (Aeson.String s)
      ...
      , not (BSC.any (\c -> c >= 'A' && c <= 'F') bs') = Right (TxMetaBytes bs)
    conv (Aeson.String s) = Right (TxMetaText s)
    conv (Aeson.Array vs) = fmap TxMetaList . traverse conv $ Vector.toList vs
    conv (Aeson.Object kvs) =
      fmap (TxMetaMap . sortCanonicalForCbor)
...
sortCanonicalForCbor :: [(TxMetadataValue, TxMetadataValue)] -> [(TxMetadataValue, TxMetadataValue)]
sortCanonicalForCbor =
  map snd . List.sortOn fst . map (\e@(k, _) -> (CBOR.uintegerFromBytes $ serialiseKey k, e))
  where
...
metadataValueToJsonDetailedSchema = conv
  where
    conv :: TxMetadataValue -> Aeson.Value
    conv (TxMetaNumber n) = singleFieldObject "int" . Aeson.Number $ fromInteger n
    conv (TxMetaBytes bs) =
      singleFieldObject "bytes" . Aeson.String $ Text.decodeLatin1 (Base16.encode bs)
    conv (TxMetaText txt) = singleFieldObject "string" . Aeson.String $ txt
    conv (TxMetaList vs) = singleFieldObject "list" . Aeson.Array $ Vector.fromList (map conv vs)
    conv (TxMetaMap kvs) =
      singleFieldObject "map" . Aeson.Array
        $ Vector.fromList [Aeson.object [("k", conv k), ("v", conv v)] | (k, v) <- kvs]
    singleFieldObject name v = Aeson.object [(name, v)]

metadataValueFromJsonDetailedSchema ::
     Aeson.Value -> Either TxMetadataJsonSchemaError TxMetadataValue
metadataValueFromJsonDetailedSchema = conv
  where
    ...
    conv (Aeson.Object m) =
      case KeyMap.toList m of
        ...
        [("bytes", Aeson.String s)]
          | Right bs <- Base16.decode (Text.encodeUtf8 s) -> Right (TxMetaBytes bs)
        [("string", Aeson.String s)] -> Right (TxMetaText s)
        [("list", Aeson.Array vs)] -> fmap TxMetaList . traverse conv $ Vector.toList vs
        [("map", Aeson.Array kvs)] -> fmap TxMetaMap . traverse convKeyValuePair $ Vector.toList kvs
        [(key, v)]
    ...
    convKeyValuePair ::
         Aeson.Value -> Either TxMetadataJsonSchemaError (TxMetadataValue, TxMetadataValue)
    convKeyValuePair (Aeson.Object m)
      | KeyMap.size m == 2
      , Just k <- KeyMap.lookup "k" m
      , Just v <- KeyMap.lookup "v" m = (,) <$> conv k <*> conv v
...
instance Error TxMetadataJsonSchemaError where
  prettyError =
    \case
      TxMetadataJsonNullNotAllowed -> "JSON null values are not supported."
      TxMetadataJsonBoolNotAllowed -> "JSON bool values are not supported."
...
parseAll p = either (const Nothing) Just . Atto.parseOnly p . Text.encodeUtf8

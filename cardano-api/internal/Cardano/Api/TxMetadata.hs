...
instance SerialiseAsCBOR TxMetadata where
  ...
  deserialiseFromCBOR AsTxMetadata bs =
    ...
    <$> (CBOR.decodeFullDecoder'
           ...
           "TxMetadata"
           CBOR.decCBOR
           bs :: Either CBOR.DecoderError (Map Word64 Shelley.Metadatum))
        ...
        (\( len, ( left, right ) ) char ->
         ...
         in  if len + sz > n
               then ( n + 1 -- Higher than 'n' to always trigger the predicate
                    ...
                    , ( left <> Text.Builder.singleton char, right ) ))
        ( 0, ( mempty, mempty ) )
...
metaBytesChunks =
  TxMetaList
  . chunks txMetadataByteStringMaxLength TxMetaBytes BS.length BS.splitAt
...
toShelleyMetadatum (TxMetaList xs)  =
  Shelley.List [ toShelleyMetadatum x | x <- xs ]
toShelleyMetadatum (TxMetaMap xs)   =
  Shelley.Map
    [ ( toShelleyMetadatum k, toShelleyMetadatum v ) | ( k, v ) <- xs ]
...
fromShelleyMetadatum (Shelley.List xs) =
  TxMetaList [ fromShelleyMetadatum x | x <- xs ]
fromShelleyMetadatum (Shelley.Map xs)  =
  TxMetaMap
    [ ( fromShelleyMetadatum k, fromShelleyMetadatum v ) | ( k, v ) <- xs ]
...
validateTxMetadata (TxMetadata m) =
  ...
  case [ ( k, err )
       | ( k, v ) <- Map.toList m, err <- validateTxMetadataValue v ] of
    []   -> Right ()
    errs -> Left errs
...
validateTxMetadataValue (TxMetaNumber n) =
  [ TxMetadataNumberOutOfRange n | n > fromIntegral (maxBound :: Word64)
  || n < negate (fromIntegral (maxBound :: Word64)) ]
validateTxMetadataValue (TxMetaBytes bs) =
  [ TxMetadataBytesTooLong len
  | let len = BS.length bs, len > txMetadataByteStringMaxLength ]
validateTxMetadataValue (TxMetaText txt) =
  [ TxMetadataTextTooLong len | let len = BS.length (Text.encodeUtf8 txt)
...
validateTxMetadataValue (TxMetaMap kvs)  =
  foldMap
    (\( k, v ) -> validateTxMetadataValue k <> validateTxMetadataValue v)
    kvs
...
instance Error TxMetadataRangeError where
  prettyError = \case
    TxMetadataNumberOutOfRange n     -> "Numeric metadata value "
      <> pretty n
...
metadataFromJson schema = \case
  ...
  where
    ...
    validateMetadataValue v = case validateTxMetadataValue v of
      []      -> Right ()
      err : _ -> Left err

    metadataValueFromJson
      :: Aeson.Value -> Either TxMetadataJsonSchemaError TxMetadataValue
    metadataValueFromJson = case schema of
...
metadataValueToJsonNoSchema = conv
  where
    ...
    conv (TxMetaMap kvs)  =
      Aeson.object [ ( convKey k, conv v ) | ( k, v ) <- kvs ]
    ...
    convKey v =
      Aeson.fromText . Text.Lazy.toStrict . Aeson.Text.encodeToLazyText . conv
      $ v
...
metadataValueFromJsonNoSchema = conv
  where
    ...
    conv (Aeson.Array vs)   =
      fmap TxMetaList . traverse conv $ Vector.toList vs
...
metadataValueToJsonDetailedSchema = conv
  where
    conv :: TxMetadataValue -> Aeson.Value
    conv (TxMetaNumber n) =
      singleFieldObject "int" . Aeson.Number $ fromInteger n
    conv (TxMetaBytes bs) =
      singleFieldObject "bytes" . Aeson.String
    ...
    conv (TxMetaMap kvs)  =
      ...
      $ Vector.fromList
        [ Aeson.object [ ( "k", conv k ), ( "v", conv v ) ] | ( k, v ) <- kvs ]
...
metadataValueFromJsonDetailedSchema = conv
  where
    conv :: Aeson.Value -> Either TxMetadataJsonSchemaError TxMetadataValue
    conv (Aeson.Object m) = case KeyMap.toList m of
      [ ( "int", Aeson.Number d ) ] -> case Scientific.floatingOrInteger d
        :: Either Double Integer of
        Left n  -> Left (TxMetadataJsonNumberNotInteger n)
        Right n -> Right (TxMetaNumber n)

      [ ( "bytes", Aeson.String s ) ]
        | Right bs <- Base16.decode (Text.encodeUtf8 s) -> Right
          (TxMetaBytes bs)
      ...
      [ ( "map", Aeson.Array kvs ) ]
        -> fmap TxMetaMap . traverse convKeyValuePair $ Vector.toList kvs

      [ ( key, v ) ]
        | key `elem` [ "int", "bytes", "string", "list", "map" ] -> Left
          (TxMetadataJsonTypeMismatch (Aeson.toText key) v)
    ...
    convKeyValuePair
      :: Aeson.Value
      -> Either TxMetadataJsonSchemaError ( TxMetadataValue, TxMetadataValue )
...
pBytes = do
  ...
  case Base16.decode remaining of
    Right bs -> return bs
    _        -> fail
      ("Expecting base16 encoded string, found: " <> show remaining)
  where
    hexUpper c = c >= 'A' && c <= 'F'

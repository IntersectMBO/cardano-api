...
instance Show (Block era) where
  ...
  showsPrec p (ShelleyBlock ShelleyBasedEraShelley block) =
    showParen
      (p >= 11)
      (showString "ShelleyBlock ShelleyBasedEraShelley " . showsPrec 11 block)

  showsPrec p (ShelleyBlock ShelleyBasedEraAllegra block) =
    showParen
      (p >= 11)
      (showString "ShelleyBlock ShelleyBasedEraAllegra " . showsPrec 11 block)

  showsPrec p (ShelleyBlock ShelleyBasedEraMary block) =
    showParen
      (p >= 11)
      (showString "ShelleyBlock ShelleyBasedEraMary " . showsPrec 11 block)

  showsPrec p (ShelleyBlock ShelleyBasedEraAlonzo block) =
    showParen
      (p >= 11)
      (showString "ShelleyBlock ShelleyBasedEraAlonzo " . showsPrec 11 block)

  showsPrec p (ShelleyBlock ShelleyBasedEraBabbage block) =
    showParen
      (p >= 11)
      (showString "ShelleyBlock ShelleyBasedEraBabbage " . showsPrec 11 block)

  showsPrec p (ShelleyBlock ShelleyBasedEraConway block) =
    showParen
      (p >= 11)
      (showString "ShelleyBlock ShelleyBasedEraConway " . showsPrec 11 block)
...
toConsensusBlock = \case
  ...
  BlockInMode _ (ShelleyBlock ShelleyBasedEraShelley b')
    -> Consensus.BlockShelley b'
  BlockInMode _ (ShelleyBlock ShelleyBasedEraAllegra b')
    -> Consensus.BlockAllegra b'
  BlockInMode _ (ShelleyBlock ShelleyBasedEraMary b') -> Consensus.BlockMary b'
  BlockInMode _ (ShelleyBlock ShelleyBasedEraAlonzo b')
    -> Consensus.BlockAlonzo b'
  BlockInMode _ (ShelleyBlock ShelleyBasedEraBabbage b')
    -> Consensus.BlockBabbage b'
  BlockInMode _ (ShelleyBlock ShelleyBasedEraConway b')
    -> Consensus.BlockConway b'
...
instance SerialiseAsRawBytes (Hash BlockHeader) where
  ...
  deserialiseFromRawBytes (AsHash AsBlockHeader) bs
    ...
    | otherwise =
      Left (SerialiseAsRawBytesError "Unable to deserialise Hash BlockHeader")
...
getBlockHeader = \case
  ShelleyBlock sbe block -> shelleyBasedEraConstraints sbe
    $ let Consensus.HeaderFields
            { Consensus.headerFieldHash = Consensus.ShelleyHash
                (Crypto.UnsafeHash hashSBS)
            , Consensus.headerFieldSlot
            , Consensus.headerFieldBlockNo } = Consensus.getHeaderFields block
      in  BlockHeader headerFieldSlot (HeaderHash hashSBS) headerFieldBlockNo

  ByronBlock block -> BlockHeader
    headerFieldSlot
...
instance ToJSON ChainPoint where
  toJSON = \case
    ...
    ChainPoint slot blockHash -> object

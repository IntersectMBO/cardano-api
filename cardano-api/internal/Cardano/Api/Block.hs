...
data Block era where
  ByronBlock :: Consensus.ByronBlock -> Block ByronEra
  ShelleyBlock
    :: ShelleyBasedEra era
...
instance Show (Block era) where
  showsPrec p (ByronBlock block) =
    showParen (p >= 11) (showString "ByronBlock " . showsPrec 11 block)
  showsPrec p (ShelleyBlock ShelleyBasedEraShelley block) =
    showParen (p >= 11) (showString "ShelleyBlock ShelleyBasedEraShelley " . showsPrec 11 block)
  showsPrec p (ShelleyBlock ShelleyBasedEraAllegra block) =
    showParen (p >= 11) (showString "ShelleyBlock ShelleyBasedEraAllegra " . showsPrec 11 block)
  showsPrec p (ShelleyBlock ShelleyBasedEraMary block) =
    showParen (p >= 11) (showString "ShelleyBlock ShelleyBasedEraMary " . showsPrec 11 block)
  showsPrec p (ShelleyBlock ShelleyBasedEraAlonzo block) =
    showParen (p >= 11) (showString "ShelleyBlock ShelleyBasedEraAlonzo " . showsPrec 11 block)
  showsPrec p (ShelleyBlock ShelleyBasedEraBabbage block) =
    showParen (p >= 11) (showString "ShelleyBlock ShelleyBasedEraBabbage " . showsPrec 11 block)
  showsPrec p (ShelleyBlock ShelleyBasedEraConway block) =
    showParen (p >= 11) (showString "ShelleyBlock ShelleyBasedEraConway " . showsPrec 11 block)
 ...
 =
  \case
    ...
    ShelleyBlock sbe Consensus.ShelleyBlock {Consensus.shelleyBlockRaw} ->
      shelleyBasedEraConstraints sbe $ getShelleyBlockTxs sbe shelleyBlockRaw

getShelleyBlockTxs ::
     forall era ledgerera blockheader. ShelleyLedgerEra era ~ ledgerera
  => Consensus.ShelleyCompatible (ConsensusProtocol era) ledgerera =>
       Consensus.ShelleyProtocolHeader (ConsensusProtocol era) ~ blockheader =>
         ShelleyBasedEra era -> Ledger.Block blockheader ledgerera -> [Tx era]
getShelleyBlockTxs era (Ledger.Block _header txs) =
  [ShelleyTx era txinblock | txinblock <- toList (Ledger.fromTxSeq txs)]
...
fromConsensusBlock :: () => Consensus.CardanoBlock L.StandardCrypto ~ block => block -> BlockInMode
fromConsensusBlock =
  \case
...
toConsensusBlock :: () => Consensus.CardanoBlock L.StandardCrypto ~ block => BlockInMode -> block
toConsensusBlock =
  \case
...
-- representation.
data instance Hash BlockHeader =
  HeaderHash SBS.ShortByteString
  ...
  \case
    ShelleyBlock sbe block ->
      shelleyBasedEraConstraints sbe
        $ let Consensus.HeaderFields { Consensus.headerFieldHash = Consensus.ShelleyHash (Crypto.UnsafeHash hashSBS)
    ...
    ByronBlock block ->
      BlockHeader
        ...
        headerFieldBlockNo
      where Consensus.HeaderFields { Consensus.headerFieldHash = Consensus.ByronHash byronHeaderHash
                                   , Consensus.headerFieldSlot
                                   , Consensus.headerFieldBlockNo
                                   } = Consensus.getHeaderFields block
  ...
  toJSON =
    \case
      ...
      ChainPoint slot blockHash ->
        object
          ["tag" .= String "ChainPoint", "slot" .= toJSON slot, "blockHash" .= toJSON blockHash]

instance FromJSON ChainPoint where
  parseJSON =
    withObject "ChainPoint" $ \o -> do
      tag <- o .: "tag"
      case tag :: Text of
...
--
toConsensusPointHF ::
     Consensus.HeaderHash block ~ Consensus.OneEraHash xs => ChainPoint -> Consensus.Point block
...
--
fromConsensusPointHF ::
     Consensus.HeaderHash block ~ Consensus.OneEraHash xs => Consensus.Point block -> ChainPoint
...
--
toConsensusPoint ::
     forall ledgerera protocol. Consensus.ShelleyCompatible protocol ledgerera
  => ChainPoint
...
--
fromConsensusPoint ::
     forall protocol ledgerera. Consensus.ShelleyCompatible protocol ledgerera
  => Consensus.Point (Consensus.ShelleyBlock protocol ledgerera)
  -> ChainPoint
...
instance ToJSON ChainTip where
  ...
  toJSON (ChainTip slot headerHash (Consensus.BlockNo bNum)) =
    object ["slot" .= slot, "hash" .= serialiseToRawBytesHexText headerHash, "block" .= bNum]
...
fromConsensusTip ::
     () => Consensus.CardanoBlock L.StandardCrypto ~ block => Consensus.Tip block -> ChainTip
fromConsensusTip = conv
  where
    conv :: Consensus.Tip (Consensus.CardanoBlock Consensus.StandardCrypto) -> ChainTip
    conv Consensus.TipGenesis = ChainTipAtGenesis
    conv (Consensus.Tip slot (Consensus.OneEraHash h) block) = ChainTip slot (HeaderHash h) block

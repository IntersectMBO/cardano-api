...
data EraHistory where
  EraHistory
    :: Consensus.CardanoBlock L.StandardCrypto ~ Consensus.HardForkBlock xs=> History.Interpreter xs
    -> EraHistory

getProgress ::
     () => SlotNo -> EraHistory -> Either Qry.PastHorizonException (RelativeTime, SlotLength)
...
getSlotForRelativeTime :: () => RelativeTime -> EraHistory -> Either Qry.PastHorizonException SlotNo
...
slotToEpoch slotNo (EraHistory interpreter) =
  case Qry.interpretQuery interpreter (Qry.slotToEpoch slotNo) of
    Right (epochNumber, slotsInEpoch, slotsToEpochEnd) ->
      Right (epochNumber, SlotsInEpoch slotsInEpoch, SlotsToEpochEnd slotsToEpochEnd)
    Left e -> Left e
...
data QueryInEra era result where
  ...
  QueryInShelleyBasedEra
    :: ShelleyBasedEra era -> QueryInShelleyBasedEra era result -> QueryInEra era result
...
data QueryInShelleyBasedEra era result where
  ...
  QueryProtocolParameters :: QueryInShelleyBasedEra era (Ledger.PParams (ShelleyLedgerEra era))
  ...
  QueryStakePoolParameters
    :: Set PoolId -> QueryInShelleyBasedEra era (Map PoolId StakePoolParameters)
  ...
  QueryPoolDistribution
    :: Maybe (Set PoolId) -> QueryInShelleyBasedEra era (SerialisedPoolDistribution era)
  QueryStakeSnapshot
    :: Maybe (Set PoolId) -> QueryInShelleyBasedEra era (SerialisedStakeSnapshots era)
  QueryStakeDelegDeposits
    :: Set StakeCredential -> QueryInShelleyBasedEra era (Map StakeCredential L.Coin)
  QueryAccountState :: QueryInShelleyBasedEra era L.AccountState
  QueryConstitution :: QueryInShelleyBasedEra era (L.Constitution (ShelleyLedgerEra era))
  QueryGovState :: QueryInShelleyBasedEra era (L.GovState (ShelleyLedgerEra era))
  QueryDRepState
...
newtype ByronUpdateState =
  ByronUpdateState Byron.Update.State
  deriving (Show)
...
instance (IsShelleyBasedEra era, FromJSON (TxOut CtxUTxO era)) => FromJSON (UTxO era) where
  parseJSON =
    withObject "UTxO" $ \hm -> do
...
decodeDebugLedgerState ::
  ...
  => FromCBOR (DebugLedgerState era) =>
       SerialisedDebugLedgerState era -> Either
...
newtype PoolState era =
  PoolState (Shelley.PState (ShelleyLedgerEra era))
...
newtype StakeSnapshot era =
  StakeSnapshot (Consensus.StakeSnapshots (Core.EraCrypto (ShelleyLedgerEra era)))

decodeStakeSnapshot ::
     forall era. ()
  => FromCBOR (Consensus.StakeSnapshots (Core.EraCrypto (ShelleyLedgerEra era))) =>
       SerialisedStakeSnapshots era -> Either DecoderError (StakeSnapshot era)
decodeStakeSnapshot (SerialisedStakeSnapshots (Serialised ls)) =
  StakeSnapshot <$> Plain.decodeFull ls

toShelleyAddrSet :: CardanoEra era -> Set AddressAny -> Set (Shelley.Addr Consensus.StandardCrypto)
...
toLedgerUTxO :: () => ShelleyBasedEra era -> UTxO era -> Shelley.UTxO (ShelleyLedgerEra era)
toLedgerUTxO sbe (UTxO utxo) =
  shelleyBasedEraConstraints sbe
    $ Shelley.UTxO . Map.fromList . map (bimap toShelleyTxIn (toShelleyTxOut sbe)) . Map.toList
    $ utxo

fromLedgerUTxO :: () => ShelleyBasedEra era -> Shelley.UTxO (ShelleyLedgerEra era) -> UTxO era
fromLedgerUTxO sbe (Shelley.UTxO utxo) =
  shelleyBasedEraConstraints sbe
    $ UTxO . Map.fromList . map (bimap fromShelleyTxIn (fromShelleyTxOut sbe)) . Map.toList
    $ utxo

fromShelleyPoolDistr :: Consensus.PoolDistr StandardCrypto -> Map (Hash StakePoolKey) Rational
...
fromShelleyDelegations
 ...
 = Map.fromList . map (bimap fromShelleyStakeCredential StakePoolKeyHash) . Map.toList

fromShelleyRewardAccounts ::
     Shelley.RewardAccounts Consensus.StandardCrypto -> Map StakeCredential L.Coin
fromShelleyRewardAccounts
    --TODO: write an appropriate property to show it is safe to use
    -- Map.fromListAsc or to use Map.mapKeysMonotonic
...
toConsensusQuery ::
  ...
  => Consensus.CardanoBlock L.StandardCrypto ~ block =>
       QueryInMode result -> Some (Consensus.Query block)
...
toConsensusQuery QueryEraHistory =
  Some $ Consensus.BlockQuery $ Consensus.QueryHardFork Consensus.GetInterpreter
...
toConsensusQuery (QueryInEra QueryByronUpdateState) =
  Some $ Consensus.BlockQuery $ Consensus.QueryIfCurrentByron Consensus.GetUpdateInterfaceState
...
toConsensusQueryShelleyBased ::
  ...
  => ConsensusBlockForEra era ~ Consensus.ShelleyBlock protocol (ShelleyLedgerEra era) =>
       Core.EraCrypto (ShelleyLedgerEra era) ~ Consensus.StandardCrypto =>
         Consensus.CardanoBlock L.StandardCrypto ~ block =>
           ShelleyBasedEra era -> QueryInShelleyBasedEra era result -> Some (Consensus.Query block)
toConsensusQueryShelleyBased sbe =
  \case
    ...
    QueryConstitution ->
      caseShelleyToBabbageOrConwayEraOnwards
        ...
        sbe
    QueryGenesisParameters -> Some (consensusQueryInEraInMode era Consensus.GetGenesisConfig)
    QueryProtocolParameters -> Some (consensusQueryInEraInMode era Consensus.GetCurrentPParams)
    QueryProtocolParametersUpdate ->
    ...
    QueryUTxO QueryUTxOWhole -> Some (consensusQueryInEraInMode era Consensus.GetUTxOWhole)
    QueryUTxO (QueryUTxOByAddress addrs) ->
      Some (consensusQueryInEraInMode era (Consensus.GetUTxOByAddress addrs'))
      where addrs' :: Set (Shelley.Addr Consensus.StandardCrypto)
    ...
    QueryUTxO (QueryUTxOByTxIn txins) ->
      ...
      where txins' :: Set (Shelley.TxIn Consensus.StandardCrypto)
            txins' = Set.map toShelleyTxIn txins
    QueryStakeAddresses creds _nId ->
      Some
        (consensusQueryInEraInMode era (Consensus.GetFilteredDelegationsAndRewardAccounts creds'))
    ...
    QueryStakePoolParameters poolids ->
      ...
      where poolids' :: Set (Shelley.KeyHash Shelley.StakePool Consensus.StandardCrypto)
    ...
    QueryStakeSnapshot mPoolIds ->
      Some
        (consensusQueryInEraInMode
           era
           (Consensus.GetCBOR
              (Consensus.GetStakeSnapshots (fmap (Set.map unStakePoolKeyHash) mPoolIds))))
    QueryPoolDistribution poolIds ->
      ...
      where getPoolIds ::
                 Set PoolId -> Set (Shelley.KeyHash Shelley.StakePool Consensus.StandardCrypto)
    ...
    QueryAccountState -> Some (consensusQueryInEraInMode era Consensus.GetAccountState)
    ...
    QueryStakeVoteDelegatees creds ->
      caseShelleyToBabbageOrConwayEraOnwards
        ...
        (const $ Some (consensusQueryInEraInMode era (Consensus.GetFilteredVoteDelegatees creds')))
        sbe
      where creds' :: Set (Shelley.Credential Shelley.Staking StandardCrypto)
            creds' = Set.map toShelleyStakeCredential creds
  where
...
consensusQueryInEraInMode ::
     forall era erablock modeblock result result' xs. ConsensusBlockForEra era ~ erablock
  => Consensus.CardanoBlock L.StandardCrypto ~ modeblock =>
       modeblock ~ Consensus.HardForkBlock xs =>
         Consensus.HardForkQueryResult xs result ~ result' =>
           CardanoEra era -> Consensus.BlockQuery erablock result -> Consensus.Query
                                                                       modeblock
                                                                       result'
...
fromConsensusQueryResult ::
  ...
  => HasCallStack =>
       Consensus.CardanoBlock L.StandardCrypto ~ block =>
         QueryInMode result -> Consensus.Query block result' -> result' -> result
fromConsensusQueryResult QueryEraHistory q' r' =
  case q' of
    Consensus.BlockQuery (Consensus.QueryHardFork Consensus.GetInterpreter) -> EraHistory r'
...
fromConsensusQueryResult (QueryInEra QueryByronUpdateState) q' r' =
  case q' of
    Consensus.BlockQuery (Consensus.QueryIfCurrentByron Consensus.GetUpdateInterfaceState) ->
  ...
  => ShelleyLedgerEra era ~ ledgerera =>
       Core.EraCrypto ledgerera ~ Consensus.StandardCrypto =>
         ConsensusProtocol era ~ protocol =>
           ShelleyBasedEra era -> QueryInShelleyBasedEra era result -> Consensus.BlockQuery
...
fromConsensusQueryResultShelleyBased sbe sbeQuery q' r' =
  case sbeQuery of
    ...
    QueryStakePoolParameters {} ->
      case q' of
        Consensus.GetStakePoolParams {} ->
          Map.map fromShelleyPoolParams . Map.mapKeysMonotonic StakePoolKeyHash $ r'
        _ -> fromConsensusQueryResultMismatch
    QueryDebugLedgerState {} ->
      case q' of
    ...
    QueryStakeDelegDeposits {} ->
      case q' of
        Consensus.GetStakeDelegDeposits {} -> Map.mapKeysMonotonic fromShelleyStakeCredential r'
        _ -> fromConsensusQueryResultMismatch
    QueryAccountState {} ->
      case q' of
    ...
    QueryStakeVoteDelegatees {} ->
      case q' of
        Consensus.GetFilteredVoteDelegatees {} -> Map.mapKeys fromShelleyStakeCredential r'
        _ -> fromConsensusQueryResultMismatch
...
fromConsensusQueryResultMismatch =
  withFrozenCallStack $ error "fromConsensusQueryResult: internal query mismatch"

fromConsensusEraMismatch :: SListI xs => Consensus.MismatchEraInfo xs -> EraMismatch

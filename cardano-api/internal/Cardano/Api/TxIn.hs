...
--
newtype TxId =
  TxId (Shelley.Hash StandardCrypto Shelley.EraIndependentTxBody)
...
instance SerialiseAsRawBytes TxId where
  ...
  deserialiseFromRawBytes AsTxId bs =
...
toShelleyTxId (TxId h) = Ledger.TxId (SafeHash.unsafeMakeSafeHash (Crypto.castHash h))

fromShelleyTxId :: Ledger.TxId StandardCrypto -> TxId
fromShelleyTxId (Ledger.TxId h) = TxId (Crypto.castHash (SafeHash.extractHash h))
...
renderTxIn (TxIn txId (TxIx ix)) = serialiseToRawBytesHexText txId <> "#" <> Text.pack (show ix)
...
toByronTxIn (TxIn txid (TxIx txix)) = Byron.TxInUtxo (toByronTxId txid) (fromIntegral txix)

...
instance SerialiseAsRawBytes PolicyId where
  ...
  deserialiseFromRawBytes AsPolicyId bs = PolicyId <$> deserialiseFromRawBytes AsScriptHash bs
...
instance Show Value where
  showsPrec d v = showParen (d > 10) $ showString "valueFromList " . shows (valueToList v)
...
mergeAssetMaps :: Map AssetId Quantity -> Map AssetId Quantity -> Map AssetId Quantity
mergeAssetMaps =
  Map.merge Map.preserveMissing Map.preserveMissing (Map.zipWithMaybeMatched mergeQuantity)
  where
...
fromLedgerValue sbe v =
  caseShelleyToAllegraOrMaryEraOnwards (const (coinToValue v)) (const (fromMaryValue v)) sbe
 ...
 =
  ...
  where
    ...
    nonAdaAssets =
      Map.fromListWith
        (Map.unionWith (<>))
        [(pId, Map.singleton aName q) | (AssetId pId aName, q) <- valueToList v]
  ...
  valueFromList
    ...
    , (aId, q) <-
        case bundle of
          ...
          ValueNestedBundle pId qs -> [(AssetId pId aName, q) | (aName, q) <- Map.toList qs]
    ]
...
instance FromJSON ValueNestedRep where
  parseJSON =
    withObject "ValueNestedRep" $ \obj ->
      ValueNestedRep <$> sequenceA [parsePid keyValTuple | keyValTuple <- KeyMap.toList obj]
    where
      ...
      parsePid (Aeson.toText -> pid, quantityBundleJson) = do
        sHash <-
          failEitherWith (\e -> "Failure when deserialising PolicyId: " ++ displayError e)
...
renderAssetIdQuantityPair (aId, quant) = Text.pack (show quant) <> " " <> renderAssetId aId

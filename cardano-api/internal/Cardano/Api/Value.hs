...
import           Data.Aeson
       ( FromJSON, FromJSONKey, ToJSON, object, parseJSON, toJSON, withObject )
...
toByronLovelace (L.Coin x) = case Byron.integerToLovelace x of
  Left _   -> Nothing
  Right x' -> Just x'
...
newtype AssetName = AssetName ByteString
  ...
  deriving ( ToJSON, FromJSON, ToJSONKey
           , FromJSONKey ) via UsingRawBytesHex AssetName

instance IsString AssetName where
  fromString s
    | let bs = Text.encodeUtf8 (Text.pack s), BS.length bs <= 32 =
...
instance Show Value where
  showsPrec d v =
    showParen (d > 10) $ showString "valueFromList " . shows (valueToList v)
...
mergeAssetMaps
  :: Map AssetId Quantity -> Map AssetId Quantity -> Map AssetId Quantity
mergeAssetMaps =
  Map.merge
    Map.preserveMissing
    Map.preserveMissing
  ...
  where
    ...
    mergeQuantity _k a b = case a <> b of
      Quantity 0 -> Nothing
      c          -> Just c
...
valueFromList = Value . Map.filter (/= 0) . Map.fromListWith (<>)
...
valueToLovelace v = case valueToList v of
  [] -> Just (L.Coin 0)
  [ ( AdaAssetId, q ) ] -> Just (quantityToLovelace q)
  _ -> Nothing

toMaryValue :: Value -> MaryValue StandardCrypto
toMaryValue v = Mary.valueFromList (L.Coin lovelace) other
  where
...
fromLedgerValue
  :: ShelleyBasedEra era -> L.Value (ShelleyLedgerEra era) -> Value
...
calcMinimumDeposit v = Mary.scaledMinDeposit (toMaryValue v)
...
valueFromNestedRep (ValueNestedRep bundles) =
  valueFromList [ ( aId, q ) | bundle <- bundles, ( aId, q ) <- case bundle of
    ValueNestedBundleAda q   -> [ ( AdaAssetId, q ) ]
    ValueNestedBundle pId qs
      -> [ ( AssetId pId aName, q ) | ( aName, q ) <- Map.toList qs ] ]
  ...
  toJSON (ValueNestedRep bundles) = object $ map toPair bundles
    where
      ...
      toPair (ValueNestedBundle pid assets) =
        ( Aeson.fromText $ renderPolicyId pid, toJSON assets )

instance FromJSON ValueNestedRep where
  parseJSON = withObject "ValueNestedRep" $ \obj -> ValueNestedRep
    <$> sequenceA [ parsePid keyValTuple | keyValTuple <- KeyMap.toList obj ]
    where

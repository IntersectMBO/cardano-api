...
import Cardano.Api.Ledger (toCompact)
    ...
    , -- ledger protocol parameters
      sgProtocolParams =
        emptyPParams
          ...
          & ppTxFeePerByteL .~ CoinPerByte (fromJust $ toCompact (Coin 1)) -- The linear factor for the minimum fee calculation
...
alonzoGenesisDefaults =
  AlonzoGenesis
    ...
    , agPlutusV1CostModel =
        fromRight (error "Failed to create PlutusV1 cost model") $ L.mkCostModel PlutusV1 defaultV1CostModel
    , agCollateralPercentage = 150
    , agCoinsPerUTxOWord = CoinPerWord $ Coin 34482
    , agExtraConfig = Just $ AlonzoExtraConfig $ Just $ L.updateCostModels apiCostModels L.emptyCostModels
    }
 where
  apiCostModels =
    L.mkCostModels $
      fromList
        ...
        ,
          ( PlutusV2
          , fromRight (error "Failed to create PlutusV2 cost model") $ L.mkCostModel PlutusV2 defaultV2CostModel
          )
        ]
  defaultV1CostModel =
    [ 205665

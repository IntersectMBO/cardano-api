...
readCardanoGenesisConfig
  ...
  -- 'Cardano.Api.Internal.Genesis.decodeAlonzGenesis' for more details)
  -> NodeConfig
  -> t m GenesisConfig
...
tickThenReapplyCheckHash cfg block (LedgerState st tbs) =
  if Consensus.blockPrevHash block == Ledger.ledgerTipHash st
    then
      let
        keys
          :: Consensus.LedgerTables (Ledger.LedgerState (Consensus.CardanoBlock Consensus.StandardCrypto)) Ledger.KeysMK
        keys = Ledger.getBlockKeySets block

        restrictedTables =
          Consensus.LedgerTables (Ledger.restrictValuesMK (Ledger.getLedgerTables tbs) (Ledger.getLedgerTables keys))

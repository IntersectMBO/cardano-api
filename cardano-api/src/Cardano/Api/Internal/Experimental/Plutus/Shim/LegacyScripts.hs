...
fromExistingApi _ (witnessable, BuildTxWith (Old.KeyWitness _)) =
  return (witnessable, AnyKeyWitness)
fromExistingApi _ (witnessable, BuildTxWith (Old.ScriptWitness _ Old.SimpleScriptWitness{})) =
  return (witnessable, AnyKeyWitness)
fromExistingApi eon (witnessable, BuildTxWith (Old.ScriptWitness _ oldApiPlutusScriptWitness)) =
...
convertToNewPlutusScriptWitness eon (Old.PlutusScriptWitness _ v scriptOrRefInput datum scriptRedeemer execUnits) witnessable = do
  let d = createDatum witnessable v datum
  newScriptWitness <-
    obtainConstraints v $
...
createDatum missingContext plutusVersion oldDatum =
  case (missingContext, oldDatum) of
    ...
    (WitTxIn{}, _) ->
      error $
        unlines
          [ "createDatum: invalid combination of witnessable and script datum"
...
toCip69Datum WitTxIn{} Old.PlutusScriptV2 (Old.ScriptDatumForTxIn (Just r)) = SpendingScriptDatum r
toCip69Datum WitTxIn{} Old.PlutusScriptV1 (Old.ScriptDatumForTxIn (Just r)) = SpendingScriptDatum r
-- \^ V2 and V3 scripts can have inline datums
toCip69Datum WitTxIn{} Old.PlutusScriptV3 Old.InlineScriptDatum = InlineDatum
...
toCip69Datum WitTxIn{} scriptVersion datum =
  error $
    unlines
      [ "toCip69Datum: invalid combination of script version and script datum"
...
legacyWitnessConversion eon = mapM (fromExistingApi eon)

...
estimateBalancedTxBody
  ...
  exUnitsMap =
    Fee.estimateBalancedTxBody
      ...
      stakeDelegDeposits
      drepDelegDeposits
      (Map.mapKeys (scriptWitnessIndexShim w) exUnitsMap)

scriptWitnessIndexShim
  :: Era era -> Ledger.PlutusPurpose Ledger.AsIx (LedgerEra era) -> ScriptWitnessIndex
scriptWitnessIndexShim era purpose = toScriptIndex (convert era) $ purposeShim era purpose

purposeShim
  :: Era era
  -> Ledger.PlutusPurpose Ledger.AsIx (LedgerEra era)
  -> Ledger.PlutusPurpose Ledger.AsIx (ShelleyLedgerEra era)
purposeShim ConwayEra pp = pp

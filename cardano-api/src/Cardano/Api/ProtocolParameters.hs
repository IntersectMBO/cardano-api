...
data CommonProtocolParametersUpdate
  = CommonProtocolParametersUpdate
  { cppMinFeeA :: StrictMaybe Ledger.CoinPerByte
...
createCommonPParamsUpdate CommonProtocolParametersUpdate{..} =
  emptyPParamsUpdate
    & Ledger.ppuTxFeePerByteL .~ cppMinFeeA
    & Ledger.ppuTxFeeFixedL .~ cppMinFeeB
...
data ProtocolParametersUpdate
  ...
  -- ^ The constant factor for the minimum fee calculation.
  , protocolUpdateTxFeePerByte :: Maybe L.CoinPerByte
...
instance ToCBOR L.CoinPerByte where
  toCBOR = toCBOR . fromCompact . L.unCoinPerByte

instance CBOR.FromCBOR L.CoinPerByte where
  fromCBOR = L.CoinPerByte . L.compactCoinOrError <$> CBOR.fromCBOR

instance CBOR.FromCBOR (L.CompactForm Coin) where
  fromCBOR = L.compactCoinOrError <$> CBOR.fromCBOR
...
toShelleyCommonPParamsUpdate
  ProtocolParametersUpdate
    ...
    let ppuCommon =
          emptyPParamsUpdate
            & ppuTxFeePerByteL .~ noInlineMaybeToStrictMaybe protocolUpdateTxFeePerByte
...
fromShelleyCommonPParamsUpdate ppu =
  ProtocolParametersUpdate
    ...
    , protocolUpdateTxFeePerByte = strictMaybeToMaybe (ppu ^. ppuTxFeePerByteL)

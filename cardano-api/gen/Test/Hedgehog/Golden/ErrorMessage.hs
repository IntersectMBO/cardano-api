...
testAllErrorMessages goldenFilesLocation errs = withFrozenCallStack $ do
  ...
  let err = undefined :: a
      ...
      notTestedConstructors =
        [ c | c <- allConstructors, c `notElem` testedConstructors ]
      testAllConstructors =
        testProperty "check if all constructors are tested"
        . withTests 1
        . property
        $ do
...
testAllErrorMessages_ goldenFilesLocation moduleName typeName errs =
  withFrozenCallStack $ do
    testGroup typeName
      $ fmap
        (uncurry $ testErrorMessage_ goldenFilesLocation moduleName typeName)
        errs

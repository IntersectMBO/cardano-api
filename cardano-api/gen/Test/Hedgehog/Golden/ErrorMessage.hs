...
testAllErrorMessages ::
  ...
  -> TestTree
testAllErrorMessages goldenFilesLocation errs =
  ...
  -- it's equivalent of having @err = undefined :: a@
    ...
    testGroup typeName $ testAllConstructors : map (testErrorMessage goldenFilesLocation) errs
...
testErrorMessage ::
  ...
  -> a -- ^ value to test
  -> TestTree
testErrorMessage goldenFilesLocation err =
  withFrozenCallStack $ do

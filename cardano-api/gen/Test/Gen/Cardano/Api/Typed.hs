...
genScriptDataSchema =
  Gen.element [ ScriptDataJsonNoSchema, ScriptDataJsonDetailedSchema ]
...
genScriptData =
  Gen.recursive
    Gen.choice
    [ ScriptDataNumber <$> genInteger, ScriptDataBytes <$> genByteString ]
    -- The Gen.recursive combinator calls these with the size halved
    [ ScriptDataConstructor <$> genConstructorInteger <*> genScriptDataList
...
genPolicyId =
  Gen.frequency
      -- mostly from a small number of choices, so we get plenty of repetition
    [ ( 9
      , Gen.element [ fromString (x : replicate 55 '0') | x <- [ 'a' .. 'c' ] ] )
       -- and some from the full range of the type
    , ( 1, PolicyId <$> genScriptHash ) ]

genAssetId :: Gen AssetId
genAssetId = AssetId <$> genPolicyId <*> genAssetName
...
genValue :: MaryEraOnwards era
         -> Gen AssetId
         -> Gen Quantity
         -> Gen (L.Value (ShelleyLedgerEra era))
genValue w genAId genQuant =
  toLedgerValue w . valueFromList
  <$> Gen.list (Range.constant 0 10) ((,) <$> genAId <*> genQuant)
...
genSigningKey
  :: Key keyrole => ShelleyApi.AsType keyrole -> Gen (SigningKey keyrole)
...
genStakeAddressReference =
  Gen.choice
    [ StakeAddressByValue <$> genStakeCredential, return NoStakeAddress ]
...
genTxOutValue sbe =
  ...
  $ TxOutValueShelleyBased sbe <$> genValueForTxOut sbe
...
genUTxO era =
  ...
  <$> Gen.map
    (Range.constant 0 5)
    ((,) <$> genTxIn <*> (toCtxUTxOTxOut <$> genTxOutTxContext era))
...
genTxValidityLowerBound =
  inEonForEra (pure TxValidityNoLowerBound) (\w -> TxValidityLowerBound w
                                             <$> genTtl)

-- TODO: Accept a range for generating ttl.
genTxValidityUpperBound
  :: ShelleyBasedEra era -> Gen (TxValidityUpperBound era)
genTxValidityUpperBound sbe = TxValidityUpperBound sbe <$> Gen.maybe genTtl
...
genTxMetadataInEra =
  inEonForEra
    (pure TxMetadataNone)
    (\w
     -> Gen.choice [ pure TxMetadataNone, TxMetadataInEra w <$> genTxMetadata ])
...
genTxWithdrawals =
  inEonForEra
    ...
    (\w -> Gen.choice [ pure TxWithdrawalsNone, pure (TxWithdrawals w mempty) ])
...
genStakeAddressRequirements =
  caseShelleyToBabbageOrConwayEraOnwards
    (\w -> StakeAddrRegistrationPreConway w <$> genStakeCredential)
    (\w -> StakeAddrRegistrationConway w <$> genLovelace <*> genStakeCredential)

genTxUpdateProposal :: CardanoEra era -> Gen (TxUpdateProposal era)
genTxUpdateProposal sbe =
  Gen.choice
  $ catMaybes [ Just $ pure TxUpdateProposalNone, forEraInEon sbe Nothing $ \w
...
genTxMintValue =
  inEonForEra
    (pure TxMintNone)
    (\supported -> Gen.choice
       [ pure TxMintNone
       , TxMintValue supported <$> genValueForMinting supported
...
genTxBodyContent sbe = do
  ...
  txProtocolParams <- BuildTxWith
    <$> forEraInEon era (pure Nothing) (Gen.maybe . genValidProtocolParameters)
...
genTxTotalCollateral =
  inEonForEra (pure TxTotalCollateralNone) (\w -> TxTotalCollateral w
                                            <$> genPositiveLovelace)
...
genTxByron = do
  makeSignedByronTransaction <$> genWitnessesByron <*> genTxBodyByron
...
genFeaturedInEra
  :: () => Alternative f => eon era -> f a -> f (Featured eon era a)
genFeaturedInEra witness gen = Featured witness <$> gen
...
genMaybeFeaturedInEra f = inEonForEra (pure Nothing) $ \w -> pure Nothing
  <|> fmap Just (genFeaturedInEra w (f w))

genTxScriptValidity :: CardanoEra era -> Gen (TxScriptValidity era)
genTxScriptValidity =
  inEonForEra (pure TxScriptValidityNone) (\w -> TxScriptValidity w
                                           <$> genScriptValidity)
...
genTx era = makeSignedTransaction <$> genWitnesses era <*> genTxBody era
...
genShelleyBootstrapWitness sbe =
  makeShelleyBootstrapWitness sbe <$> genWitnessNetworkIdOrByronAddress
  <*> genTxBody sbe
  <*> genSigningKey AsByronKey

genShelleyKeyWitness :: () => ShelleyBasedEra era -> Gen (KeyWitness era)
genShelleyKeyWitness sbe =
  makeShelleyKeyWitness sbe <$> genTxBody sbe <*> genShelleyWitnessSigningKey

genShelleyWitness :: () => ShelleyBasedEra era -> Gen (KeyWitness era)
genShelleyWitness sbe =
  Gen.choice [ genShelleyKeyWitness sbe, genShelleyBootstrapWitness sbe ]
...
genRational = (\d -> ratioToRational (1 % d)) <$> genDenominator
  where
...
genRationalInt64 = (\d -> ratioToRational (1 % d)) <$> genDenominator
  where
...
genProtocolParameters era = do
  ...
  protocolParamUTxOCostPerByte <- inEonForEra
    @BabbageEraOnwards
    (pure Nothing)
    (const (Just <$> genLovelace))
    era
...
genValidProtocolParameters sbe =
  shelleyBasedEraTestConstraints sbe $ LedgerProtocolParameters <$> Q.arbitrary
...
genProtocolParametersUpdate era = do
  ...
  protocolUpdateUTxOCostPerByte <- inEonForEra
    @BabbageEraOnwards
    (pure Nothing)
    (const (Just <$> genLovelace))
    era
...
genExecutionUnits =
  ExecutionUnits <$> Gen.integral (Range.constant 0 1000)
...
genTxOutDatumHashTxContext era = case era of
  ...
  ShelleyBasedEraConway  -> Gen.choice
    [ pure TxOutDatumNone
...
genTxOutDatumHashUTxOContext
  :: ShelleyBasedEra era -> Gen (TxOutDatum CtxUTxO era)
...
genGovernancePoll =
  GovernancePoll <$> Gen.text (Range.linear 1 255) Gen.unicodeAll
  <*> Gen.list
    (Range.constant 1 10)
...
genGovernancePollAnswer =
  GovernancePollAnswer <$> genGovernancePollHash
...
genProposals w =
  ...
  $ TxProposalProcedures <$> genTxProposalsOSet w
  <*> return (BuildTxWith mempty)

genTxProposalsOSet :: ConwayEraOnwards era
...
genProposal :: ConwayEraOnwards era
            -> Gen (L.ProposalProcedure (ShelleyLedgerEra era))
genProposal w = conwayEraOnwardsTestConstraints w Q.arbitrary

-- TODO: Generate map of script witnesses
genVotingProcedures
  :: ConwayEraOnwards era -> Gen (Api.TxVotingProcedures BuildTx era)

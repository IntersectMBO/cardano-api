...
genJsonForTxMetadata mapping = Gen.sized $ \sz -> Aeson.object
  <$> Gen.list
...
genJsonForTxMetadataValue TxMetadataJsonNoSchema = genJsonValue
  where
    ...
    genJsonValue = Gen.sized $ \sz -> Gen.frequency
    ...
    genJsonMap = Gen.sized $ \sz -> Gen.list (Range.linear 0 (fromIntegral sz))
...
genJsonForTxMetadataValue TxMetadataJsonDetailedSchema = genJsonValue
  where
    ...
    genJsonValue = Gen.sized $ \sz -> Gen.frequency
      ...
      , ( fromIntegral (signum sz)
        , singleFieldObject "list" <$> Gen.scale (`div` 2) genJsonList )
      , ( fromIntegral (signum sz)
        , singleFieldObject "map" <$> Gen.scale (`div` 2) genJsonMap ) ]
    ...
    genJsonList :: Gen Aeson.Value
    genJsonList = fmap toJSON $ Gen.sized $ \sz
      -> Gen.list (Range.linear 0 (fromIntegral sz)) genJsonValue

    genJsonMap :: Gen Aeson.Value
    genJsonMap = fmap toJSON $ Gen.sized $ \sz -> Gen.list
      (Range.linear 0 (fromIntegral sz))
...
genTxMetadata = Gen.sized $ \sz -> TxMetadata . Map.fromList
  <$> Gen.list
    (Range.linear 0 (fromIntegral sz))
    ((,) <$> Gen.word64 Range.constantBounded <*> genTxMetadataValue)

genTxMetadataValue :: Gen TxMetadataValue
genTxMetadataValue = Gen.sized $ \sz -> Gen.frequency
  ...
  where
    ...
    genTxMetaList = Gen.sized $ \sz
      -> Gen.list (Range.linear 0 (fromIntegral sz)) genTxMetadataValue

    genTxMetaMap :: Gen [ ( TxMetadataValue, TxMetadataValue ) ]
    genTxMetaMap = Gen.sized $ \sz -> Gen.list
      (Range.linear 0 (fromIntegral sz))

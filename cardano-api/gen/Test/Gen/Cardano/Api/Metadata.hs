  ...
  where
    ...
    genJsonValue =
      Gen.sized $ \sz ->
        Gen.frequency
          ...
          , (fromIntegral (signum sz), Aeson.toJSON <$> Gen.scale (`div` 2) genJsonList)
          , (fromIntegral (signum sz), Aeson.object <$> Gen.scale (`div` 2) genJsonMap)
          ]
    ...
    genJsonText =
      ...
      where
        ...
        genBytes =
          (bytesPrefix <>) . Text.decodeUtf8 . Base16.encode . BS.pack
            <$> Gen.list (Range.linear 0 64) (Gen.word8 Range.constantBounded)
    genJsonList :: Gen [Aeson.Value]
    genJsonList = Gen.sized $ \sz -> Gen.list (Range.linear 0 (fromIntegral sz)) genJsonValue
    genJsonKey :: Gen Aeson.Key
    genJsonKey = fmap Aeson.fromText genJsonText
    genJsonMap :: Gen [(Aeson.Key, Aeson.Value)]
    genJsonMap =
      Gen.sized $ \sz ->
        Gen.list (Range.linear 0 (fromIntegral sz)) $ (,) <$> genJsonKey <*> genJsonValue
genJsonForTxMetadataValue TxMetadataJsonDetailedSchema = genJsonValue
  where
    ...
    genJsonValue =
      Gen.sized $ \sz ->
        Gen.frequency
          ...
          , (fromIntegral (signum sz), singleFieldObject "list" <$> Gen.scale (`div` 2) genJsonList)
          , (fromIntegral (signum sz), singleFieldObject "map" <$> Gen.scale (`div` 2) genJsonMap)
          ]
    ...
    genJsonBytes =
      toJSON . Text.decodeLatin1 . Base16.encode . BS.pack
        <$> Gen.list (Range.linear 0 64) (Gen.word8 Range.constantBounded)
    genJsonText :: Gen Aeson.Value
    genJsonText =
      fmap toJSON $ Gen.ensure validText $ Text.pack <$> Gen.list (Range.linear 0 64) Gen.alphaNum
      where
        validText t = BS.length (Text.encodeUtf8 t) <= 64
    genJsonList :: Gen Aeson.Value
    genJsonList =
      fmap toJSON $ Gen.sized $ \sz -> Gen.list (Range.linear 0 (fromIntegral sz)) genJsonValue
    genJsonMap :: Gen Aeson.Value
    genJsonMap =
      fmap toJSON
        $ Gen.sized
        $ \sz ->
            Gen.list (Range.linear 0 (fromIntegral sz)) $ mkKVPair <$> genJsonValue <*> genJsonValue
      where
...
genTxMetadata =
  Gen.sized $ \sz ->
    TxMetadata . Map.fromList
      <$> Gen.list
            (Range.linear 0 (fromIntegral sz))
            ((,) <$> Gen.word64 Range.constantBounded <*> genTxMetadataValue)
...
genTxMetadataValue =
  Gen.sized $ \sz ->
    Gen.frequency
      ...
      , (fromIntegral (signum sz), TxMetaList <$> Gen.scale (`div` 2) genTxMetaList)
      , (fromIntegral (signum sz), TxMetaMap <$> Gen.scale (`div` 2) genTxMetaMap)
      ]
  where
    ...
    genTxMetaBytes = BS.pack <$> Gen.list (Range.linear 0 64) (Gen.word8 Range.constantBounded)
    genTxMetaText :: Gen Text
    genTxMetaText = Text.pack <$> Gen.list (Range.linear 0 64) Gen.alphaNum
    genTxMetaList :: Gen [TxMetadataValue]
    genTxMetaList =
      Gen.sized $ \sz -> Gen.list (Range.linear 0 (fromIntegral sz)) genTxMetadataValue

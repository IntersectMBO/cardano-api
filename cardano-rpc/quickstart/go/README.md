...
The first run may download a newer Go toolchain: `go.mod` declares a Go version newer than the one the pinned dev shell ships, and the `go` command fetches the declared version automatically.

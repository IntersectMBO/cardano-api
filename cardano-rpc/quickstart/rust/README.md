...
> [!IMPORTANT]
> `pallas-txbuilder`'s `build_conway_raw` performs no automatic fee or change balancing: the fee and every output must already be exact, unlike MeshTxBuilder in the [TypeScript quickstart](../typescript/README.md) or PyCardano in the [Python quickstart](../python/README.md#build-and-submit-a-transaction).
> This example builds the transaction once with a fee guessed from the protocol parameters, measures the real encoded size of that draft, recomputes the fee from the real size, then rebuilds with the corrected change output.
> It is demo scaffolding for plain ADA payments: it spends every UTxO it finds at the sender address instead of running proper coin selection, and it does not handle native assets, datums, or reference scripts.

...
Pieces 1-3 and 9 are done: `NodeKernelAccess`, the FetchBlock proto/handler, `mkNodeKernelAccess` and node wiring, and the FollowTip server-streaming endpoint.
Pieces 4-7 rewrite existing N2C methods to use `NodeKernelAccess`, pending the snapshot API ([plan-snapshot-api-phases.md](plan-snapshot-api-phases.md)).
Piece 8 is the final validation gate.

Piece 1 as built deferred the snapshot/submit capability (`withSnapshot`, `LedgerSnapshot`, `submitTx`) that pieces 4-7 depend on; [plan-snapshot-api-phases.md](plan-snapshot-api-phases.md) is the phased plan for building it before those rewrites start.
Piece 10 adds ReadMempool, WaitForTx and WatchMempool - not a migration but new methods unlocked by kernel access, depending only on its own phase 1 capability field ([plan-mempool-phases.md](plan-mempool-phases.md)).
...
|---|-------------|-------|------|
| 1 | NodeKernelAccess types and plumbing (done, as built) | `NodeKernelAccess` record, env/monad/tracing/server wiring | cardano-rpc |
| 2 | FetchBlock proto and handler (done, as built) | SyncService proto, codegen, `fetchBlockMethod`, server registration | cardano-rpc |
| 3 | mkNodeKernelAccess and node wiring (done, as built) | `mkNodeKernelAccess` from `NodeKernel`, `Run.hs` IORef wiring, E2E test | cardano-node |
...
| 9 | FollowTip streaming (done, as built) | Server-streaming chain follower via gRPC | both |
| 10 | [Mempool methods](10-mempool-methods.md) | ReadMempool, WaitForTx, WatchMempool via the mempool capability | both |
...
| **Mempool** | `addLocalTxs`, `getSnapshot` | SubmitTx (planned, piece 5), ReadMempool/WaitForTx/WatchMempool (planned, piece 10) |

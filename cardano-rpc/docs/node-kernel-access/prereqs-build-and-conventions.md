...
Note: this snapshot design is forward-looking - it is the target interface for pieces 4-7, not current code (as built, `NodeKernelAccess` is the thinner `chainDb` / `systemStart` / `readEraHistory` record).

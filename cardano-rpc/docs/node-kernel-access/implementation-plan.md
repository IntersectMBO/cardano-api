...
- 01-node-access-types.md (done) - `NodeKernelAccess` types, `Env`, `Monad`, `Tracing`, `Server`, and cabal wiring
- 02-fetchblock-proto-and-handler.md (done) - SyncService proto, codegen, `fetchBlockMethod`, server registration
- 03-mk-node-access-and-wiring.md (done) - `mkNodeKernelAccess` in cardano-node, `Run.hs` wiring, E2E test, tracing
...
- [10-mempool-methods.md](10-mempool-methods.md) - ReadMempool, WaitForTx, WatchMempool handlers over the mempool capability
...
| File | Change | Story |
|------|--------|-------|
| `cardano-api/cardano-rpc/src/Cardano/Rpc/Server.hs` | New signature, re-export `NodeKernelAccess` + `LedgerSnapshot`, IORef wiring, register `methodsSyncRpc` | 01 (done), 02 (done) |
| `cardano-api/cardano-rpc/src/Cardano/Rpc/Server/Internal/Env.hs` | `RpcEnv` holds `IORef`, drop `LocalNodeConnectInfo` | 01 (done) |
| `cardano-api/cardano-rpc/src/Cardano/Rpc/Server/Internal/Monad.hs` | `Has` instance + `MonadRpc` update | 01 (done) |
| `cardano-api/cardano-rpc/src/Cardano/Rpc/Server/Internal/Tracing.hs` | Replace N2C trace with ledger-access traces, add `TraceRpcSync` | 01 (done) |
...
| `cardano-api/cardano-rpc/src/Cardano/Rpc/Server/Internal/UtxoRpc/Submit.hs` | Rewrite submit with snapshot for era detection; add `readMempoolMethod`/`watchMempoolMethod`/`waitForTxMethod` | [05](05-rewrite-submit-method.md), [10](10-mempool-methods.md) |
...
| `cardano-api/cardano-rpc/src/Cardano/Rpc/Server/NodeKernelAccess/Type.hs` | Add `mempool` capability field | [10](10-mempool-methods.md) |
| `cardano-api/cardano-rpc/cardano-rpc.cabal` | Add `NodeKernelAccess` module, proto gen modules | 01 (done), 02 (done) |
| `cardano-node/cardano-node/src/Cardano/Node/Run.hs` | IORef + writeIORef in kernel hook | 03 (done) |
| `cardano-node/cardano-node/src/Cardano/Node/Tracing/Tracers/Rpc.hs` | Update trace instances | 03 (done) |
| `cardano-node/cardano-node/cardano-node.cabal` | Add new module | 03 (done) |
...
| `cardano-rpc/src/Cardano/Rpc/Server/Internal/UtxoRpc/Predicate.hs` | Filtering untouched by this migration; gains `matchesTxPredicate` under piece [10](10-mempool-methods.md) |

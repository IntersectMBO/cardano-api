...
hprop_tx_has_certificate_any_stake_credential_matches_mir_cert :: Property
hprop_tx_has_certificate_any_stake_credential_matches_mir_cert = H.property $ do
  -- the any_stake_credential wildcard must reach the MIR targets, which are
  -- the reward recipients of the certificate
  credential <- forAll genStakeCredential
  let target = defMessage & U5c.stakeCredential .~ mkStakeCredential credential
      cert = defMessage & U5c.mirCert .~ (defMessage & U5c.to .~ [target])
      tx = mkTx [] [] [] [cert]
      pat =
        defMessage
          & U5c.hasCertificate .~ (defMessage & U5c.anyStakeCredential .~ serialiseStakeCredential credential)

  ...
  def
    & appFuns
      [ \r -> do
          ...
          compactCoinsPerUtxoByte <-
            note "Invalid coinsPerUtxoByte" $ L.toCompact $ L.Coin coinsPerUtxoByte
          pure $ set L.ppCoinsPerUTxOByteL (L.CoinPerByte compactCoinsPerUtxoByte) r
      ...
      , \r -> do
          ...
          compactMinFeeConst <-
            note "Invalid minFeeConstant" $ L.toCompact $ L.Coin minFeeConst
          pure $ set L.ppTxFeePerByteL (L.CoinPerByte compactMinFeeConst) r

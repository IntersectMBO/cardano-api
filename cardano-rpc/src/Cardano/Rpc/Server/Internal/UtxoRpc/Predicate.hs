...
matchesFilter field mComponent =
  maybe True (\wanted -> mComponent == Just wanted) (nonEmptyField field)
...
nonEmptyField field = if BS.null field then Nothing else Just field
...
matchesAddressPattern
  ...
  -> AddressInEra era
  -> Bool
matchesAddressPattern pat address =
  matchesFilter (pat ^. UtxoRpc.exactAddress) (Just $ serialiseToRawBytes address)
    && matchesFilter (pat ^. UtxoRpc.paymentPart) mPaymentCredentialBytes
    && matchesFilter (pat ^. UtxoRpc.delegationPart) mStakeCredentialBytes
 where
  mShelleyAddress :: Maybe (Address ShelleyAddr)
  mShelleyAddress = case address of
...
matchesAddressPatternBytes pat addressBytes =
  matchesFilter (pat ^. UtxoRpc.exactAddress) (Just addressBytes)
    && matchesFilter (pat ^. UtxoRpc.paymentPart) mPaymentCredentialBytes
    && matchesFilter (pat ^. UtxoRpc.delegationPart) mStakeCredentialBytes
 where
  mShelleyAddress = do
...
matchesCertificatePattern pat cert =
  all (`matchesExactCredential` mStakeRegistration) (pat ^. UtxoRpc.maybe'stakeRegistration)
    && all (`matchesExactCredential` mStakeDeregistration) (pat ^. UtxoRpc.maybe'stakeDeregistration)
    && all matchesStakeDelegationPattern (pat ^. UtxoRpc.maybe'stakeDelegation)
    && all matchesPoolRegistrationPattern (pat ^. UtxoRpc.maybe'poolRegistration)
    && all matchesPoolRetirementPattern (pat ^. UtxoRpc.maybe'poolRetirement)
    ...
    && all
      ...
      (nonEmptyField $ pat ^. UtxoRpc.anyStakeCredential)
    && all
      (matchesAnyPoolKeyHash (certPoolKeyHashes credentials))
      (nonEmptyField $ pat ^. UtxoRpc.anyPoolKeyhash)
    && all (matchesAnyDRep (certDRepBytes credentials)) (nonEmptyField $ pat ^. UtxoRpc.anyDrep)
 where
  -- classified once, then read for all three any_* checks below
  credentials = certificateCredentials cert

  mStakeRegistration = cert ^. UtxoRpc.maybe'stakeRegistration
  mStakeDeregistration = cert ^. UtxoRpc.maybe'stakeDeregistration
  mStakeDelegation = cert ^. UtxoRpc.maybe'stakeDelegation
  mPoolRegistration = cert ^. UtxoRpc.maybe'poolRegistration
  mPoolRetirement = cert ^. UtxoRpc.maybe'poolRetirement

  mDelegationStakeCredential = mStakeDelegation <&> (^. UtxoRpc.stakeCredential)
  mDelegationPoolKeyHash = mStakeDelegation <&> (^. UtxoRpc.poolKeyhash)
  mOperatorKeyHash = mPoolRegistration <&> (^. UtxoRpc.operator)
  mRetirementPoolKeyHash = mPoolRetirement <&> (^. UtxoRpc.poolKeyhash)
  mRetirementEpoch = mPoolRetirement <&> (^. UtxoRpc.epoch)

  matchesExactCredential expected mActual =
    (credentialBytes <$> mActual) == Just (credentialBytes expected)
  ...
  matchesStakeDelegationPattern delegPat =
    isJust mStakeDelegation
      && all
        (`matchesExactCredential` mDelegationStakeCredential)
        (delegPat ^. UtxoRpc.maybe'stakeCredential)
      && matchesFilter (delegPat ^. UtxoRpc.poolKeyhash) mDelegationPoolKeyHash
  ...
  matchesPoolRegistrationPattern regPat =
    isJust mPoolRegistration
      && matchesFilter (regPat ^. UtxoRpc.operator) mOperatorKeyHash
      && matchesFilter (regPat ^. UtxoRpc.poolKeyhash) mOperatorKeyHash

  matchesPoolRetirementPattern retirePat =
    isJust mPoolRetirement
      && matchesFilter (retirePat ^. UtxoRpc.poolKeyhash) mRetirementPoolKeyHash
      && (retirePat ^. UtxoRpc.epoch == 0 || mRetirementEpoch == Just (retirePat ^. UtxoRpc.epoch))
...
credentialBytes credential =
  fromMaybe (credential ^. UtxoRpc.scriptHash) (credential ^. UtxoRpc.maybe'addrKeyHash)
  ...
  case cert ^. UtxoRpc.maybe'certificate of
    ...
    Just (Proto variant) -> case variant of
      ...
      UtxoRpc.Certificate'MirCert mirCert ->
        CertificateCredentials
          ((^. UtxoRpc.stakeCredential) <$> Proto mirCert ^. UtxoRpc.to)
          []
          []
      ...
      UtxoRpc.Certificate'RegDrepCert regDrepCert -> do
        let drepCredential = Proto regDrepCert ^. UtxoRpc.drepCredential
        CertificateCredentials [drepCredential] [] [credentialBytes drepCredential]
      UtxoRpc.Certificate'UnregDrepCert unregDrepCert -> do
        let drepCredential = Proto unregDrepCert ^. UtxoRpc.drepCredential
        CertificateCredentials [drepCredential] [] [credentialBytes drepCredential]
      UtxoRpc.Certificate'UpdateDrepCert updateDrepCert -> do

...
import Cardano.Api (SystemStart, unEpochNo, unSlotNo)
...
import Data.SOP.BasicFunctors qualified as SOP (K (..))
...
eraSummariesToProto systemStart summary =
 ...
 where
  eraNames :: [Text]
  eraNames = SOP.hcollapse eraNamesTable
...
-- | Era names in 'CardanoEras' order, as the lowercase names the UTxO RPC
-- spec uses. The explicit 'SOP.Nil' pins the table to exactly the eight
-- current Cardano eras, so a ninth era is a compile error here rather than a
-- silently misnamed summary entry.
eraNamesTable :: SOP.NP (SOP.K Text) (CardanoEras Consensus.StandardCrypto)
eraNamesTable =
  SOP.K "byron"
    SOP.:* SOP.K "shelley"
    SOP.:* SOP.K "allegra"

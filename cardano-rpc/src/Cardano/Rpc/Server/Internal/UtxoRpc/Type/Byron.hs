...
byronBlockTxs blockOrBoundary =
  case blockOrBoundary of
    Byron.ABOBBlock byronBlock ->
      byronTxToUtxoRpcTx <$> aUnTxPayload (Byron.blockTxPayload byronBlock)
      ...
      outputs =
        toList (txOutputs tx) <&> \(TxOut address value) ->
          defMessage
            ...
            & U5c.coin .~ inject (lovelaceToInteger value)

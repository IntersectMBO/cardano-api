...
getPParamsJsonMethod :: MonadRpc e m => Proto Empty -> m (Proto Rpc.PParamsJson)
getPParamsJsonMethod _ = do
  nodeConnInfo <- grab
  AnyCardanoEra era <- liftIO . throwExceptT $ determineEra nodeConnInfo
  eon <- forEraInEon @ConwayEraOnwards era (error "Minimum Conway era required") pure
  let sbe = convert eon
  ...
  let pparamsJson = conwayEraOnwardsConstraints eon $ A.encode pparams

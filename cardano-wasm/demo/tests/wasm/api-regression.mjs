...
// Run: npm install && node api-regression.mjs  (from this directory; Node >= 22.7, e.g. from `nix develop .#demo`)
...
// One certificate kind, driven the way the demo drives it: every certificate
// carries the stake credential's witness (registration includes an explicit
// deposit, which requires the credential's witness just like delegation and
// unregistration do), so fees are estimated at payment + stake = 2 witnesses.
async function certRoundTrip(makeCert) {
  ...
  const feeOneWit = await tx.estimateMinFee(pparams, 1, 0, 0);
  const fee = await tx.estimateMinFee(pparams, 2, 0, 0);
  assert(typeof fee === "bigint" && fee > 0n, `bad fee: ${fee}`);
  assert(
    fee > feeOneWit,
    `the stake witness should raise the fee: ${fee} <= ${feeOneWit}`
  );
  const idBeforeFee = await tx.getTxId();
  tx.setFee(fee);
  ...
  const signed = await tx.signWithPaymentKey(paySkey);
  const cborOneSig = await signed.txToCbor();
  signed.alsoSignWithStakeKey(stakeSkey);
  ...
  assert(
    cbor.includes(certHex),
    "signed tx CBOR does not contain the certificate"
  );
  assert(

...
ZC0ZCcardanozmwasmzm10zi0zi0zi0zminplacezmcardanozmwasmZCCardanoziWasmziInternalziJavaScriptziGRPCZC: async ($1,$2) => {{ let tx = new cardano_node.submit.AnyChainTx();                                   tx.setRaw(new Uint8Array($2.match(/[0-9a-fA-F]{2}/g).map((byte) => parseInt(byte, 16))));                                   let txList = new cardano_node.submit.SubmitTxRequest();                                   txList.addTx(tx);                                   return (($1).submit.submitTx(txList, {}).then((val) => (val.toObject().resultsList[0].ref == '')?('error: ' + val.toObject().resultsList[0].errorMessage):('ok: ' +val.toObject().resultsList[0].ref))) }},
ZC1ZCcardanozmwasmzm10zi0zi0zi0zminplacezmcardanozmwasmZCCardanoziWasmziInternalziJavaScriptziGRPCZC: async ($1,$2) => {{ let req = new cardano_node.query.ReadUtxosRequest();      let addresses = new proto.utxorpc.v1alpha.cardano.AddressArray();      addresses.addItems(btoa($2));      req.setCardanoAddresses(addresses);      let pres = await ($1).query.readUtxos(req, {});      let res = pres.toObject();      return res.itemsList.map(utxo => {        return {          txId: globalThis.cardanoWasm.base64ToHex(utxo.txoRef.hash),          txIndex: utxo.txoRef.index,          lovelace: BigInt(utxo.cardano.coin),          assets: utxo.cardano.assetsList,          datum: utxo.cardano.datum,          script: utxo.cardano.script,        };      });   }},
ZC2ZCcardanozmwasmzm10zi0zi0zi0zminplacezmcardanozmwasmZCCardanoziWasmziInternalziJavaScriptziGRPCZC: async ($1) => {{ let req = new cardano_node.query.ReadUtxosRequest();      let pres = await ($1).query.readUtxos(req, {});      let res = pres.toObject();      return res.itemsList.map(utxo => {        return {          address: atob(utxo.cardano.address),          txId: globalThis.cardanoWasm.base64ToHex(utxo.txoRef.hash),          txIndex: utxo.txoRef.index,          lovelace: BigInt(utxo.cardano.coin),          assets: utxo.cardano.assetsList,          datum: utxo.cardano.datum,          script: utxo.cardano.script,        };      });  }},
ZC3ZCcardanozmwasmzm10zi0zi0zi0zminplacezmcardanozmwasmZCCardanoziWasmziInternalziJavaScriptziGRPCZC: async ($1) => (atob((await ($1).node.getProtocolParamsJson(new proto.Empty(), {})).toObject().json)),
ZC4ZCcardanozmwasmzm10zi0zi0zi0zminplacezmcardanozmwasmZCCardanoziWasmziInternalziJavaScriptziGRPCZC: async ($1) => (($1).node.getEra(new proto.Empty(), {})),
ZC5ZCcardanozmwasmzm10zi0zi0zi0zminplacezmcardanozmwasmZCCardanoziWasmziInternalziJavaScriptziGRPCZC: async ($1) => (globalThis.createClient($1)),
ZC43ZCcardanozmwasmzm10zi0zi0zi0zminplacezmcardanozmwasmZCCardanoziWasmziInternalziJavaScriptziBridgeZC: ($1) => (BigInt($1)),
ZC44ZCcardanozmwasmzm10zi0zi0zi0zminplacezmcardanozmwasmZCCardanoziWasmziInternalziJavaScriptziBridgeZC: ($1) => (($1).toString()),
ZC45ZCcardanozmwasmzm10zi0zi0zi0zminplacezmcardanozmwasmZCCardanoziWasmziInternalziJavaScriptziBridgeZC: ($1) => (JSON.stringify($1)),
ZC46ZCcardanozmwasmzm10zi0zi0zi0zminplacezmcardanozmwasmZCCardanoziWasmziInternalziJavaScriptziBridgeZC: ($1) => (JSON.parse($1)),
